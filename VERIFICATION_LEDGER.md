...
| σ₈ / S₈ con A_s FIJADO a Planck — **DIAGNÓSTICO condicionado, NO predicción** | 0.833368 / 0.845614 (los papers imprimen 0.8335 / 0.846) | `src/p05_IS/techo_sigma8_As_fijo.py` desde `class_ssee/output/can_cold__pk.dat`; log `results/logs/p5_techo_sigma8_As_fijo.json` (Ω_m,CMB=0.308881) | 2026-08-01: ya NO se llama "el desafío 3.5σ" — ese 3.5σ es artefacto de fijar A_s a Planck (importar la tensión Planck–KiDS). A_s es libre en el modelo · **2026-09-08**: la Fuente citaba `ssee_paper5_IS_perturbations.py`, que **no contiene ni calcula** el número — mala atribución corregida. Recomputado desde el P(k) guardado: **0.833368**, donde los papers imprimen 0.8335 — difieren 1.3e-04, **45× menos que el ±0.006 que ellos mismos citan**, así que ningún número de ningún paper se mueve; lo que cambia es que ahora el Registro lleva el valor que se reproduce. **Su control queda NO CONCLUYENTE**: la referencia ΛCDM disponible sale 1.4% alta por no llevar neutrinos masivos, así que está respaldado el ORIGEN del número, no la calibración del método. Superado por R3 (A_s libre, dato crudo): S₈=0.7555±0.0192, 0.11σ |
