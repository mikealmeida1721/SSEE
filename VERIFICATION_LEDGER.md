...
| Ω_b h² (posterior MCMC reframe) | **0.02207 ± 0.00045** (≈BBN 0.02218) | `ssee_paper2_mcmc_reframe.py` (R25; era 0.02221 con Ω_m congelado); log `results/logs/mcmc_paper2_reframe.log` | 2026-07-25 (log citado el 2026-09-08: existía y contenía el valor, sólo faltaba el puntero) |
...
| σ₈ / S₈ con A_s FIJADO a Planck — **DIAGNÓSTICO condicionado, NO predicción** | 0.833368 / 0.845614 (los papers imprimen 0.8335 / 0.846) | `src/p05_IS/techo_sigma8_As_fijo.py` desde `class_ssee/output/can_cold__pk.dat`; log `results/logs/p5_techo_sigma8_As_fijo.json` (Ω_m,CMB=0.308881) | 2026-08-01: ya NO se llama "el desafío 3.5σ" — ese 3.5σ es artefacto de fijar A_s a Planck (importar la tensión Planck–KiDS). A_s es libre en el modelo · **2026-09-08**: la Fuente citaba `ssee_paper5_IS_perturbations.py`, que **no contiene ni calcula** el número — mala atribución corregida. Recomputado desde el P(k) guardado: **0.833368**, donde los papers imprimen 0.8335 — difieren 1.3e-04, **45× menos que el ±0.006 que ellos mismos citan**, así que ningún número de ningún paper se mueve; lo que cambia es que ahora el Registro lleva el valor que se reproduce. **PROCEDENCIA INCOMPLETA, y no se cita como certificado**: la corrida CLASS de la que sale **no tenía `.ini`** (no consta con qué se corrió), **estaba fuera del repo** (`class_ssee/output/` está en `.gitignore`; copiada a `results/logs/p5_techo_evidencia/` el 2026-09-08 para que exista), y su gemela del mismo minuto es la corrida de DOS SECTORES con la partícula, retirada — el techo nació como su término de comparación. Ninguna de las corridas guardadas lleva **neutrinos masivos**, y el fondo canónico de SSEE sí los lleva (Σm_ν=0.06849 eV), así que 0.8335 podría ser el techo de una variante sin ellos y no el del modelo. Control útil (mismo integrador en los dos): SSEE/ΛCDM = **1.0137**, SSEE da 1.4% más estructura. **Pendiente para cerrarlo**: `.ini` versionado con el fondo canónico y sus neutrinos. Superado por R3 (A_s libre, dato crudo): S₈=0.7555±0.0192, 0.11σ |
...
| **S₈ ΛCDM control metodológico (Paper 6, MCMC R4 contra KiDS CRUDO)** | **0.7571±0.0194 — 0.06σ KiDS** · χ²_min=**262.746**/212 dof | Cobaya+CAMB, 4 cadenas MPI, R−1=0.0256, 13 libres (fondo LIBRE); log `results/logs/growth_2026-07/R4_lcdm_kids_S8.json` | 2026-08-07 (corrida) · 2026-09-07 (χ²_min recuperado de la cadena y escrito al log: Paper 6 ya lo publicaba y el log no lo respaldaba). Licencia la comparación Δχ²=2.65 con Δk=4 de la tabla S₈ |
| **`ω_c` que pide BOSS DR12 (perfil, amplitud propia de cada modelo)** | **SSEE 0.117524±0.003933 → 0.51σ de `KAL₀·ω_b·n_s`** · ΛCDM 0.114726±0.003714 → 1.42σ de Planck 0.1200 | `src/p06_growth/perfil_wc_boss.py` y `perfil_wc_boss_lcdm.py`; logs `results/logs/perfil_wc_boss.log` + `results/logs/perfil_wc_boss_lcdm.log` | 2026-09-08. Con la amplitud del CMB clavada dan 0.114108±0.003099 (1.74σ) y 0.112169±0.003455 (2.27σ): los DOS fondos se desplazan (−2.9% y −2.2%) ⟹ el desplazamiento es del DATO. **La brecha de amplitud NO es `ω_c` disfrazado.** Control R53 pre-registrado: al devolver su amplitud, `ω_c` vuelve hacia su referencia en los dos casos. El χ² con `ω_c` LIBRE **no se cita** — en SSEE ese parámetro es algebraico y soltarlo saca al modelo de sí mismo. Paper 6 §`par:wc-profile` |
...
- **67.7869 ⁺⁰·³⁵¹/₋₀·³⁵² — prior H_alg 67.962 + DESI DR2 + ω_m algebraico FIJO (R25)
...
2. **✓ H₀ consistente:** 67.53±0.35 con prior Planck (0.26σ) y 67.79±0.35 con prior
   H_alg (reframe). Ambos coherentes (Δ=0.33<0.6σ); suben del 66.41 con bug 0.160.
3. **✓ Ω_b h² ≈ BBN:** posterior 0.02207±0.00045 vs OP-1 algebraico 0.02242 (0.64σ).
