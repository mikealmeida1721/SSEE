...
ax1.loglog(k_c, P_c, 'b--', lw=1.6, label=rf'SSEE cold ceiling ($\sigma_8={s8_cold:.3f}$)', alpha=0.85)
ax1.loglog(k_p, P_p, 'r-',  lw=2.2, label=rf'SSEE $\varphi$-DM forward ($m_\varphi=40.7$ eV, $\sigma_8={s8_part:.3f}$)', zorder=4)
ax1.axvline(k_fs, color='orange', ls='--', lw=1.5, label=rf'$k_{{fs}}={k_fs}$ h/Mpc (DW, falsifiable)')
...
ax1.set_title(r'SSEE-V3.6: forward $\varphi$-DM $P(k)$ — CLASS output, zero fit', fontsize=13)
...
ax2 = axes[1]
ax2.semilogx(kg, ratio, 'r-', lw=2, label=r'$P_{\varphi\mathrm{DM}}/P_\mathrm{cold}$ (CLASS, forward)')
...
ax2.set_ylabel(r'$P_{\varphi\mathrm{DM}}/P_\mathrm{cold}$', fontsize=12)
ax2.set_title(r'Free-streaming suppression measured by CLASS (not imposed)', fontsize=12)
...
ax.errorbar(Z_RSD, fo, yerr=fe, fmt='ko', capsize=4, ms=6, label='RSD data (6 surveys)')
...
ax.set_title(rf'SSEE-V3.6: $f\sigma_8$ forward two-sector — mean tension {np.mean(np.abs(t_2s)):.2f}$\sigma$ ($<1\sigma$; single-sector {np.mean(np.abs(t_single)):.2f}$\sigma$, $\Lambda$CDM 0.73$\sigma$)', fontsize=11)
