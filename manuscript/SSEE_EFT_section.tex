...
$66.55^{+0.44}_{-0.44}$~km\,s$^{-1}$\,Mpc$^{-1}$ by the MCMC analysis of
Paper~2 (MIRA-prior, self-consistent calibration).  Given the energy scale $\rho_{\mathrm{DE},0}$, the amplitude reads
%
\begin{equation}
  \mathcal{A}
...
$\gamma_{\mathrm{IS}} = 0.554 \pm 0.001$ — consistent with $\gamma_{\Lambda\mathrm{CDM}} \approx 0.55$
...
with $\Omega_{m,\mathrm{cosm}}=0.320$ in the Poisson source,
yields $\sigma_8=0.820$ and $S_8=0.847$ at the \emph{single-sector linear} level
($3.9\sigma$ KiDS).  This linear single-sector value is not the final SSEE prediction:
the two-sector $\varphi$-Dark Matter of Paper~6 (canonical particle
$m_\phi=36.95$~eV), whose free-streaming suppresses small-scale power below
$k_{\mathrm{fs}}=0.659\,h\,\mathrm{Mpc}^{-1}$, brings the prediction to
$\sigma_8=0.742$, $S_8=0.766$ ($0.01\sigma$ KiDS), resolving the tension.
...
$\mathcal{M} = (3\varphi+\pi)/4 \approx 1.9989$ (Paper~3, \S2.3).
...
\begin{enumerate}
  \item \textbf{Growth index.}
        ...
        gives $\gamma_{\mathrm{IS}} = 0.554$, closer to $\Lambda$CDM.
...
Growth index $\gamma_\mathrm{bg}$ & \checkmark Numerical & $0.657 \pm 0.002$ (IS background, CDM growth eq.); full IS: $0.554$ (Paper~5) \\
$S_8$ (CMB sector) & \checkmark Numerical & $0.837$ (IS bkg); $0.847$ (full IS single-sector, Paper~5); $0.766$ (two-sector $\varphi$-DM, Paper~6; $0.01\sigma$ KiDS) \\
