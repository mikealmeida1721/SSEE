...
\end{equation}
%
we solve the system numerically (\texttt{src/ssee\_is\_growth.py}) with
...
used as an ansatz in Paper~3.  The discrepancy is disclosed explicitly: the IS
formalism provides the causal theoretical structure, while $\gamma = \varphi^{-1}$
remains a target prediction to be confirmed or falsified by forthcoming
Stage-IV weak-lensing surveys (LSST, Euclid).
...
see \texttt{src/ssee\_inflation\_connection.py}).
