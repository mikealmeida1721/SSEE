...
$67.95^{+0.40}_{-0.40}$~km\,s$^{-1}$\,Mpc$^{-1}$ by the MCMC analysis of
Paper~2 ($\omega_m$-direct CMB anchor as prior).  Given the energy scale $\rho_{\mathrm{DE},0}$, the amplitude reads
%
\begin{equation}
  \mathcal{A}
...
SSEE structural constants is $P_{sc}/\mathrm{IGNIS}$, where
$P_{sc} = 2\varphi+\pi\approx6.378$ (Dynamical Evolution Scalar) and
$\mathrm{IGNIS} = \pi+P_{sc}\approx9.519$ (Disruptive convergence, the $w_a$ denominator).
...
%
\begin{equation}
  \Gamma = -\frac{P_{sc}}{\mathrm{IGNIS}} = -\frac{P_{sc}}{\pi+P_{sc}},
  \label{eq:Gamma_ssee}
\end{equation}
%
...
%
\begin{equation}
  \boxed{w_a = -\frac{P_{sc}}{\mathrm{IGNIS}} = -\frac{P_{sc}}{\pi+P_{sc}} \approx -0.66997.}
  \label{eq:wa_ssee}
\end{equation}
%
...
It differs by 6\% from the algebraic prediction $\gamma = \varphi^{-1} = 0.618$.
...
Note that the direct IS background growth-factor integral (this Appendix) yields $S_8=0.837$,
...
with $\Omega_{m,\mathrm{CMB}}=0.30889$ in the Poisson source,
...
amplitude ceiling is $S_8=0.846$, $3.5\sigma$).  This single-sector value is not the final SSEE prediction:
the two-sector $\varphi$-Dark Matter of Paper~6 (canonical particle
$m_\phi=40.70$~eV), whose free-streaming suppresses small-scale power below
$k_{\mathrm{fs}}=0.754\,h\,\mathrm{Mpc}^{-1}$, brings the prediction to
$\sigma_8=0.747$, $S_8=0.758$ ($0.04\sigma$ KiDS), resolving the tension.

\paragraph{$S_{8}$ constraint.}
Using the IS growth factor $D_{\mathrm{IS}}(a)$ and the CMB-sector matter
density $\Omega_{m,\mathrm{CMB}} = \omega_m/h^2 = 0.30889$
...
$S_{8} \approx 0.59$ ($6\sigma$ tension); per the pre-registered
two-$\Omega_m$ criterion (Paper~1 \S\ref{sec:two_omega_m}), the $S_8$ Poisson
source lives on clustering scales $k<k_{\rm fs}$ and therefore takes
$\Omega_{m,\mathrm{CMB}}=0.30889$, not the dynamical value. The full
two-sector resolution is developed in Paper~6.
...
$\Omega_{m,\mathrm{CMB}} = \omega_m/h^2 = 0.30889$ (Paper~3), built from the
...
$\Omega_{m,\mathrm{dyn}}=0.160$ to the CMB scale through a MIRA factor
...
enters as an observational input.  Notably, $w_a = -P_{sc}/\mathrm{IGNIS}$ is
...
\begin{enumerate}
  \item \textbf{Growth index.}
        ...
        $\varphi^{-1} = 0.618$.  The full IS-coupled result (Paper~5)
...
$S_8$ (CMB sector) & \checkmark Numerical & $0.837$ (IS bkg); $0.846$ (single-sector ceiling, Paper~5); $0.758$ (two-sector $\varphi$-DM, Paper~6; $0.04\sigma$ KiDS) \\
