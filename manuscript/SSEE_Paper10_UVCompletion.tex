...
$H_0^{\rm local,alg} = 73.040$ km/s/Mpc when applied to $H_0^{\rm alg}$,
matching the SH0ES central value $73.04\pm1.04$ km/s/Mpc \citep{Riess2022}
to rounding precision (a numerical coincidence within the Type-P input).
Applied to the canonical $H_0^{\rm MIRA}=67.068$, the corresponding UV
prediction is $H_0^{\rm local,MIRA} \approx 72.077$ km/s/Mpc ($0.93\sigma$
from SH0ES) --- the physically meaningful UV value within this paper.
...
km/s/Mpc on the Type-P anchor, numerically coincident with the SH0ES central
value $73.04 \pm 1.04$ km/s/Mpc \citep{Riess2022}; on the canonical MIRA anchor
it is $H_0^{\rm UV,MIRA} = 72.08$ km/s/Mpc ($0.93\sigma$ from SH0ES). Both UV
results are conditional on Postulate~C.1 (see §\ref{sec:theorem}).
...
$M^4/\rhoc = 5\phiG^8 = 234.89$, so $H_0^{\rm UV} = 73.04$~km/s/Mpc is
...
Freedman et al.\ \citep{Freedman2024} & --- & --- & $69.96 \pm 1.54$ & $1.9\sigma$ (UV) \\
\bottomrule
\end{tabular}
\end{table}
...
4-digit precision reported by SH0ES \citep{Riess2022}.  This agreement is a rounding
...
$H_0 = 69.96\pm1.54$\,km/s/Mpc.  The SSEE UV prediction $H_0^{\rm UV}=73.040$\,km/s/Mpc
lies $\sim1.9\sigma$ above this anchor; the IR prediction $H_0^{\rm IR}=72.86$\,km/s/Mpc
lies $\sim1.9\sigma$ above it as well.
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    The current SSEE value $73.040$ km/s/Mpc falls inside the SH0ES band.
  }
...
\begin{equation}
  ...
  \;=\; 73.040\;\text{km/s/Mpc}\,.
\end{equation}
Conditional on that postulate, the agreement $H_0^{\rm UV} < 0.001\sigma$ from
SH0ES is a conditional theorem rather than a coincidence; it is not an
...
central value to within $0.001$ km/s/Mpc which, conditional on Postulate~C.1,
indicates that the UV correction is a consistent completion rather than a
destabilisation of the IR identity. As stressed throughout, this agreement is
a self-consistency check, not an independent prediction.
