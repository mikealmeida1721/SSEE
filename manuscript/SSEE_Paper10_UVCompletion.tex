...
Applied to the global background anchor $H_0=3(\phiG+\pi)^2=67.962$ (Postulate~D,
Paper~1) this gives $H_0^{\rm IR}=72.86$ km/s/Mpc ($0.17\sigma$ from SH0ES); the
earlier MIRA-mapped anchor $67.037$ (giving $71.87$, $1.12\sigma$) is superseded
by the 2026 reframe.
...
global anchor $H_0=67.962$, the UV prediction is
$H_0^{\rm UV} = 73.040$ km/s/Mpc ($0.00\sigma$ from SH0ES), matching the SH0ES
central value $73.04\pm1.04$ km/s/Mpc \citep{Riess2022} --- the headline UV
value of this paper, refining the IR cascade $72.86$ ($0.17\sigma$).
\textbf{The UV value is conditional on Postulate~C.1 (UV-IR $\varphi/\pi$
separability; §\ref{sec:theorem})}, which fixes $\alphaK^{\rm full}$ via the
cutoff $M$; the IR value $72.86$ is independent of it.
...
$H_0^{\rm IR} = 72.86$ km/s/Mpc ($0.17\sigma$ SH0ES; Paper~9) is
...
\end{equation}
The $0.17\sigma$ agreement uses the global background anchor
$H_0 = 3(\phiG+\pi)^2 = 67.962$ (Postulate~D, Paper~1), the single dimensionful
input shared by every sector of the suite under the 2026 reframe. The earlier
register used the MIRA-mapped CMB anchor $H_0^{\rm MIRA} = 67.037$ ($\to 71.87$,
$1.12\sigma$); this is now superseded, the matter factor $\pi/\varphi$ pairing
the CMB sector with the single anchor $67.96$ (Paper~1).
...
With this value, the corrected Hubble constant on the global anchor is
$H_0^{\rm UV} = 73.040$ km/s/Mpc ($0.00\sigma$ from SH0ES) --- the headline UV
prediction, coincident with the SH0ES central value $73.04 \pm 1.04$ km/s/Mpc
\citep{Riess2022}, refining the IR cascade $72.86$ ($0.17\sigma$). The UV value
is conditional on Postulate~C.1 (see §\ref{sec:theorem}), which fixes
$\alphaK^{\rm full}$ via the cutoff $M$; the IR value is independent of it.
...
evaluated at the model's physical Hubble rate $H_0 = 67.962$~km/s/Mpc
...
($H_0^{\rm UV}=73.040$ on the global anchor $67.962$) is
...
\begin{equation}
  H_0^{\rm UV}
  \;=\; \frac{H_0}{1 - \fsc^{\rm UV}}
  \;=\; \frac{67.962}{1 - 0.06952}
  \;=\; 73.040 \text{ km/s/Mpc}\,,
  \label{eq:H0UV}
\end{equation}
the headline UV prediction ($0.00\sigma$ from SH0ES), refining the IR cascade
$72.86$ ($0.17\sigma$) on the same global anchor $H_0=3(\phiG+\pi)^2$.
...
\centering
\caption{%
  IR vs.\ UV Hubble constant predictions on the global background anchor
  $H_0=3(\phiG+\pi)^2=67.962$ (Postulate~D). The UV completion with
  $M^4 = 45\alpha^2\rhoc$ sharpens the residual tension from $0.17\sigma$ (IR)
  to $0.00\sigma$ (UV), conditional on Postulate~C.1 (see §\ref{sec:theorem}).
  The superseded MIRA-mapped anchor $67.037$ gave $71.87\!\to\!72.05$
  ($1.12\!\to\!0.96\sigma$), retired under the 2026 reframe.
}
\label{tab:H0}
\begin{tabular}{lllll}
\toprule
...
\midrule
IR ($M\to\infty$, Papers 1--9) & 0.40330 & 0.06725 & 72.86 & $0.17\sigma$ \\
UV ($M^4=45\alpha^2\rhoc$, this paper) & 0.41691 & 0.06952 & \textbf{73.040} & $0.00\sigma$ (cond.$^{a}$) \\
\midrule
SH0ES \citep{Riess2022}       & --- & --- & $73.04 \pm 1.04$  & --- \\
Freedman et al.\ \citep{Freedman2024} & --- & --- & $69.96 \pm 1.54$ & $2.0\sigma$ (UV) \\
\bottomrule
\end{tabular}
\end{table}
...
global anchor $H_0=67.962$, $H_0^{\rm UV} = 73.040$ km/s/Mpc ($0.00\sigma$ from
SH0ES) --- the headline UV prediction, refining the IR value $72.86$
($0.17\sigma$), and matching the SH0ES central value $73.04$ to the last digit.
...
$H_0^{\rm UV}=73.040$\,km/s/Mpc lies $\sim1.4\sigma$ above this anchor; the
canonical IR prediction $H_0^{\rm IR}=72.86$\,km/s/Mpc lies $\sim1.3\sigma$
...
$H_0^{\rm UV}$ & $H_0/(1-\fsc^{\rm UV})$ & \textbf{73.040} & Eq.~\eqref{eq:H0UV} \\
...
\begin{enumerate}
  ...
  \item The local Hubble constant should be $H_0^{\rm local} = 73.040$
        km/s/Mpc (global anchor $H_0=67.962$; Type-P $73.040$) [Eq.~\eqref{eq:H0UV}].
\end{enumerate}
...
\begin{equation}
  ...
  \;=\; \frac{H_0}{1 - \alphaK^{\rm full}(\alpha_{\rm obs})\,/\,(3\MIRA)}\,.
  \label{eq:Hpred}
\end{equation}
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    $H_0=67.962$ km/s/Mpc (solid green), with the SH0ES band
    ...
    At $\alpha=\varphi^4/3$ the canonical value is $73.040$ km/s/Mpc
    ($0.00\sigma$ from SH0ES), inside the SH0ES band.
  }
...
$H_0^{\rm UV}=73.040$ km/s/Mpc ($0.00\sigma$ from SH0ES). On the Type-P
...
\to H_0^{\rm UV} = 73.040\;\mathrm{km/s/Mpc}$ ($0.00\sigma$ from SH0ES;
...
Paper 9 & $H_0^{\rm IR} = 72.86$ (1.12$\sigma$) & UV closes to $73.040$ ($0.00\sigma$; conditional$^{a}$) \\
...
\bottomrule
\end{tabular}
\footnotesize{$^{a}$ Canonical UV result $H_0^{\rm UV}=73.040$ km/s/Mpc ($0.00\sigma$ from SH0ES),
conditional on Postulate C.1 (UV-IR separability, §\ref{sec:theorem}). It agrees with the SH0ES
central value $73.04$ to rounding; the absolute-scale origin (Postulate~D) remains open, as in $\Lambda$CDM.}
\end{table}
...
The canonical UV result $H_0^{\rm UV} = 73.040$ km/s/Mpc ($0.00\sigma$ from
SH0ES), conditional on Postulate~C.1, indicates that the UV correction is a
consistent completion rather than a destabilisation of the IR identity. It
agrees with the SH0ES central value to within $0.001$ km/s/Mpc. As stressed
...
$H_0^{\rm UV} = 73.040$ km/s/Mpc ($0.00\sigma$ from SH0ES), conditional on
Postulate~C.1, matching the SH0ES central value $73.04$ to rounding.
...
\begin{enumerate}
  ...
  \item The canonical IR Hubble residual of Paper~9
        ($H_0^{\rm IR}=72.86$, $0.17\sigma$) is reduced to
        $H_0^{\rm UV}=73.040$ ($0.00\sigma$ from SH0ES) by the UV
