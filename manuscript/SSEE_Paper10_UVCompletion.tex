...
\citep{Riess2022} to within the rounding precision of the published result (conditional on
...
Equivalently,
\begin{equation}
  M \;=\; \phiG^2 \times 5^{1/4} \times \rho_\Lambda^{1/4}
    \;\approx\; 8.81 \text{ meV}\,,
  \label{eq:Mmev}
\end{equation}
where $\rho_\Lambda^{1/4} \approx 2.25$ meV is the canonical cosmological
constant scale today.\footnote{Convention note: in the dimensionless formulation
of §\ref{sec:full_KX} we work in units $\rhoc = 1$, where $M^4 = 5\phiG^8$ is a
pure number ($\approx 234.89$) and all downstream observables ($\alpha_K$,
$f_{\rm screen}$, $\varepsilon$) are dimensionless ratios. The conversion to
physical units in Eq.~\eqref{eq:Mmev} uses the canonical $\Lambda$ scale
$\rho_\Lambda^{1/4} = 2.25$~meV rather than the SSEE $\rho_{\rm crit}$ at
$h=0.6796$ (which would give $M \approx 9.74$~meV); this choice makes $M$
naturally invariant under the $H_{\rm alg} \leftrightarrow H_{\rm MIRA}$
ambiguity and does not affect any predicted observable. The two presentations
agree at the $\sim 10\%$ level and differ only in the chosen reference
density.}
