...
$\sK^{\rm full} = 0.41691$ ($+3.37\%$ over the IR value). Applied to the
...
pressure-dilution rate $\sK = -3w_0(1+w_0) = 0.403302$ \citep{SSEE7,SSEE8} and the lensing factor
...
CMB matter density is the standard $\Omega_{m,\mathrm{CMB}}=\omega_m/h^2=0.308881$
(no matter factor; OP-8 dissolved), built from algebraic $\omega_b,\omega_c$, and
the single anchor $67.962$ is both the background $H$ and the CMB-preferred value
(Paper~3).
...
$\sK^{\rm IR} = 0.403302$ exactly and is the description used in
Papers~1--9.
...
$58$ orders of magnitude, returning $\fsc = 0.069521611144$ at
...
$K = c_1 X + c_2 X^2$ with no potential and $c_2 X/c_1 = -0.522735$; in
...
$18.949$ instead of $0.41691$, a factor of $45.5$. Whether one theory
...
$\rho_\phi(1+w_0) = \sK^{\rm IR}/3 = 0.13443$ (in units $\rhoc = 1$),
we find
\begin{equation}
...
\begin{equation}
  \sK \;=\; -3w_0(1+w_0) \;=\; 3\,s_{\rm DE}\,s_m \;=\; 0.403302\,,
  \label{eq:sKdef}
\end{equation}
with $s_{\rm DE} = |w_0| = 0.839950$ and $s_m = 1+w_0 = 0.160050$. It contains
...
for the SSEE action has the distinct value $\alpha_K = 15.591335$
...
\begin{tabular}{lllll}
\toprule
Regime & $\sK$ & $\fsc$ & $H_0^{\rm local}$ [km/s/Mpc] & Tension SH0ES \\
\midrule
...
$H_0^{\rm UV}=73.040$\,km/s/Mpc lies $\sim2.0\sigma$ above this anchor; the
canonical IR prediction $H_0^{\rm IR}=72.86$\,km/s/Mpc lies $\sim1.9\sigma$
above it.
...
$\sK^{\rm full}$ & quadratic + cutoff & 0.41691 & Eq.~\eqref{eq:sKfullform} \\
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    At $\alpha = \varphi^4/3 = 2.284701$ (green dashed), $s_K^{\rm full}=0.41691$
...
\begin{equation}
  \phiG \;\xrightarrow{\alpha = \phiG^4/3}\; \alpha
        ...
        \;\xrightarrow{\sK^{\rm full}}\; H_0^{\rm local}
  \;=\; 73.040\;\text{km/s/Mpc}\,.
\end{equation}
...
$\KAL/\KALeff = 1.333842$ and $(\KAL/\KALeff)^2 = 1.779135$, which is exactly
the factor by which Route~A overshoots ($417.91\,\rhoc$ against
$234.89\,\rhoc$). The discrepancy is thus $33\%$ in the normalisation and
...
the two are not like-dimensioned objects and $1.333842$ must not be read as a
...
The UV-corrected screening rate $\sK^{\rm full} = 0.41691$ is measurable
...
Paper 9 & $H_0^{\rm IR} = 72.86$ (0.17$\sigma$) & UV closes to $73.040$ ($0.00\sigma$; conditional$^{a}$) \\
...
$\sK^{\rm full} = 0.41691$ and the canonical local Hubble constant is
...
\bibitem{SSEE7}
M.~E.~Almeida Vallejo,
\emph{Canonical EFT of SSEE: $\beta_c = -\AURA$, $\sK = 0.403302$},
SSEE Paper~7 (2026).
