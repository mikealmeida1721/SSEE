...
\paragraph{Most direct approach.}
The full $P(X,\phi)$ Lagrangian from the $\alpha$-attractor inflaton is
\begin{equation}
  \mathcal{L} \;=\; -3\alpha\ln\!\!\left(1 - \frac{X}{3\alpha}\right) - V(\phi)\,,
\end{equation}
with $\alpha = \phiG^4/3$.
Matching $\mathcal{L}$ to $K(X) = X/\KAL + X^2/M^4$ in the dark-energy
regime (small $X$, $V(\phi)\approx V_0$) using the SSEE field normalisation
\begin{equation}
  \phiG^2 \;\equiv\; \frac{\partial\phi}{\partial\chi}\bigg|_{\rm transition}^2
\end{equation}
as a boundary condition would fix $M^4 = 45\alpha^2\rhoc$ directly.
This derivation is deferred to a dedicated Letter (Paper~B of the SSEE series),
where it is demonstrated that the factor $(\phiG+3\pi)^2/(10\phiG^4)$ is
absorbed by the normalisation of $\phi$ at the end of the inflationary plateau.
