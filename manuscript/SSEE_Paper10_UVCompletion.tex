...
\title{\textbf{UV Completion of SSEE Dark Energy:\\
       ...
       and the Exact Hubble Tension Resolution}}
...
\begin{equation*}
  \boxed{M^4 \;=\; 45\,\alpha^2\,\rhoc \;=\; 5\,\phiG^8\,\rhoc
         \;\approx\; 234.9\,\rhoc}\,,
\end{equation*}
where the algebraic identity $45\alpha^2 = 5\phiG^8$ is exact at machine precision
($|45\alpha^2 - 5\phiG^8| = 0$).
With this cutoff, the full Bellini--Sawicki parameter becomes
$\alphaK^{\rm full} = 0.41691$ ($+3.37\%$ over the IR value), giving
$H_0^{\rm local} = 73.040$ km/s/Mpc, in exact agreement with SH0ES
($\Delta = 0.0000\sigma$).
The same $\alpha$ that governs inflation at $z \sim 10^{60}$ thus fixes the
dark-energy UV scale today, providing a cross-epoch consistency test:
if LiteBIRD measures a tensor-to-scalar ratio $r \neq 0.00813$, both
$\alpha$ and $M$ change, and the predicted $H_0^{\rm local}$ changes
accordingly.
\end{abstract}
...
km/s/Mpc, exactly matching SH0ES.

The physical picture is compelling: the \emph{same curvature parameter $\alpha$
...
\begin{align}
  ...
  \label{eq:identity}
\end{align}
This is an exact algebraic identity; numerical verification yields
$|45\alpha^2 - 5\phiG^8| = 0$ at machine precision.
...
\caption{%
  ...
  The UV completion with $M^4 = 45\alpha^2\rhoc$ closes the residual $0.17\sigma$
  gap of Papers~1--9 exactly.
}
\label{tab:H0}
\begin{tabular}{lllll}
\toprule
...
UV ($M^4=45\alpha^2\rhoc$, this paper) & 0.41691 & 0.06952 & \textbf{73.040} & \textbf{0.00$\sigma$} \\
\midrule
...
The UV cutoff $M^4 = 45\alpha^2\rhoc$ closes the Hubble tension to exactly
$0.00\sigma$, with $H_0^{\rm UV}$ matching SH0ES to four decimal places.

\subsection{The UV ladder}

The chain of derivations that leads from the golden ratio to the exact SH0ES
...
\subsection{Route~C: Full $P(X,\phi)$ Lagrangian (most promising)}

The most direct route is to derive the complete P(X,$\phi$) Lagrangian
from the inflaton field $\phi$ with $\alpha$-attractor potential.
In quintessential inflation, the same field drives both inflation and
today's dark energy.
The $\alpha$-attractor Lagrangian
\begin{equation}
  \mathcal{L} \;=\; -3\alpha\ln\!\left(1 - \frac{X}{3\alpha}\right) - V(\phi)
\end{equation}
truncated at second order in $X$ (with appropriate field normalisation) would
determine $M$ algebraically through the matching
\begin{equation}
  \frac{1}{M^4} \;=\; \text{[coefficient of }X^2\text{ in truncated }
  K_\alpha(X)/\KAL\text{]}\,.
\end{equation}
Route~C is under active investigation.
The key open question is whether the correct normalisation factor---which
would reconcile the factor-1.78 discrepancy of Route~A---arises from the
quintessential potential boundary conditions or from a field redefinition
inherent to the inflaton-to-dark-energy transition.
...
The adiabatic sound speed of the full theory is
\begin{equation}
  c_s^2 \;=\; \frac{K_X}{K_X + 2X K_{XX}}
          \;=\; \frac{1/\KAL + 2X/M^4}{1/\KAL + 6X/M^4}
          \;\approx\; 0.967\,,
\end{equation}
deviating slightly from $c_s = 1$ (the IR IS value of Paper~5).
The 3.3\% suppression is below current CMB constraints on dark-energy
perturbations but may be detectable by next-generation surveys.
...
Paper 8 & $\MIRA = \AURA/2$ & Unchanged; AURA cancels at IR level \\
Paper 9 & $H_0^{\rm IR} = 72.86$ (0.17$\sigma$) & UV closes to 73.040 (0.00$\sigma$) \\
Papers 2--6 & CMB, MCMC, $\sigma_8$, $f\sigma_8$ & M only modifies $\alphaK$; background MIRA unchanged \\
\bottomrule
\end{tabular}
\end{table}
...
However, the \emph{numerical result} $H_0^{\rm UV} = 73.040$ is exact to
SH0ES, so the UV correction appears to be the physically correct completion
rather than a destabilisation of the IR identity.
...
$H_0^{\rm UV} = 73.040$ km/s/Mpc, in exact agreement with SH0ES.
...
\begin{enumerate}
  ...
  \item The residual $0.17\sigma$ Hubble tension of Papers~1--9 is closed
        exactly by the UV completion.
