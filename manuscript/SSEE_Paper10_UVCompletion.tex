...
$\sK^{\rm full} = 0.41691$ ($+3.37\%$ over the IR value). Run on the SH0ES
...
$H_0^{\rm glob,UV} = 67.96214$ km/s/Mpc, coincident with $3(\phiG+\pi)^2$ to
...
($H_0^{\rm glob,UV}=67.96214$ from the SH0ES datum) is
...
The SH0ES uncertainty propagates to $\pm0.970$ km/s/Mpc on the output, which is
...
IR vs.\ UV global rate returned by the cascade from the measured local
  ...
  uncertainty is $\pm0.970$ km/s/Mpc and dominates both.
}
\label{tab:H0}
\begin{tabular}{lllll}
\toprule
...
\midrule
IR ($M\to\infty$, Papers 1--9) & 0.40330 & 0.06725 & 68.13 & $0.17\sigma$ \\
UV ($M^4=45\alpha^2\rhoc$, this paper) & 0.41691 & 0.06952 & \textbf{67.96214} & $4.3{\times}10^{-6}\sigma$ (cond.$^{a}$) \\
\midrule
...
the SH0ES datum, $H_0^{\rm glob,UV} = 67.96214$ km/s/Mpc --- the headline UV
...
$H_0^{\rm glob,UV}$ & $H_0^{\rm SH0ES}(1-\fsc^{\rm UV})$ & \textbf{67.96214} & Eq.~\eqref{eq:H0UV} \\
$3(\phiG+\pi)^2$ (pure, target) & --- & $67.96214$ & Paper~1 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item The cascade run on the measured local rate should return
        $H_0^{\rm glob} = 67.96214$ km/s/Mpc, reproducing $3(\phiG+\pi)^2$
        [Eq.~\eqref{eq:H0UV}].
\end{enumerate}
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    is the dimensionless target $3(\varphi+\pi)^2=67.96214$ and the shaded band
    is the propagated SH0ES uncertainty $\pm0.970$ km/s/Mpc.
...
measurement of the cutoff: the propagated SH0ES uncertainty is $\pm0.970$
