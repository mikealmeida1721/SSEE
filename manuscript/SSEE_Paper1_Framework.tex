...
$k_{\mathrm{fs}}$ (Dodelson-Widrow from $m_\phi$) & $0.762\,h\,\mathrm{Mpc}^{-1}$ & DESI\,Y3 / Euclid $P(k)$, 2026--2028 & Prediction \\
\midrule
...
\begin{description}
  ...
  the canonical $\omega_m$-direct CMB fit gives $\Delta\mathrm{BIC}=-23.9$ at the
  global anchor $H_0^{\rm alg}=67.962$~km\,s$^{-1}$\,Mpc$^{-1}$ ($k=2$ vs $k=6$;
  $\chi^2_{\rm SSEE}=1005.5$). The earlier legacy Cobaya scan gave
  $\Delta\mathrm{BIC}=-32.2$ at the superseded MIRA anchor $H_0=67.037$.
...
% Fase B: titular plik_lite TTTEEE omega_m-directo (Om_m=0.30889, N=613) -> DeltaBIC=-23.9 (log p3_cmb_reframe_omega_m). Diagonal recomputado omega_m-directo -> -34.9 (log paper3_cmb_reframe, N=5914). Fila Cobaya -32.2 sigue legacy MIRA (superseded).
\texttt{plik\_lite} TTTEEE & 613 & $\omega_m$-direct & 2 & 6 & $\mathbf{-23.9}$ &
...
Planck PR4 diagonal & 5914 & $\omega_m$-direct & 2 & 6 & $-34.9$ &
  CMB spectra, diagonal cov.\ (cross-check; $\Omega_m{=}0.30889$, $H_0{=}67.962$) \\[3pt]
\texttt{plik\_lite} TTTEEE & 613 & MIRA bg (legacy) & 2 & 6 & $-32.2$ &
  CMB parsimony; \emph{superseded by the $\omega_m$-direct row above} \\
\bottomrule
\end{tabularx}
\end{table}
...
$1003.8$). The diagonal-covariance cross-check, also $\omega_m$-direct, gives
$\Delta\mathrm{BIC}=-34.9$ (the larger magnitude reflects the inflated effective
$N$ under the independence approximation). The legacy MIRA-background values
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item the free-streaming scale $k_{\rm fs}$ predicted from $m_\phi=41.02$~eV
...
\textbf{Observational} &
  Planck~PR4 $\Delta\mathrm{BIC}=-23.9$ ($\omega_m$-direct canonical, $k=2$); $r_d^{\rm SSEE}=147.17$~Mpc ($0.32\sigma$) &
...
The $\Delta\mathrm{BIC}=-5.55$ (DESI dynamic) and $-23.9$ (CMB $\omega_m$-direct)
are not directly comparable (different $N$, backgrounds, and $\Lambda$CDM $k$ counts).
The canonical CMB value is the $\omega_m$-direct $\Delta\mathrm{BIC}=-23.9$
(\texttt{plik\_lite}, $k=2$, $N=613$), with the $\omega_m$-direct diagonal
cross-check at $-34.9$ ($N=5914$). The legacy MIRA-background values
($-32.2$ Cobaya, aggressive $k{=}1$ $-41.0$) used the superseded
$\Omega_m=0.31993$ mapping and are retired (see Paper~3).
...
$\Sigma_{\rm Sov}$ & $3(\varphi+\pi)$ & $\varphi+\pi+K_v$ — non-self-add & Maximal / dominant & Maximal dim. invariant $\to w_0,w_a$ & — \\
...
In Paper~3, the $\Delta\mathrm{BIC}=-23.9$ ($\omega_m$-direct, \texttt{plik\_lite}
likelihood, canonical $k=2$; the legacy MIRA-background $-32.2$ is superseded)
