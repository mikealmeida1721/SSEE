...
\begin{itemize}\itemsep1pt
  \item \textbf{Postulate~M (matter factor) --- retired.} Earlier versions
        ...
        $\Omega_{m,\rm dyn}=0.160050$ (DESI) and $\omega_m$ (CMB) are two
...
distinct matter density values across the suite: $\Omega_{m,\rm dyn}=0.160050$
...
\caption{The Two-$\Omega_m$ criterion: which matter density enters each
  ...
  ($\Omega_{m,\rm dyn}=0.160050$, $\Omega_{m,\rm CMB}=\omega_m/h^2=0.308881$)
...
DESI DR2 BAO chain, low-$z$ $E(z)$                                  & $0.160050$ & local geometry, $k\gg k_{\rm fs}$. \\
$r_d$ inside the BAO chain                                          & $0.160050$ & enters $E(z)$-consistent late-time mapping. \\
Galaxy-cluster dynamical mass                                       & $0.160050$ & non-linear scales, $\phi$-DM unbound. \\
\midrule
...
$\Omega_{m,\rm dyn}=(\pi-\varphi)/[2(\varphi+\pi)]=1{+}w_0$         & $0.160050$ & dyn-sector identity (independent of CMB). \\
Tensor-to-scalar $r=\varphi^{-10}$                                  & --      & primordial, no $\Omega_m$ dependence. \\
\midrule
\multicolumn{3}{l}{\textit{Paper 5 — Israel--Stewart growth:}} \\
SSEE Friedmann background $E(z)$                                    & $0.160050$ & dynamical sector, $1+w_0=\Omega_{m,\rm dyn}$. \\
Poisson source in linear growth ODE                                 & $0.308881$ & full matter on $k<k_{\rm fs}$ scales. \\
$\Omega_{m,\rm eff}=\Omega_{m,\rm dyn}+\Omega_{\rm DE}r$ (clustering)& $0.160050$+ & DE clustering deviation, $r\ll1$. \\
\midrule
\multicolumn{3}{l}{\textit{Paper 6 — two-sector dark matter:}} \\
$\Omega_{\rm CDM}$ (always active)                                  & $0.160050$ & dynamical CDM, all $k$. \\
...
EFT slope $\lambda=\sqrt{3\,\Omega_{m,\rm dyn}}\approx 0.693$        & $0.160050$ & background scalar dynamics. \\
$\alpha_K(0)=3\Omega_{\rm DE}(1+w_0)$                                & $0.160050$ & $1+w_0=\Omega_{m,\rm dyn}$ identity. \\
...
Small-scale photon deflection, CDM only                              & $0.160050$ & only CDM clusters at $k\gg k_{\rm fs}$. \\
...
Late-time local CPL background $E(z)$                                & $0.160050$ & local dynamical sector. \\
...
$\Omega_{m,\rm dyn}=0.160050$; any observable that integrates over scales
...
$E(z)$ geometry with $\Omega_{m,\rm dyn}=0.160050$ and uses an $r_d$ value
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item $\chi^2_r(H(z)) > 2.5$ with full systematic covariance and $\Omega_{m,\rm dyn}=0.160050$.
\end{itemize}
\textbf{Observational sector} is falsified if:
\begin{itemize}[nosep,leftmargin=*]
  \item Euclid/KiDS/DES require $\Omega_m < 0.25$ at $z < 1$
        (excludes the dynamical-sector value $\Omega_{m,\rm dyn}=0.160050$); or
  \item the free-streaming scale $k_{\rm fs}$ predicted from $m_\phi=40.70$~eV
        ...
        $\Omega_{m,\rm dyn}=0.160050$ from $\Omega_{m,\rm CMB}=0.308881$.
\end{itemize}
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item The $\Omega_m$ difference between sectors: $\Omega_{m,\rm dyn}=0.160050$ and
