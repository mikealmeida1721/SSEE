...
space is performed (see the look-elsewhere analysis). Table~\ref{tab:register}
...
($k_{\mathrm{fs}}$, $r$) are genuine predictions.
...
$f\sigma_8(z{=}0.5)$ & mean $0.76\sigma$ vs 6 RSD surveys & BOSS/eBOSS/6dFGS & Retrodiction \\ % TODO Fase B: recompute f sigma8 con Om_m=0.30889 / m_phi=42.47
\midrule
\multicolumn{4}{l}{\textit{Paper 6 — $\varphi$-Dark Matter, two-sector}} \\
$m_\phi = \Sigma m_\nu^{\rm active}\,(\mathrm{PYROS}\cdot\mathrm{VITA}\cdot\mathrm{MIKA})$ & $42.47\,\mathrm{eV}$ & Matter $P(k)$ via $k_{\mathrm{fs}}$ (no SM portal) & Prediction \\
$k_{\mathrm{fs}}$ (Dodelson-Widrow from $m_\phi$) & $0.659\,h\,\mathrm{Mpc}^{-1}$ & DESI\,Y3 / Euclid $P(k)$, 2026--2028 & Prediction \\ % TODO Fase B: recompute k_fs (DW) con m_phi=42.47
\midrule
...
takes as given versus what it derives in terms of four register-level postulates
(D, S, M, I). That four-postulate accounting --- not the count of physical
statements above --- is the authoritative measure of the model's free choices;
Principle~1 above supplies the de~Sitter endpoint invoked there by Postulate~S.
...
\begin{itemize}\itemsep1pt
  ...
  \item The $\varphi$-DM mass $m_\phi=42.47$\,eV in Paper~6 follows from the
        free-scalar mass term $m_\phi=\Sigma m_\nu^{\rm active}\,(\mathrm{PYROS}\cdot\mathrm{VITA}\cdot\mathrm{MIKA})$;
        the residual open question (OP-9) is the UV origin of the pure
        $(\varphi,\pi)$ multiplier $\mathrm{PYROS}\cdot\mathrm{VITA}\cdot\mathrm{MIKA}=615.33$,
  ...
  \item The non-minimal coupling $\xi$ of $\varphi$-DM is currently a free
        parameter (OP-11).
\end{itemize}
The framework therefore carries $\sim$2 effective dimensionless ans\"atze
beyond the strictly algebraic background (down from three after OP-8 closed),
against six fitted parameters in
$\Lambda$CDM $(\Omega_m,\Omega_b,n_s,A_s,\tau,H_0)$. Resolution of
OP-9/11 would restore the literal zero-parameter status; see
...
$\phi$-DM mass $m_\phi=42.47$~eV via the Dodelson--Widrow relation. The
...
$\Delta\mathrm{BIC}=-23.9$ in favour of SSEE ($k=2$ vs $k=6$). The earlier
...
\begin{description}
  ...
  % TODO Fase B: re-fit CMB con H global=67.96 y Om_m=0.30889 (chi2_r, Delta BIC dependientes)
...
% TODO Fase B: re-correr pipeline P3 completo con Om_m=0.30889 (omega_m-directo); estas filas son del background MIRA legacy (0.31993)
\texttt{plik\_lite} TTTEEE & 6469 & $\omega_m$-direct & 2 & 6 & $\mathbf{-23.9}$ &
  CMB parsimony, $\omega_m$-direct canonical ($\Omega_m{=}0.30889$, Paper~3 Fase~B) \\[3pt]
Planck PR4 diagonal & 5914 & MIRA bg (legacy) & 2 & 6 & $-28.0$ &
  CMB spectra, diagonal cov.; \emph{pending Fase~B recompute} \\[3pt]
\texttt{plik\_lite} TTTEEE & 6469 & MIRA bg (legacy) & 2 & 6 & $-32.2$ &
  CMB parsimony (superseded MIRA background); \emph{pending Fase~B} \\
\bottomrule
\end{tabularx}
\end{table}
...
The canonical CMB result is the $\omega_m$-direct row, $\Delta\mathrm{BIC}=-23.9$
(\texttt{plik\_lite}, $k=2$, $\Omega_m=0.30889$, no matter factor), which favours
SSEE while matching $\Lambda$CDM in fit quality ($\chi^2_{\rm SSEE}=1005.5$ vs
$1003.8$). The legacy MIRA-background rows ($-32.2$, $-28.0$, $-41.0$) used the
superseded $\Omega_m=0.31993$ mapping and are pending a full Fase~B recompute.
...
$H_0^{\rm MCMC}$ & CMB full posterior (Paper~3) & $67.96$ (anchor; PENDIENTE Fase B) & Planck PR4 & M \\ % TODO Fase B: re-fit CMB posterior con Om_m=0.30889, H global=67.96
...
\small $^\S$ Canonical CMB-sector matter density ($\omega_m$-direct reframe, 2026):
...
$\chi^2_{\Lambda\rm CDM}=1003.8$, $\Delta\mathrm{BIC}=-23.9$. Earlier register
...
construction.\\[2pt]
\small $^\dagger$ $H_0=3(\varphi+\pi)^2\approx67.962$\,km\,s$^{-1}$\,Mpc$^{-1}$
