...
AURA      & $2\varphi$ & $3.236068$ & Resonance scale & A \\
\midrule
...
MIRA      & AURA$/2 = \varphi$ & $1.618034$ & CMB--dyn.\ bridge & A$^*$ \\
...
\end{tabular}
\\[4pt]
\small $^*$ MIRA serves a dual role: (i) as a \textit{derived constant}
(MIRA $= \varphi$, exact algebraic identity), and (ii) as a
\textit{sector-mapping operator} projecting $\Omega_{m,\rm dyn}=0.160$
onto $\Omega_{m,\rm CMB}=0.3199$ via $\Omega_{m,\rm CMB} = 2 \times
\mathrm{MIRA} \times \Omega_{m,\rm dyn}$.
The dual role is physically meaningful: the same geometric ratio
$\varphi$ governs both the internal structure of the hierarchy and the
observational frequency of the CMB sector.
