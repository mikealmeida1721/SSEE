...
$f\sigma_8(z{=}0.5)$ & mean $0.74\sigma$ vs 6 RSD surveys (Paper~5 corrected) & BOSS/eBOSS/6dFGS & Retrodiction \\
\midrule
\multicolumn{4}{l}{\textit{Paper 6 — $\varphi$-Dark Matter, two-sector}} \\
$m_\phi = \Sigma m_\nu^{\rm active} \times H_0^{\mathrm{alg}}$ & $5.60\,\mathrm{eV}$ & Matter $P(k)$ via $k_{\mathrm{fs}}$ (no SM portal) & Prediction (ansatz) \\
...
\begin{itemize}\itemsep1pt
  \item The MIRA factor $(3\varphi+\pi)/4$ has an algebraic value but no
        ...
        $\varphi$-MDE).
  \item The $\varphi$-DM mass $m_\phi=5.60$\,eV in Paper~6 is a numerological
        ansatz (OP-9), depending on $\Sigma m_\nu$, itself classified as
        Type~P (phenomenological) in Paper~4 due to the integer offset in
        $\mathcal{R}=4\,\mathrm{KAL}-22$ (OP-14).
...
OP-8/9/11/14 would restore the literal zero-parameter status; see
...
scale, $k_{\rm fs}=0.493\,h\,\mathrm{Mpc}^{-1}$, derived in Paper~6 from the
$\phi$-DM mass ansatz $m_\phi=5.60$~eV via the Dodelson--Widrow relation. The
...
Using $\sum m_\nu^{\rm active}=0.0824$\,eV (the SSEE-derived value, Paper~4) the
cosmological neutrino density is $\rho_\nu\approx27.7$\,eV\,cm$^{-3}$.
...
$\sum m_\nu^{\rm active}$ & $\mathcal{R}\,\Omega_b h^2\cdot94.07/(\tau_\Pi H_0)$ (Paper~4) & $0.0824$ eV & Planck+BBN & P \\
\bottomrule
\end{tabularx}
\\[4pt]
