...
The IS correction for $\Omega_c h^2$ uses this value as input.\\[2pt]
...
\subsection*{Primary register — Level 1 operations on $\{\varphi,\pi\}$}

All seven structural constants of \SSEE\ follow from a \emph{single layer} of
arithmetic operations ($+$, $\times$, integer scale) applied to the two axioms:
...
\midrule
$\Omega$ & DNAV / Stability Metric  & $\varphi+\pi$ & 4.7596 \\
$\beta$  & BIAL / Base Coupling     & $(\varphi+\pi)/2$ & 2.3798 \\
$\mathrm{KAL}_0$ & Structural Viscosity & $\beta+\pi$ & 5.5214 \\
$P_{sc}$ & PYROS / Dynamical Scalar & $\Omega+\varphi$ & 6.3776 \\
$K_v$    & KRYSTOS / Structural Constraint & $\varphi+\pi+\Omega$ & 9.5192 \\
$T_r$    & TRIAL / Saturation Horizon & $3(\varphi+\beta)$ & 11.9935 \\
$M_v$    & MIKAEL\_V / Maximal Invariant & $\varphi+\pi+K_v$ & 14.2788 \\
\bottomrule
\end{tabular}
\end{center}

\noindent No constants were introduced \emph{ad hoc}: all seven emerge from
systematic enumeration of Level~1 combinations of $\varphi$ and $\pi$.
The \emph{closure criterion} is exhaustiveness at Level~1: the seven constants
are the complete set of non-redundant, monotonically ordered elements generated
by $\{\varphi,\pi\}$ under a single layer of arithmetic (addition, halving, integer
scaling $\leq\!\times\!3$).  $M_v$ is the maximal element under these constraints.
Level-2 operations (products, integer powers) are not part of the primary register
and would introduce additional free structure not derivable from the axioms alone.
...
This expression is not a tuned fit to the Planck value.
It follows from the $\alpha$-attractor slow-roll formula
$n_s = 1 - 2/N$ combined with the \SSEE\ e-fold count
$N = 2\varphi^7 \approx 58.07$ (App.~A §A.5):
\[
  n_s = 1 - \frac{2}{2\varphi^7} = 1 - \varphi^{-7} \approx 0.96556.
\]
The factor $2\varphi^7$ derives from $\alpha = \varphi^4/3$ and the Starobinsky
attractor relation $N = \sqrt{3\alpha}\cdot(\ldots)$; the full derivation is in
App.~A §A.5. The Planck~2018 value $n_s = 0.9649\pm0.0042$ (0.2$\sigma$
separation) was compared \emph{after} the formula was derived.
