...
\texttt{plik\_lite} TTTEEE & 6413 & MIRA-corr. & 1 & 6 & $-40.3$ &
  CMB parsimony: algebraic prediction vs $\Lambda$CDM free-fit \\
\bottomrule
\end{tabularx}
\end{table}
...
The $-40.3$ and $+31.1$ both use CMB data but differ in likelihood type and $k$.
...
\textbf{Observational} &
  Planck~PR4 $\Delta\mathrm{BIC}=-40.3$; $r_d^{\rm SSEE+MIRA}=147.16$~Mpc ($0.25\sigma$) &
...
The $\Delta\mathrm{BIC}=-5.55$ and $-40.3$ are not directly comparable
(different $N$, backgrounds, and $\Lambda$CDM $k$ counts).
The $\Delta\mathrm{BIC}=+31.1$ and $-40.3$ bracket the CMB result's sensitivity
to the assumed $k_{\rm SSEE}$ (see Paper~3 Table~7).
