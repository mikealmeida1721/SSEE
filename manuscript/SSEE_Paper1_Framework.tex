...
space is performed. A look-elsewhere analysis over the \emph{complete} closed
dictionary of named $(\varphi,\pi)$ constants generates $317$ distinct ratios in
$(0,5]$, and each background equation-of-state value is matched by a \emph{unique}
one at machine precision---a $1$-in-$317$ specificity that persists when the full
dictionary, not a hand-picked subset, is scanned (the ratio count is robust to
value-duplicate constants; reproducible via \texttt{look\_elsewhere\_full.py},
Zenodo Constant Dictionary v1.1). Table~\ref{tab:register}
