...
signal a ghost or gradient instability in the SSEE action, whose geometric
bulk-viscosity term preserves the null energy condition at the background level
(see App.~A, EFT embedding).  Whether the phantom crossing survives in the
full causal Israel--Stewart perturbation sector is an open question deferred to Paper~4.
...
\small
\begin{tabular}{p{2.8cm}rp{1.9cm}rrlp{2.2cm}}
\toprule
Dataset & $N$ & Background & $k_{\rm SSEE}$ & $k_{\Lambda}$ & $\Delta$BIC & Hypothesis \\
\midrule
DESI~DR2 + Planck prior & 16 & Std.\ Friedmann & 2 & 3 & $+206$ &
  Full model; background incompatibility with LCDM-calibrated $\Omega_m$ \\[4pt]
DESI~DR2 + Planck prior & 16 & Std.\ Friedmann & 1 & 3 & $-5.55$ &
  Dynamic sector: $w_0,w_a$ fixed, $H_0$ free \\[4pt]
Planck~PR4 diagonal & 5914 & MIRA-corrected & 2 & 6 & $+31.1$ &
  CMB spectra, conservative $k$ upper bound \\[4pt]
\texttt{plik\_lite} TTTEEE & 6413 & MIRA-corrected & 1 & 6 & $-40.3$ &
...
\paragraph{Global falsification rule (theory-level).}
\begin{tcolorbox}[colback=red!8,colframe=red!70!black,title=Global Falsification Criterion]
...
\begin{tcolorbox}[colback=red!4,colframe=red!60!black,title=SSEE Falsification Criteria]
...
\textbf{Algebraic} &
  ...
  Any tension arising from unit-convention mismatch \\[6pt]
\textbf{Dynamic} &
  DESI~DR2 $\Delta$BIC$=-5.55$; $w_0,w_a$ within $0.05\sigma$ &
  DESI~DR5+ excludes $(-0.840,-0.670)$; or $\chi^2_r(H(z))>2.5$ with full cov. &
  $\Delta$BIC$=+206$ from LCDM-calibrated $\Omega_m$ prior \\[6pt]
\textbf{Observational} &
  Planck~PR4 $\Delta$BIC$=-40.3$; $r_d^{\rm SSEE+MIRA}=147.16$~Mpc ($0.25\sigma$) &
  Euclid/KiDS $\Omega_m<0.25$ at $z<1$; or MIRA ratio $\neq1.999$ at $>3\sigma$ &
  $\chi^2_r=1.062 > 1$ vs $\Lambda$CDM's $1.043$ (non-equivalent background) \\
\bottomrule
\end{tabular}
\end{table}
...
$\sum m_\nu$ & (Acoustic Residue, Paper~4 in prep.) & $0.084$ eV & Planck+BBN & A \\
\bottomrule
\end{tabular}
\\[4pt]
...
MIRA & Mapped Inference Resonance Amplitude (AURA/2 $=(3\varphi+\pi)/4\approx1.9989$) & Paper 3 §2.4 \\
