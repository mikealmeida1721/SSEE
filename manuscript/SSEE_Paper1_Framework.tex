...
\begin{itemize}
  ...
  \item $\boldsymbol{P}$ ($\Omega+\varphi\approx6.3777$): Dynamical Evolution Scalar.
  \item $\boldsymbol{K_v}$ ($\varphi+\pi+\Omega\approx9.5193$): Structural Constraint Invariant.
  \item $\boldsymbol{T_r}$ ($3(\varphi+\beta)\approx11.9935$): 3D Saturation Horizon.
  \item $\boldsymbol{M_v}$ ($\varphi+\pi+K_v\approx14.2789$): Maximal Dimensional Invariant.
\end{itemize}
...
and $\Omega_{m,\rm CMB}=0.308881$. These are not free parameters and, since the
...
\caption{The Two-$\Omega_m$ criterion: which matter density enters each
  ...
  ($\Omega_{m,\rm dyn}=0.160$, $\Omega_{m,\rm CMB}=\omega_m/h^2=0.308881$)
...
CMB peak positions $\ell_n$                                         & $0.308881$ & $D_A(z_*)$ integrates $\phi$-DM, non-rel.\ at $z_*$. \\
$r_d$ from full CAMB Boltzmann                                      & $0.308881$ & $\phi$-DM non-rel.\ at recomb. \\
\midrule
\multicolumn{3}{l}{\textit{Paper 4 — algebraic identities:}} \\
$\Omega_{m,\rm CMB}=\omega_m/h^2$, $\omega_m=\omega_b{+}\omega_c{+}\omega_\nu$ & $0.308881$ & CMB matter density, algebraic (no factor). \\
...
Poisson source in linear growth ODE                                 & $0.308881$ & full matter on $k<k_{\rm fs}$ scales. \\
...
$\Omega_{m,\rm growth}=\Omega_{\rm CDM}+\Omega_{\phi\mathrm{DM}}$    & $0.308881$ & sum on clustering scales $=\Omega_{m,\rm CMB}$. \\
\midrule
...
Growth ratio $G_{2s}=D_1^{\rm SSEE}(\Omega_m{=}0.308881)/D_1^{\Lambda\rm CDM}$ & $0.308881$ & two-sector growth comparison. \\
\midrule
...
Cluster/LSS lensing total, $\Omega_{m,\rm eff}=\Omega_{\rm CDM}+\Omega_{\phi\mathrm{DM}}$ & $0.308881$ & both sectors on $k<k_{\rm fs}$. \\
\midrule
...
$\Omega_{m,\rm CMB}=0.308881$. The two values are not tied by a factor; each is
...
\begin{equation}
  ...
  + \frac{\Sigma m_\nu}{93.14} = 0.14267 ,
  \qquad
  \Omega_{m,\rm CMB} = \frac{\omega_m}{h^2} = 0.308881 .
\end{equation}
A direct Planck \texttt{plik\_lite} TTTEEE evaluation at this configuration
($H_0=67.96$, $\Omega_{m,\rm CMB}=0.308881$, $\omega_b$ algebraic) gives
...
\begin{equation}
  \boxed{w_0 = -\frac{T_r}{M_v} = -\frac{3\varphi+\pi}{2(\varphi+\pi)} = -0.8399},
  \qquad
  \boxed{w_a = -\frac{P}{I_g} = -\frac{2\varphi+\pi}{2(\varphi+\pi)} = -0.6700}.
...
denominators reduce to the same number, $2(\varphi+\pi)=9.5193$, but they are
...
\begin{description}
  ...
  Governed by $\Omega_{m,\rm dyn}=0.160050$, $w_0=-0.839950$, $w_a=-0.669975$.
...
\texttt{plik\_lite} TTTEEE & 613 & $\omega_m$-direct & 2 & 6 & $-24.0$ &
  Compressed-nuisance point est.\ (cross-check; $\Omega_m{=}0.308881$) \\[3pt]
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item the free-streaming scale $k_{\rm fs}$ predicted from $m_\phi=40.70$~eV
        ...
        $\Omega_{m,\rm dyn}=0.160$ from $\Omega_{m,\rm CMB}=0.308881$.
\end{itemize}
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item The $\Omega_m$ difference between sectors: $\Omega_{m,\rm dyn}=0.160$ and
        $\Omega_{m,\rm CMB}=0.308881$ are two \emph{independent} algebraic predictions
...
$K_v$     & $\varphi+\pi+\Omega$ & $9.519254$ & Struct.\ constraint (entity KRYSTOS$_V$) & A \\
$I_g$     & $\pi+P_{sc}$ & $9.519254$ & $w_a$ denominator (entity IGNIS; same value as $K_v$, distinct entity) & A \\
...
giving $\Omega_{m,\rm CMB}=\omega_m/h^2=0.308881$ independently of
$\Omega_{m,\rm dyn}=0.160050$ (OP-8 closed).
...
\midrule
$\Omega$ & OMEGA / Stability Metric & $\varphi+\pi$ & 4.7596 & L1\\
$\beta$  & BIAL / Base Coupling     & $(\varphi+\pi)/2$ & 2.3798 & L1\\
$K_v$    & KRYSTOS$_V$ / Structural Constraint & $\varphi+\pi+\Omega$ & 9.5193 & L1\\
$M_v$    & ATLAS / Maximal Invariant & $\varphi+\pi+K_v$ & 14.2789 & L1\\
\midrule
$\mathrm{KAL}_0$ & Structural Viscosity & $\beta+\pi$ & 5.5214 & L2\\
$P_{sc}$ & PYROS / Dynamical Scalar & $\Omega+\varphi$ & 6.3777 & L2\\
...
$\Omega_{m,\rm CMB}=\omega_m/h^2=0.308881$ independently of $\Omega_{m,\rm dyn}$
