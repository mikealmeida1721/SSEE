...
% Posterior H0 del MCMC reframe (prior H_alg=67.962, DESI baja a 67.16).
% Fuente: results/logs/mcmc_paper2_reframe.log (100w×25k, seed 42, N_eff=80262).
\newcommand{\HZeroPosterior}{67.16^{+0.44}_{-0.44}}
...
direct CMB fit of Paper~3 (canonical $\Delta\mathrm{BIC}=-33.0$ full \texttt{plik}
...
($65.8^{+1.2}_{-1.1}$). The simultaneous consistency across both ends
of the prior spectrum confirms that the algebraic anchor bridges the Planck CMB and
...
\begin{figure}[H]
  ...
  ($\HZeroPosterior$) sits $1.15\sigma$ below the CMB anchor (DESI BAO pulls it
  down) and $0.29\sigma$ from Planck.}
...
($H_0^{\SSEE}=67.16$ vs $H_0^{\LCDM}=67.99$\,km\,s$^{-1}$\,Mpc$^{-1}$),
but this compensation is insufficient to absorb the full background
discrepancy. The residual tension is $1.4\sigma$ in $H_0$ but
...
$1.15\sigma$ from $H_0^{\rm alg}$ and at $0.29\sigma$ from the
...
The $0.80$\,km\,s$^{-1}$\,Mpc$^{-1}$ downward shift from the anchor
($1.2\%$) is an explicit consequence of the background-mismatch
...
canonical $\Delta\mathrm{BIC}=-33.0$ (full \texttt{plik} MCMC, $\omega_m$-direct,
...
likelihood ($\Delta\mathrm{BIC}=-33.0$, full \texttt{plik} MCMC, $\omega_m$-direct).
...
\begin{enumerate}
  ...
  \item Under the $\omega_m$-direct reframe the sound horizon falls out
    ...
    confirms it with the full CMB likelihood (canonical $\Delta\mathrm{BIC}=-33.0$,
