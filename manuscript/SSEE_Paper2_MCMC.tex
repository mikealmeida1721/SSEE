...
% Posterior H0 del MCMC reframe (prior H_alg=67.962, DESI DR2 baja a 66.41).
% DR1-era 67.16 SUPERSEDED (V-L4-DESI): los datos eran DR1 mal etiquetados.
% Fuente: results/logs/mcmc_paper2_reframe.log (100w×25k, seed 42, N_eff=80262).
\newcommand{\HZeroPosterior}{66.41^{+0.39}_{-0.39}}
...
lies within $0.16\sigma$ of the official DESI~DR2 $w_0w_a$CDM posterior
...
on the two-sector $\phi$-DM reconciliation of the background} developed in
Papers~3 and~6 ($\omega_m$-direct; no matter factor, OP-8 dissolved).
...
should be interpreted as conditional on the $\omega_m$-direct two-sector $\phi$-DM
reconciliation of the background developed in Papers~3 and~6.}
...
The companion Papers~3 and~6 resolve the background
tension via the $\omega_m$-direct two-sector $\phi$-DM construction (no matter
factor; OP-8 dissolved). A unified domain delineation is given
...
The drag-epoch scale built from the dynamical sector alone
($\Omega_{m,\mathrm{dyn}}=0.160$) is $\rdSSEE \approx 175.6$\,Mpc, larger than
the Planck-inferred $r_d=147.09\pm0.26$\,Mpc \citep{Planck2018}: this is the
BAO-sector footprint of the low dynamical matter density.
Under the $\omega_m$-direct reframe the physical sound horizon is \emph{not}
obtained by any $|w_0|$ rescaling but falls out directly from the CMB-sector
matter density. The recombination matter budget is the standard physical
...
$k_{\rm fs}=0.762\,h\,\mathrm{Mpc}^{-1}$ (Paper~6), not a factor pair.
...
The \SSEE\ $H_0=\HZeroPosterior$\,km\,s$^{-1}$\,Mpc$^{-1}$ sits
between the $\omega_m$-direct CMB anchor
($H_0^{\rm prior,SSEE}=67.962\pm0.54$, Eq.~\ref{eq:H0_prior_alg}),
the published $\Lambda$CDM-calibrated Planck~2018
value ($67.36\pm0.54$), and the BAO-only run
($65.8^{+1.2}_{-1.1}$). With the DESI~DR2 uncertainties ($\sim$40\% tighter than DR1) the late-time
BAO geometry dominates the posterior and pulls it $2.9\sigma$ below the
algebraic anchor: the raw dynamic-sector background is strained by the DR2
precision (the same two-$\Omega_m$ structural tension documented in
\S\ref{sec:model_comparison}, amplified). We report this honestly rather than
tuning the prior; the anchor-bridge reading that held at DR1 precision no
longer holds unqualified at DR2 precision.
...
\begin{figure}[H]
  ...
  ($H_0=67.962$, $\Omega_b h^2=0.02218$). The $H_0$ posterior
  ($\HZeroPosterior$) sits $2.9\sigma$ below the CMB anchor (the tighter
  DESI~DR2 BAO errors pull it down strongly) and $1.4\sigma$ from Planck.}
...
(DESI+CMB+Pantheon+, $w_0=-0.838\pm0.055$, $w_a=-0.62^{+0.22}_{-0.19}$;
arXiv:2503.14738, eq.~26; $\rho=-0.85$ estimated from the public contours) is:
\begin{equation}
  \chi^2_{2\rm D}
  = \Delta\mathbf{w}^\top\mathbf{C}^{-1}\Delta\mathbf{w} = 0.27
  \;\Rightarrow\; 0.16\sigma.
\end{equation}
Against the other official DR2 combinations the distance is $1.23\sigma$
(DESY5), $1.50\sigma$ (Union3) and $1.52\sigma$ (no-SN DESI+CMB): the
algebraic point is consistent with all four, with the tension range
$0.2$--$1.5\sigma$ set by the supernova compilation, and $w_0$ essentially
exact against Pantheon+ ($\Delta w_0=-0.04\sigma$). The result is robust to
the assumed correlation ($\rho\in[-0.92,-0.70]$ shifts Pantheon+ by
$<0.2\sigma$).
...
with free dark-energy parameters).  The $0.16\sigma$ quoted above is the Mahalanobis distance of the \SSEE\
...
$0.16\sigma$ Pantheon+; $1.2$--$1.5\sigma$ for the other compilations) and
...
\begin{figure}[H]
  ...
  $0.16\sigma$ of the DESI~DR2+CMB+Pantheon+ contour.}
  \label{fig:w0wa}
\end{figure}
...
CMB+BAO+SN likelihood, where \SSEE\ sits at $0.16\sigma$ of the DESI~DR2
...
\begin{equation}
  ...
  \label{eq:rd_ratio}
\end{equation}
The BAO data partially compensate via an $H_0$ downshift
($H_0^{\SSEE}=66.41$ vs $H_0^{\LCDM}=67.99$\,km\,s$^{-1}$\,Mpc$^{-1}$;
the $\Lambda$CDM baseline re-run against the corrected DR2 vector is
pending), but this compensation is insufficient to absorb the full
background discrepancy. The mismatch is in the
...
matter density is not $\Omega_{m,\rm dyn}$ but the standard physical density
$\Omega_{m,\mathrm{CMB}} = \omega_m/h^2 = 0.30889$, where
$\omega_m=\omega_b+\omega_c+\omega_\nu=0.14267$ is built piece-by-piece from
SSEE algebra ($\omega_c=\mathrm{KAL_0}\,\omega_b\,n_s$, forward; Paper~3).
The two matter sectors then split as
$\Omega_m^{\rm CMB} = \Omega_{\rm CDM} + \Omega_{\phi\text{-DM}} = 0.16005 + 0.14884 = 0.30889$,
where the $\phi$-dark matter component
($\Omega_{\phi\text{-DM}} = \Omega_{m,\mathrm{CMB}}-\Omega_{m,\rm dyn}$, a
difference with \emph{no} matter factor; algebraic mass $m_\phi = 41.02$\,eV)
contributes at scales below its free-streaming cutoff
$k_{\rm fs}=0.762\,h$\,Mpc$^{-1}$.
With this identification, the tension reduces from $21.3\sigma$ to $0.88\sigma$
($\Omega_{m,\mathrm{CMB}}=0.30889$ vs Planck $0.3153\pm0.0073$).
The $21.3\sigma$ figure therefore quantifies the cost of applying $\Lambda$CDM
priors to a two-sector model. OP-8 is now dissolved: there is no matter
factor to derive --- the residue is only that $\omega_b$ and the forward
identity for $\omega_c$ hold.
...
$H_0^{\rm MCMC}=\HZeroPosterior$\,km\,s$^{-1}$\,Mpc$^{-1}$,
is the late-time BAO posterior under this anchor as prior; it lies at
$2.9\sigma$ from $H_0^{\rm alg}$ (prior width) and at $1.4\sigma$ from the
$\Lambda$CDM-calibrated Planck value. The two measure different quantities:
$H_0^{\rm alg}$ characterises the early-Universe geometry, while
$H_0^{\rm MCMC}$ is the late-time posterior after the DESI data pull
the value downward to compensate for the enlarged $r_d$.
The $1.55$\,km\,s$^{-1}$\,Mpc$^{-1}$ downward shift from the anchor
($2.3\%$) is an explicit consequence of the background-mismatch
compensation, amplified by the tighter DESI~DR2 uncertainties; it is a
quantified strain of the raw dynamic-sector background, not a silent
inconsistency (see \S\ref{sec:model_comparison}).
...
\begin{figure}[H]
  ...
  pure-geometric sound horizon ($175.6$\,Mpc) exceeds \LCDM\ by $1.190\times$.
  Figure~\ref{fig:rd_dual} shows how the $\omega_m$-direct CMB sector closes this gap.}
  \label{fig:tension}
\end{figure}
...
\begin{figure}[H]
  ...
  physical CMB drag scale, set by the full algebraic
  $\omega_m=\omega_b+\omega_c+\omega_\nu=0.14267$
  ($\Omega_{m,\mathrm{CMB}}=\omega_m/h^2=0.30889$, $\omega_m$-direct, no
  matter factor), is $r_d = 147.17$\,Mpc (CAMB; log
  \texttt{p3\_rd\_reframe\_omega\_m}), consistent with
  the Planck-inferred value $147.09\pm0.26$\,Mpc (grey band, $0.32\sigma$) and the
  \LCDM\ Eisenstein--Hu value ($147.3$\,Mpc). The $175.6$\,Mpc scale shown
  in Fig.~\ref{fig:tension} is therefore the \emph{geometric BAO-sector}
  quantity. (The earlier MIRA mapping $\times|w_0|$ to $147.6$\,Mpc at
  $\Omega_{m,\mathrm{CMB}}=0.31993$ is superseded: $r_d$ now falls out
  directly from $\omega_m$.)}
  \label{fig:rd_dual}
\end{figure}
...
The companion Paper~3 reports $\chi^2_r=1.044$ (TT, 1971 multipoles) and a
canonical $\Delta\mathrm{BIC}=-32.9$ (full \texttt{plik} MCMC, $\omega_m$-direct,
$k_{\rm SSEE}=2$ vs $k_{\Lambda\rm CDM}=6$; $-23.9$ in the \texttt{plik\_lite}
point estimate), decisive evidence consistent with the hypothesis that the $+206$ penalty is a
...
\begin{itemize}
  \item \textbf{Dark-energy sector}: $(\wz,\wa)=(-0.840,-0.670)$
    lies within $0.16\sigma$ of DESI~DR2 (Pantheon+). The CPL free-fit posterior
...
and no matter factor (OP-8 dissolved).  The $21\sigma$ figure is therefore a
diagnostic of applying $\Lambda$CDM-calibrated priors to the dynamical sector,
not a falsification; the companion Paper~3 confirms this with the full CMB
...
Paper~6 free-streams below $k_{\rm fs}=0.762\,h$\,Mpc$^{-1}$, lowering the
effective amplitude to $\sigma_8^{\rm eff}=0.748$, $S_8^{\rm eff}=0.759$,
which resolves the KiDS-1000 tension ($0.01\sigma$).
...
Paper~6 adds the $\varphi$-DM two-sector and finds $\sigma_8^{\rm eff}=0.748$,
$S_8^{\rm eff}=0.759$ ($0.01\sigma$ KiDS).
...
\begin{enumerate}
  ...
  \item Under the $\omega_m$-direct reframe the sound horizon falls out
    ...
    full \texttt{plik} MCMC), reinterpreting the $21\sigma$ $\Omega_m$ tension as a diagnostic of
    applying $\Lambda$CDM-calibrated priors to the dynamical sector rather
    than a falsification of the dynamical ansatz.
...
the $0.16\sigma$ joint tension quoted in Section~\ref{sec:w0wa}.
...
At $z=2.33$ the $\phi$-DM (mass $41.02$\,eV, non-relativistic since
...
is recovered within ${\sim}1\sigma$. The largest residual across the 13 points
is at $z=0.51$ ($D_H/r_d$ pull ${\sim}+3.6\sigma$), driven by the CPL phantom
crossing $w(z{=}0.51)\approx-1.07$; an identical tension ($+4.4\sigma$) appears
in \LCDM\ with Planck-calibrated $r_d$ and $H_0$, confirming it is a DESI~DR2
feature rather than an \SSEE-specific failure.
...
$D_H/r_d=11.8$ (a $19.3\sigma$ discrepancy).  This is the same two-sector
...
($\ln B=+2.26$) then holds as a test of the \emph{dynamical sector} under this
