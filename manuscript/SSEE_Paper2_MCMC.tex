...
\begin{equation}
  ...
  \label{eq:rd_mapping}
\end{equation}
which is within $1\sigma$ of the Planck-inferred value $r_d = 147.09\pm0.26$\,Mpc \citep{Planck2018}.
...
algebraically fixed (Paper~1, Eq.~Y), and the sound horizon $r_d$ is
...
\begin{figure}[H]
  ...
  values. The $H_0$ posterior is consistent with Planck at $0.88\sigma$.}
...
algebraic mass $m_\phi = 5.60$\,eV) contributes at scales below its
free-streaming cutoff $k_{\rm fs}=0.493\,h$\,Mpc$^{-1}$.
...
H_0^{\rm alg}$), not a fitted quantity.}
...
$S_8=0.847$ ($3.9\sigma$ KiDS); Paper~6 adds the $\varphi$-DM two-sector and finds $\sigma_8^{\rm eff}=0.794$, $S_8^{\rm eff}=0.820$ ($2.6\sigma$ KiDS).
...
\begin{itemize}
  ...
  \item \textbf{If Ly-$\alpha$ forest observations exclude}
    $\sum m_\nu^{\rm active}=0.0824$\,eV at $>3\sigma$, the neutrino bridge in the
...
the $0.09\sigma$ joint tension quoted in Section~\ref{sec:w0wa}.
