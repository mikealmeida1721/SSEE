...
\subsection*{B.2 Predictive cross-validation (LOO-$z$)}

To test directly whether the parsimony advantage of SSEE is genuine
and not a boundary artefact, we split the 13 DESI~DR2 measurements by
redshift:
\begin{itemize}
\item \textbf{Training set} ($z<1.0$; 7 points, $z\in\{0.295,0.51,0.706,0.93\}$):
  each model is fit to these data only.
\item \textbf{Test set} ($z\geq1.0$; 6 points, $z\in\{1.317,1.491,2.33\}$):
  the trained MAP parameters are used to predict these unseen data.
\end{itemize}
SSEE has $k=1$ free parameter ($H_0$, with $w_0$, $w_a$, $\Omega_m$
fixed algebraically); \LCDM\ has $k=2$ ($H_0,\Omega_m$); CPL has
$k=4$ ($H_0,\Omega_m,w_0,w_a$).  The $z$-split exploits the fact that
the DESI covariance matrix has zero off-diagonal blocks between
different $z$-bins, making the sub-matrix inversion exact.

\begin{table}[h]
\centering
\caption{Predictive cross-validation results (LOO-$z$ split).
$\Delta\ln L_{\rm test}=\ln L_{\rm test}^{\rm SSEE}-\ln L_{\rm test}^{\rm model}$.
Positive values favour \textsc{ssee}.}
\label{tab:cv}
\begin{tabular}{lccc}
\toprule
Model & $k$ & $\chi^2_r$(train, $N=7$) & $\Delta\ln L_{\rm test}$ ($N=6$) \\
\midrule
\textsc{ssee} & 1 & 3.043 & --- \\
\LCDM         & 2 & 3.217 & $-0.17$ \\
CPL           & 4 & 1.055 & $+72.83$ \\
...
\begin{itemize}
  \item \textbf{SSEE vs \LCDM}: $\Delta\ln L_{\rm test}=-0.17$ nats
    ($\Delta\chi^2=0.34$ for 6 points).  This difference is negligible:
    SSEE ($k=1$) and \LCDM\ ($k=2$) have \emph{identical} predictive
    accuracy on the high-redshift test set despite SSEE having half the
    free parameters.
  \item \textbf{SSEE vs CPL}: $\Delta\ln L_{\rm test}=+72.83$; CPL
    ($k=4$) overfits the 7-point training set ($\chi^2_r=1.06$) and
    extrapolates catastrophically to the test set ($\chi^2_r=24.9$).
    This confirms that the freedom in the CPL dark-energy sector is
    \emph{not} informative about high-$z$ BAO.
\end{itemize}

Taken together, the Savage--Dickey ratio and the cross-validation
demonstrate that the dynamic-sector BIC advantage of SSEE
($\Delta\mathrm{BIC}_{\rm dyn}=-5.55$) reflects genuine algebraic
predictive power, not an artefact of fewer sampled parameters.
