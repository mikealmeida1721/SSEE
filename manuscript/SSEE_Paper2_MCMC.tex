...
Its value of~2 is an effective geometric approximation: the dimensionless ratio
$\Omega_{m,\mathrm{CMB}}/\Omega_{m,\mathrm{dyn}} = \mathrm{MIRA} \approx 2$
follows algebraically from the same $\{\varphi,\pi\}$ basis as $w_0$ and $w_a$
(Paper~4, \S2), predating DESI~DR2 (Zenodo: \texttt{10.5281/zenodo.19679049}).
The physical interpretation — viscous energy redistribution between matter and
radiation sectors — is quantified in Paper~1, Appendix~A: the IS growth
index $\gamma_{\rm IS}=0.657\pm0.002$ (Paper~1; superseded by $0.554$ in Paper~5)
and relaxation time $\tau_\Pi H_0 = \mathrm{KAL}_0/(3\Omega_{\mathrm{DE}})\approx 2.191$
are derived numerically from the $\{\varphi,\pi\}$ axioms, yielding
$S_8 = 0.837$ (within $0.4\sigma$ of Planck; Paper~5 gives $S_8=0.725$ with
$\gamma_{\rm IS}=0.554$ and $\Omega_{m,\mathrm{dyn}}=0.160$).
...
A fair comparison restricted to the dynamical dark-energy sector --- where the
