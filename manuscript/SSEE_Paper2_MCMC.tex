...
$k_{\rm fs}=0.762\,h$\,Mpc$^{-1}$), in the perturbations rather than the
background.
...
--- $\phi$-dark matter of algebraic mass $m_\phi=41.02$\,eV clustering below
$k_{\rm fs}=0.762\,h$\,Mpc$^{-1}$ --- describes how the total density partitions
...
\begin{figure}[H]
  ...
  \caption{\textit{Left}: $H_0$ posteriors for all models. With the total
  matter density in the background geometry the \SSEE\ posterior sits at
  $0.88\sigma$ from Planck~2018 (and $0.04\sigma$ from the algebraic anchor).
  \textit{Right}: MAP $r_d$ values; the \SSEE\ sound horizon
  ($148.2$\,Mpc) matches \LCDM\ ($147.8$\,Mpc, ratio $1.002$).}
  \label{fig:tension}
\end{figure}
...
independently favours \SSEE\ at $\Delta\mathrm{BIC}=-23.9$.
...
$H(z)$, and enlarges $D_H$ to an $8.1\sigma$ discrepancy. This is the same
