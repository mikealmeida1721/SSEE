...
(Paper~4, \S2), predating DESI~DR2 (Zenodo: \texttt{10.5281/zenodo.19679049}).
...
$\Omega_b h^2 = (\pi-\varphi)/H_0^{\rm SSEE} = 0.02242$ (Paper~4)
at $0.4\sigma$.
...
\paragraph{H$_0$: algebraic prediction vs.\ MCMC posterior.}
The \SSEE\ framework offers two independent estimates of $H_0$. The
geometric algebraic prediction is
$H_0^{\rm alg} = M_v \times \Omega = 3(\varphi+\pi)^2 = 67.962$\,km\,s$^{-1}$\,Mpc$^{-1}$,
where $M_v$ and $\Omega$ are fixed structural constants. The CMB-derived
physical value, obtained by re-evaluating Planck~\texttt{plik\_lite} under
the SSEE-MIRA background (Paper~3), is
$H_0^{\rm MIRA} = 67.037\pm0.54$\,km\,s$^{-1}$\,Mpc$^{-1}$. These two
estimates agree at $0.10\sigma$ (Planck-error scale), which is striking
given the unrelated origins of the two derivations (pure algebra vs.\
CMB peak fitting) and supports interpreting MIRA as a physical bridge
rather than a tuning factor.
...
The $\sim$1.2\,km\,s$^{-1}$\,Mpc$^{-1}$ difference (2.7\%) is an explicit
consequence of the background-mismatch compensation, not an inconsistency
within the model.
...
The companion Paper~3 reports $\chi^2_r=1.062$ (TT, 1971 multipoles) and
...
\SSEE\ predicts $H_0^{\rm alg} = 3(\varphi+\pi)^2 = 67.96$\,km\,s$^{-1}$\,Mpc$^{-1}$,
consistent with Planck~2018 at $1.1\sigma$ and with the DESI~DR2+CMB joint inference
...
\textit{The algebraic $H_0$ prediction is CMB-consistent but not distance-ladder consistent;
