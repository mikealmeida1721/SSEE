...
% Posterior H0 del MCMC reframe (prior H_alg=67.962, DESI DR2, geometría total 0.308881).
% 66.41 SUPERSEDED (V-L4-DESI 2026-07-09): el sector 0.160 estaba en E(z); con la
% materia total el posterior sube a 67.79 (0.50sigma del anchor). 67.16 era DR1.
...
% posterior quedaba sesgado hacia ella (fix R25, 2026-07-25). Valor vigente: 67.79.
% Fuente: results/logs/mcmc_paper2_reframe.log (100w×25k, seed 42, N_eff=78170).
\newcommand{\HZeroPosterior}{67.79^{+0.35}_{-0.35}}
...
favoured by $\Delta\mathrm{BIC}=-6.43$ over \LCDM\ and $-6.35$ over CPL, and
...
$\Omega_m=\omega_m/h^2=0.308881$ ($\omega_m$-direct; no matter factor, OP-8
dissolved), for which the SSEE sound horizon matches \LCDM\
($r_d^{\SSEE}/r_d^{\LCDM}=1.002$) and $\Omega_m$ agrees with Planck at
$0.88\sigma$. The cold sector $1+w_0=0.160050$ enters the perturbations
...
The two-sector $\phi$-DM framework once invoked to explain how the total
density partitions across scales has since been retracted (Paper~6): the
subtraction defining the second sector mixed a measured density with an
equation-of-state number. The results reported here do not depend on it ---
they use the total $\Omega_m$ throughout, which is what the correction of the
category error above already required.
\end{abstract}
...
uses the \emph{total} gravitating matter density $\Omega_m=\omega_m/h^2=0.308881$,
built piece-by-piece from SSEE algebra ($\omega_m$-direct; no matter factor,
OP-8 dissolved; Paper~3). The bookkeeping value $1+w_0=0.160050$ is the dark-energy
equation of state, \emph{not} a matter density: it enters only the perturbations
and the EFT coupling $\alpha_K$, never $E(z)$. (An earlier two-sector
$\phi$-DM reading of that split is retracted in Paper~6; nothing below depends
on it.) With the total density the SSEE sound horizon matches
$\Lambda$CDM and model selection yields a single, unambiguous figure:
$\Delta\mathrm{BIC}=-6.43$ favouring SSEE over $\Lambda$CDM and $-6.35$ over CPL
(Section~\ref{sec:bic}).
...
There is one matter density, $\Omega_m=\omega_m/h^2=0.308881$
(\S\ref{sec:folding}; no matter factor, OP-8 dissolved), hence
$\Omega_{\rm DE}=1-\Omega_m=0.691119$.
...
\begin{figure}[H]
  ...
  the physical density is $\Omega_{\rm DE}=0.691119$), shown against
...
Placing the cold sector alone ($1+w_0=0.160050$) in the background geometry would
...
that $\Omega_{m,\mathrm{CMB}}=\omega_m/h^2=0.308881$ and a CAMB integration of
...
(OP-8 dissolved; the actual ratio is $0.308881/0.160=1.93$, not a fixed factor).
MIRA survives only as the Hubble screening amplitude of Paper~9.
There is one matter density, $\Omega_m=\omega_m/h^2=0.308881$; the quantity
formerly written $\Omega_{m,\rm dyn}=0.160050$ is $1+w_0$, an equation-of-state
...
MCMC; $-24.0$ \texttt{plik\_lite} TTTEEE point estimate): if that fit had yielded $\chi^2_r\gg1$ or excluded
...
\begin{figure}[H]
  ...
  marks the structural retention at $x=0$, which enters both the cluster
...
fixed by SSEE algebra ($\omega_b=(\pi-\varphi)/[3(\varphi+\pi)^2]=0.022418$,
$\omega_c=\mathrm{KAL_0}\,\omega_b\,n_s=0.119514$; Paper~3, $\omega_m$-direct),
the CMB $\chi^2$ is minimised at the algebraic background value
$H_0^{\rm alg}/(\mathrm{km\,s^{-1}\,Mpc^{-1}})=3(\varphi+\pi)^2$:
\begin{equation}
...
matter density follows as $\Omega_{m,\mathrm{CMB}}=\omega_m/h^2=0.308881$,
...
matter density $\Omega_{m,\mathrm{CMB}}=\omega_m/h^2=0.308881$
(\S\ref{sec:folding}, \S\ref{sec:results_cosmo}); the value
$s_m=(\pi-\varphi)/[2(\varphi+\pi)]=1+w_0=0.160050$ labels the
...
where $\KALz\approx5.521$ is the algebraic Structural Retention constant
...
model-selection statistics. The headline posterior $H_0=67.79\pm0.35$ (with the
...
$T_r/M_v=0.839950$ (which is $|w_0|$, \emph{not} a density) are algebraically
...
is $H_0=67.53\pm0.35$\,km\,s$^{-1}$\,Mpc$^{-1}$ ($0.26\sigma$ from Planck);
under the algebraic prior (Eq.~\ref{eq:H0_prior_alg}) the reframe run gives the
headline $H_0=\HZeroPosterior$ ($0.66\sigma$ from Planck, $0.50\sigma$ from the
anchor $H_0^{\rm alg}=67.962$). With the total matter density
$\Omega_m=0.308881$ in the background geometry, the DESI~DR2 posterior
...
\begin{figure}[H]
  ...
  $\Omega_m=0.308881$ in $E(z)$, the $H_0$ posterior ($\HZeroPosterior$) is
  consistent with the CMB anchor ($0.50\sigma$) and lies $0.66\sigma$ from
...
prediction $w_0=-0.839950$ by $\sim6.3\sigma$ \emph{when evaluated against the
...
value $\Omega_m=\omega_m/h^2=0.308881$ (see below), \SSEE\ yields
$r_d^{\mathrm{SSEE}}=148.2\ \mathrm{Mpc}\approx r_d^{\LCDM}=147.8\ \mathrm{Mpc}$
and $100\theta_\ast=1.04089$ ($0.67\sigma$ from Planck): the sound horizon and
...
$\Omega_m = \omega_m/h^2 = 0.308881$, with
$\omega_m=\omega_b+\omega_c+\omega_\nu=0.14267$ built piece-by-piece from SSEE
algebra ($\omega_c=\mathrm{KAL_0}\,\omega_b\,n_s$, forward; Paper~3). This is
the standard physical matter density. The bookkeeping quantity $1+w_0=0.160050$ is
...
under the algebraic prior --- $0.66\sigma$ from Planck and $0.50\sigma$
from the algebraic anchor $H_0^{\rm alg}=67.962$. Removing the $H_0$ prior
altogether leaves DESI~DR2 alone (with only a BBN prior on $\omega_b$) at
$H_0=67.66\pm0.46$, $0.65\sigma$ from the anchor: the data are
\emph{consistent} with the algebraic value rather than pulling against it,
though they do not single it out. Under a common
Planck prior the three-model run gives $H_0^{\SSEE}=67.53\pm0.35$
($0.39\sigma$) versus $H_0^{\LCDM}=68.27\pm0.28$.

\paragraph{The $\Omega_m$ comparison.}
Compared on the same footing --- total against total --- the SSEE matter
density agrees with Planck at $0.88\sigma$
($\Omega_m=0.308881$ vs $0.3153\pm0.0073$). An earlier ``$21.3\sigma$ tension''
arose only from comparing the cold \emph{sector} ($1+w_0=0.160050$) against the
...
density $\Omega_m=0.308881$ with $1+w_0$, an equation-of-state number, so it
...
favours SSEE by $\Delta\mathrm{BIC}=-6.43$ over \LCDM\ and $-6.35$ over CPL, with
...
$H_0^{\rm alg}/(\mathrm{km\,s^{-1}\,Mpc^{-1}}) = 3(\varphi+\pi)^2 = 67.962$ is not a
...
With the \emph{total} matter density $\Omega_m=0.308881$ in the background
geometry, the MCMC posterior reported above,
$H_0^{\rm MCMC}=\HZeroPosterior$\,km\,s$^{-1}$\,Mpc$^{-1}$, \emph{coincides}
with the anchor: it lies $0.50\sigma$ from $H_0^{\rm alg}$ and $0.66\sigma$
from the $\Lambda$CDM-calibrated Planck value. The DESI~DR2 data are therefore
\emph{consistent} with the algebraic anchor rather than pulling against it. The earlier
...
\begin{figure}[H]
  ...
  \SSEE\ posterior sits at $67.53\pm0.35$, $0.26\sigma$ from Planck~2018 and
  fully consistent with the titular algebraic-anchor-prior value
  $67.79\pm0.35$ ($0.66\sigma$ Planck, $0.50\sigma$ from the anchor
...
\begin{figure}[H]
  ...
  $\omega_m=\omega_b+\omega_c+\omega_\nu=0.14267$
  ($\Omega_m=\omega_m/h^2=0.308881$, $\omega_m$-direct, no matter factor), the
  ...
  if the cold sector $1+w_0=0.160050$ is (incorrectly) used as the background
...
\end{equation}
With the total matter density $\Omega_m=0.308881$ in the background, the \SSEE\
...
drafts was an artefact of placing the cold sector $1+w_0=0.160050$ in the
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.839950\textwidth]{fig8_bao_residuals.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.839950\textwidth]{fig2_cluster_sensitivity.pdf}
...
$\Omega_m=0.308881$ in the background geometry). Jeffreys scale
...
\midrule
\SSEE & 2 & $-5.47$ & 16.49 & $0.00$  & $0.00$  & 14.95 \\
\LCDM      & 3 & $-7.30$ & 22.92 & $-6.43$ & $-5.65$ & 20.61 \\
CPL        & 5 & $-4.49$ & 22.85 & $-6.35$ & $-4.04$ & 18.97 \\
\bottomrule
\end{tabular}
\end{table}

With the total gravitating matter density in the background geometry, \SSEE\ is
the preferred model on every information criterion: $\Delta\mathrm{BIC}=-6.43$
over \LCDM\ and $-6.35$ over CPL, $\Delta\mathrm{AIC}=-5.65$ and $-4.04$, and
$\Delta\mathrm{DIC}=-5.66$ relative to \LCDM\ (Appendix~\ref{app:dic}). This is
...
-24.0$ (\texttt{plik\_lite} TTTEEE, $\omega_m$-direct). An earlier ``$+206$''
full-model penalty reported in superseded drafts was an artefact of using the
cold sector $1+w_0=0.160050$ as the background matter density; with the correct
...
is the SSEE-derived value (Paper~4, Neutrino Mass Sum), not a fitted
quantity.}%
% 2026-09-07: se retira de aqui la referencia a la relacion de masa del
% phi-DM, m_phi = Sum m_nu x (SOLAR^2 . KRYSTOS_V). No se retira por los
% nombres --- SOLAR = beta + KAL_0 y KRYSTOS_V = phi + pi + Omega siguen
% siendo entidades algebraicas vigentes del registro (Paper 1) --- sino
% porque la PARTICULA fue retirada el 2026-08-01 (Registro linea 128:
% "historico, no citar"): su densidad salia de restar Omega_m,CMB menos
% 0.160, y ese 0.160 es 1+w_0, un numero de la ecuacion de estado, no una
% densidad. Lo que la frase afirma de Sum m_nu (derivado, no ajustado)
% sigue siendo cierto y se conserva.
...
The \SSEE\ algebraic value $H_0^{\rm alg}/(\mathrm{km\,s^{-1}\,Mpc^{-1}}) = 3(\varphi+\pi)^2 = 67.962$
...
\begin{itemize}
  ...
  \item \textbf{Background structure}: with the total matter density
    $\Omega_m=0.308881$ the sound horizon matches \LCDM\ and \SSEE\ is favoured
    at $\Delta\mathrm{BIC}=-6.43$. Both $\Omega_m$ ($0.34\sigma$) and $H_0$
...
$\Omega_{m,\mathrm{CMB}}=0.308881$ is $r_d=147.17$\,Mpc directly
...
error --- comparing the cold sector $1+w_0=0.160050$ against the Planck
\emph{total} $\Omega_m$; the total $\Omega_m=0.308881$ agrees with Planck at
...
$\Omega_{m,\mathrm{CMB}}=0.308881$ ($=\omega_m/h^2$, no matter factor; OP-8
...
$\sigma_8=0.8149$, $S_8=0.827$, a $2.7\sigma$ tension with KiDS-1000
\citep{Heymans2021} and DES~Y3 \citep{Abbott2022}.
(Substituting $1+w_0=0.160050$ for a matter density here would give
$S_8\approx0.59$; that substitution is a category error and is not part of the
model --- see Paper~1.) The physical $S_8$ is measured in Paper~6 by fitting the
primordial amplitude to the \emph{raw} KiDS-1000 $\xi_\pm$ data with this
background held fixed: $\sigma_8=0.7446\pm0.0189$, $S_8=0.7555\pm0.0192$,\footnote{Updated 2026-09-20. Against the later \textsc{KiDS-Legacy} release (357 raw $\xi_\pm$ points; Wright et al.\ 2025, arXiv:2503.19441 --- published sixteen months before this analysis, over which no temporal priority is claimed) the amplitude the shear asks for lies $0.49\sigma$ from the one the CMB fixes under the \emph{same} algebraic background, so $A_s$ need not be free either: fixed to that value, the growth sector carries \emph{no} free cosmological parameter and predicts $S_8=0.8273$ against $0.8265\pm0.0176$ measured. The three $\chi^2$ compared span $1.0$ over 357 points, so the shear does \emph{not} discriminate between the models; what separates them is the parameter count. See Paper~6.}
statistically indistinguishable from the $\Lambda$CDM control run through the same pipeline on the same raw data ($0.7571\pm0.0194$), which $\Lambda$CDM needs four more free parameters to achieve. On the raw data itself --- the only quantity neither model produced --- $\Delta\chi^2=2.65$ for four \emph{fewer} free parameters, where four uninformative parameters would already return $4$ by chance ($\Delta\mathrm{BIC}=-19.0$ favouring \SSEE{}). There is no $S_8$ tension.
...
$\Omega_{m,\mathrm{CMB}}=0.308881$ and finds the single-sector ceiling
$\gamma_{\rm IS}=0.5504$ and, \emph{with $A_s$ fixed to the Planck value},
$\sigma_8=0.8149$, $S_8=0.827$ --- a number that reports the Planck--shear
disagreement, not a property of \SSEE{}, since $A_s$ is free in the model.
Fitted to the raw shear data instead, Paper~6 obtains $S_8=0.7555\pm0.0192$
(matching the $\Lambda$CDM control, $0.7571\pm0.0194$) with a single matter sector; the $3.4\sigma$ gap between
the two determinations is between the datasets and is not resolved here.
...
\begin{enumerate}
  ...
  \item \textbf{Damping tail.} The $\omega_m$-direct background
    ($\Omega_m=0.308881$) must be compatible with the Silk-damping scale \citep{Silk1968}
    and the third acoustic peak height. This is the most constraining test
    of the background geometry (Paper~3).
  ...
  \item \textbf{Transfer function derivation.} The semi-linear transfer
    function integrating the $\KALz$ structural retention into the
...
\begin{itemize}
  ...
  \item \textbf{If Ly-$\alpha$ forest observations exclude}
    ...
    for reconciling the cluster-mass predictions with observations.
...
\begin{enumerate}
  ...
  \item With the total matter density $\Omega_m=0.308881$ in the background,
    the \SSEE\ sound horizon matches \LCDM\ ($r_d=148.2$\,Mpc) and \SSEE\ is
    favoured on model selection by $\Delta\mathrm{BIC}=-6.43$ over \LCDM\ and
    $-6.35$ over CPL, predicting the held-out BAO data better in
    cross-validation.
  ...
  \item Under the $\omega_m$-direct reframe the sound horizon falls out
    ...
    the total $\Omega_m=0.308881$ agrees with Planck at $0.88\sigma$.
  ...
  \item A cross-check at high-$z$ is provided by the Ly-$\alpha$ forest BAO
    ...
    prediction (CMB-sector matter $\Omega_{m,\mathrm{CMB}}=0.308881$ in $E(z)$,
...
gravitating matter density $\Omega_m=\omega_m/h^2=0.308881$ in the background
...
\begin{equation}
  \Delta\mathrm{DIC}(\mathrm{SSEE}-\mathrm{\LCDM}) = -5.66,\quad
  \Delta\mathrm{DIC}(\mathrm{SSEE}-\mathrm{CPL}) = -4.02,
\end{equation}
...
\midrule
$p_D$   & $2.00$  & $3.00$  & $4.99$  & — \\
DIC     & $14.95$ & $20.61$ & $18.97$ & $-5.66$ \\
BIC     & $16.49$ & $22.92$ & $22.85$ & $-6.43$ \\
AIC     & $14.95$ & $20.60$ & $18.99$ & $-5.65$ \\
\bottomrule
\end{tabular}
\end{table}
...
($s_m=0.160050$, $r_d=175.6$\,Mpc).}
With the algebraic $H_0=67.962$\,km\,s$^{-1}$\,Mpc$^{-1}$ and
$s_m=0.160050$, the CPL dark-energy fraction at $z=2.33$ is
...
$s_m=0.160050$ in $E(z)$ reduces matter domination at high $z$, lowers
...
($\Omega_{m,\mathrm{CMB}}=0.308881$, $r_d=147.17$\,Mpc).}
At $z=2.33$ the full matter density sets the expansion, so
$E(z)$ takes the algebraic value
$\Omega_{m,\mathrm{CMB}}=\omega_m/h^2=0.308881$ (free-streaming suppresses
...
requires the full CMB-sector matter density $\Omega_{m,\mathrm{CMB}}=0.308881$,
where the cold sector counts as non-relativistic matter, rather than the
EoS-sector value $s_m=0.160050$: retaining it in $E(z)$ with $r_d=147.17$ would predict
