...
$1.19\times$ relative to \LCDM\ and yielding $\Delta\mathrm{BIC}=+206$
...
This $21.3\sigma$ figure is a \textbf{documented background incompatibility},
not a theory-level falsification of \SSEE.

It is a direct consequence of applying a prior on $\Omega_m$ that was
calibrated entirely within \LCDM\ cosmology to a model (\SSEE) that
assigns a fundamentally different physical role to $\Omega_{m,\rm eff}=0.160$.
In \SSEE\ the matter fraction is not a free parameter but an algebraic
theorem: $\Omega_{m,\rm eff} = 1 - T_r/M_v = 1 - (3\varphi+\pi)/[2(\varphi+\pi)]$.
When the MCMC sampler is forced to compare this fixed prediction against the
\LCDM-calibrated BAO $r_d$ formula (Eq.~\ref{eq:rd}), it cannot reconcile the
two without generating a large residual in $\Omega_m$.
The $21.3\sigma$ is therefore a measure of the \emph{incompatibility between
the two background cosmologies}, not a measure of how wrong the \SSEE\
prediction is.
A fair comparison requires either (a) a self-consistent \SSEE\ $r_d$ formula
or (b) an analysis restricted to the dynamical dark-energy sector, for which
\SSEE\ achieves $\Delta\mathrm{BIC}=-5.55$ (Section~\ref{sec:bic}).
