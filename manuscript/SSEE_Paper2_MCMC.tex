...
with $\rho(H_0,\Omega_m)=-0.85$. The SSEE algebraic baryon density derived in
Paper~4, $\Omega_b h^2 = (\pi-\varphi)/H_0^{\rm SSEE} = 0.02242$, is consistent
with this prior at $0.3\sigma$; the $0.2\%$ offset does not affect the posteriors
reported below. This prior was calibrated under
...
consistent with Planck~2018 ($67.36\pm0.54$) at $0.88\sigma$
[$\Delta H_0/\sqrt{\sigma_{\rm SSEE}^2+\sigma_{\rm Planck}^2}
=0.61/\sqrt{0.44^2+0.54^2}=0.88\sigma$]
and with the BAO-only run ($65.8^{+1.2}_{-1.1}$) at $0.6\sigma$, confirming
...
($H_0^{\SSEE}=66.75$ vs $H_0^{\LCDM}=67.99$\,km\,s$^{-1}$\,Mpc$^{-1}$),
...
The \SSEE\ framework independently derives an algebraic estimate
$H_0^{\rm alg} = M_v \times \Omega = 3(\varphi+\pi)^2 = 67.962$\,km\,s$^{-1}$\,Mpc$^{-1}$,
where $M_v$ and $\Omega$ are fixed structural constants.
The MCMC posterior, $H_0^{\rm MCMC}=66.75\pm0.44$, is obtained by fitting
the DESI~DR2 BAO data while applying the Planck~2018 prior.
The two values measure different quantities: $H_0^{\rm alg}$ is the theoretical
prediction of the background geometry; $H_0^{\rm MCMC}$ is the posterior after
the DESI data pull the value downward to compensate for the enlarged $r_d$.
