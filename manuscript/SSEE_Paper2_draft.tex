...
\begin{abstract}
We present a partial Bayesian validation of the dynamical sector of the
...
baseline \citep{Zhang2026}. Using geometric constants fixed by $\pi$ and
the golden ratio $\Phi$ — with $H_0$ as the sole free parameter — we
compare the fixed-geometry model against \LCDM\ and a free CPL
...
lies within $0.05\sigma$ of the DESI~DR2 best-fit in the $\wz$--$\wa$
plane. The cluster-mass sector achieves $\chi^2_r=0.12$ with the IGIMF
correction as the essential ingredient.
However, the fixed background $\OmDE^{\SSEE}\approx0.840$ implies
...
does \textit{not} confirm the full model. This tension is the BAO
manifestation of the structural $\OmDE$ discrepancy identified in
Paper~I; it delimits the regime where the standard Friedmann background
is no longer applicable to \SSEE. Under the \textit{Folding Symmetry}
hypothesis, the acoustic-mapping operator $|\wz|$ reproduces a
Planck-compatible effective scale,
$\rdeff=\rdSSEE\cdot|\wz|\approx147.2$\,Mpc, consistent with the
Planck~2018 value within $1\sigma$. This reinterpretation is a
hypothesis to be tested, not a resolution of the tension, and motivates
the modified Friedmann CMB analysis and semi-linear transfer function
required in Paper~3.
\end{abstract}
...
The \SSEE\ framework \citep{SSEE_paperI} derives all cosmological
observables from geometric invariants built from $\pi$ and
$\Phi=(1+\sqrt{5})/2$, without free parameters or exotic particles.
...
dynamical dark-energy analyses \citep{DESI_DR2,MNRAS2026}, we conduct a
...
mapping is:
\begin{equation}
  \rdeff = \rdSSEE\cdot|\wz| \approx 175.2\times0.840 \approx 147.2\;\text{Mpc},
  \label{eq:rd_mapping}
\end{equation}
...
\end{equation}
with sound horizon from the Eisenstein \& Hu (1998) fitting formula,
...
We use \texttt{emcee}~v3.1 \citep{emcee} with $N_w=32$ walkers and
$N_s=3000$ steps. After discarding $N_b=800$ burn-in steps (the first
$\sim27\%$ of the chain), walkers are visually stable and the
acceptance fraction lies in the range 0.25--0.50 for all models,
consistent with optimal mixing. Effective sample sizes are
$N_{\rm eff}\gtrsim1375$ for \LCDM\ and CPL; the \SSEE\ chain shows
higher autocorrelation due to its constrained parameter space
($N_{\rm eff}\sim660$), which is sufficient for marginal statistics
but warrants a longer run in a submission version.
...
\midrule
\SSEE-V3.6 & 2 & $\sim$0.38 & $\sim$70 & $\sim$43 &  $\sim$660 \\
\LCDM      & 3 &      0.41  & 42.6     &  70.5    & 1653 \\
CPL        & 5 &      0.35  & 51.2     &  58.6    & 1375 \\
\bottomrule
\end{tabular}
\end{table}
...
The \SSEE\ $H_0=66.66^{+0.47}_{-0.46}$\,km\,s$^{-1}$\,Mpc$^{-1}$ is
consistent with Planck~2018 ($67.36\pm0.54$) at $0.97\sigma$ and with
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\textwidth]{fig5_corner_ssee.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.68\textwidth]{fig6_corner_lcdm.pdf}
...
i.e.\ the prediction is statistically indistinguishable from the
best-fit dynamical dark-energy solution. The CPL posterior confirms
dynamical dark energy with $(\wz,\wa)\approx(-0.80,-0.58)$, bracketing
the \SSEE\ values.
...
($H_0^{\SSEE}=66.66$ vs $H_0^{\LCDM}=67.99$\,km\,s$^{-1}$\,Mpc$^{-1}$),
but this compensation is insufficient to absorb the full background
discrepancy. The residual tension is $0.97\sigma$ in $H_0$ but
...
\midrule
\multicolumn{7}{l}{$\chi^2_r$: SSEE\,$=0.122$ \;|\; $f_\nu{=}0$:\,$0.032$ \;|\;
  Std IMF:\,$11.93$ \;|\; $\KALz$ only:\,$12.65$} \\
\multicolumn{7}{l}{$\Delta\chi^2$ vs SSEE: $-0.36$ \;|\; $+47.2$ \;|\; $+50.1$} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{IGIMF is essential.}
Without the IGIMF correction $\Delta\chi^2\approx+47$;
\SSEE\ under-predicts masses by $\sim1.7\times$.

\paragraph{Neutrino bridge.}
$f_\nu=0$ lowers $\chi^2_r$ to $0.032$: the four $R_{500}$ clusters
...
$\chi^2_r=12.65$ without IGIMF. Geometric amplification requires
the corrected baryonic baseline.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.84\textwidth]{fig2_cluster_sensitivity.pdf}
  \caption{Cluster mass predictions by scenario. IGIMF correction
  is the load-bearing ingredient ($\Delta\chi^2\approx+47$ without it).}
...
\midrule
\SSEE-V3.6      & 2 & $-118.67$ & 242.89 & $+206.18$ & 241.34 & $+207.99$ \\
...
operator $|\wz|$ is applied, $\rdeff\approx147.2$\,Mpc recovers the
...
\begin{enumerate}
  ...
  \item Cluster masses achieve $\chi^2_r=0.12$ with IGIMF; removing
    the IGIMF correction raises $\chi^2_r$ to $11.93$
    ($\Delta\chi^2\approx+47$), confirming its essential role.
  ...
  \item The Folding Symmetry hypothesis — $\rdeff=\rdSSEE\cdot|\wz|
    \approx147.2$\,Mpc — would recover the Planck sound horizon within
    $1\sigma$ if confirmed, reinterpreting the $21\sigma$ tension as a
...
dark-energy prediction that the data cannot reject — the burden of proof
now falls on the background geometry.
