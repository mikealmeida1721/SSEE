...
$\wz$--$\wa$ plane (note: \LCDM\ has $w_0=-1$, $w_a=0$ fixed and is
not a free model in this plane; the relevant comparison is against CPL). The cluster-mass sector achieves $\chi^2_r=0.13$ across seven clusters with the IGIMF
...
The correct test is a full CMB power-spectrum likelihood with the \SSEE\
Friedmann background built from first principles, which is the subject of
Paper~3.  The fact that Paper~3 recovers $\chi^2_r=1.062$ with
$\Delta\mathrm{BIC}=-6.9$ \emph{under the \SSEE\ background} provides
the complementary evidence that this $+206$ penalty is a calibration
artefact, not a physical falsification.
