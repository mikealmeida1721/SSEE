...
radiation sectors — remains a qualitative argument; a quantitative IS-perturbation
derivation is deferred to future work.
...
For a broader consistency check, we extend the analytic forward-model
to three additional clusters: Perseus \citep{Simionescu2011},
A2142 \citep{Tchernin2016}, and A2744 \citep{Owers2011}.
For these clusters we apply the same IGIMF correction factor ($\approx1.71$)
derived from the Zhang~2026 sample, since an independent IGIMF re-analysis
is not available for each source.
...
($10^{14}\,M_\odot$, within $R_{500}$).
$M_{\rm bar}^{\rm IGIMF}$: IGIMF-corrected baryonic baseline.
$M_{\rm obs}$: total observed mass within $R_{500}$.
First four from \citet{Zhang2026}; remainder use the same IGIMF
correction factor applied to published $M_{500}$ values.}
...
Perseus       & \citet{Simionescu2011} & 1.10 & 6.0  & 0.7 \\
A2142         & \citet{Tchernin2016}   & 1.80 & 9.8  & 1.0 \\
A2744         & \citet{Owers2011}      & 2.40 & 14.5 & 2.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\SSEE-V3.6      & 2 & $-124.87$ & 255.28 & $+218.58$ & 253.73 & $+220.38$ \\
...
and should not be conflated: (i) $\Delta\mathrm{BIC}=+218$ quantifies whether
