...
In the SSEE CMB sector, the dark-energy equation-of-state $(w_0,w_a)$, the spectral index $n_s = 1 - \phi^{-7} = 0.965558$, the baryon density $\Omega_b h^2 = (\pi-\varphi)/(3\Omega^2) = 0.022418$, and the CDM density $\Omega_c h^2 = \mathrm{KAL}_0\,\Omega_b h^2\,n_s = 0.119514$ ($\omega_m$-direct) are all algebraically fixed, and the geometry anchor $H_0 = 3(\varphi+\pi)^2 = 67.962$ is derived (Paper~9). The only two model parameters genuinely sampled are therefore:
...
\begin{figure}[H]
  ...
  green line marks the SSEE algebraic prediction $H_0^\mathrm{alg}=67.962$\,km\,s$^{-1}$\,Mpc$^{-1}$.
...
$H_0=67.962$~km\,s$^{-1}$\,Mpc$^{-1}$) and verified the
