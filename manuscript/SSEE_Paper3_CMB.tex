...
In a full, unified evaluation using the official off-diagonal \texttt{plik\_lite} TTTEEE likelihood via Cobaya, scanning only $H_0$ ($k=2$ vs $k=6$ for $\Lambda$CDM), the combined spectra yield $\Delta\chi^2 = +3.8$ and $\Delta\mathrm{BIC}=-31.3$, decisively favouring SSEE. Even under a hyper-conservative penalty treating fixed priors as free parameters ($k=4$), $\Delta\mathrm{BIC}=-13.8$ continues to strongly favour the framework.
