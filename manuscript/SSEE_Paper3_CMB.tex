...
$\Delta\mathrm{BIC}=-23.9$.) The $\Delta\mathrm{BIC}$ is driven by parsimony, not by
fit quality.
...
w_a &= -\frac{P_{sc}}{\mathrm{IGNIS}} = -\frac{P_{sc}}{\pi+P_{sc}} = -0.66997, \label{eq:wa}\\
...
$w_0$ & $-0.8399$ & $-T_r/M_v$ \\
$w_a$ & $-0.66997$ & $-P_{sc}/\mathrm{IGNIS}$ \\
...
SSEE achieves $\chi^2_r=0.719$ over the 9 bandpowers
...
$\chi^2_r=0.719$ vs.\ $0.757$ for $\Lambda$CDM --- slightly \emph{better} agreement
...
Under the diagonal approximation, SSEE is favoured ($\Delta\mathrm{BIC}=-34.9$):
...
\texttt{plik\_lite} point est. & TTTEEE          & 613  & 2 & 6 & $-23.93$ (SSEE decisively favoured) \\
...
\begin{itemize}
  ...
  \item Lensing (PP): $\chi^2_r=0.719$ vs.\ $0.757$ for $\Lambda$CDM — SSEE marginally better (both $\chi^2_r<1$).
  \item \textbf{\texttt{plik\_lite} TTTEEE ($\omega_m$-direct):} $\chi^2_{\mathrm{eff}} = 1005.50$ at $H_0 = 67.962$ km\,s$^{-1}$\,Mpc$^{-1}$ (the CMB-minimising value with SSEE-fixed $\omega_b,\omega_c$).
  \item $\Delta\mathrm{BIC}=-23.93$ ($k=2$ vs $k=6$), decisively favouring SSEE over $\Lambda$CDM.
