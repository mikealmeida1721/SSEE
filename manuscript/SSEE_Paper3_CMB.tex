...
defined a priori in the SSEE constant dictionary as the
\textit{Observational Frequency}, fixed by the algebraic construction and not
tuned to the CMB data,
...
$\chi^2_r=1.044$ vs.\ $\Lambda$CDM's $1.043$ over 1971 TT multipoles.
The full TT+TE+EE+lensing analysis yields $\chi^2_r = 1.044, 1.041, 1.040, 0.866$
...
In a full, unified evaluation using the official off-diagonal \texttt{plik\_lite} TTTEEE likelihood via Cobaya, scanning only $H_0$ ($k=2$ vs $k=6$ for $\Lambda$CDM), the combined spectra yield $\Delta\chi^2 = +2.9$ (SSEE fits marginally worse in absolute $\chi^2$) and $\Delta\mathrm{BIC}=-32.2$, decisively favouring SSEE. The $\Delta\mathrm{BIC}$ is dominated by the parameter-count penalty ($\Delta k \times \ln N \approx 4\times\ln 6469 \approx 35.1$ BIC units) that rewards SSEE's zero-free-parameter design; the raw $\Delta\chi^2=+2.9$ penalty is more than offset by this parsimony gain. Even under a hyper-conservative penalty treating fixed priors as free parameters ($k=4$), $\Delta\mathrm{BIC}=-14.7$ continues to strongly favour the framework.
...
This constant was not introduced post-hoc: its value $1.9989$ is fixed by the
algebraic construction of the dictionary---the same system that determines $w_0$,
$w_a$, and $\Omega_{m,\mathrm{dyn}}$---and is not a free parameter available for
tuning to the CMB data.

\subsection{MIRA is fixed by construction, not fitted}
\label{sec:mira_timeline}

MIRA is one entry of the closed constant dictionary
\citep{AlmeidaSSEE2026genesis}, with value
$\mathrm{MIRA}=\mathrm{AURA}/2=(3\varphi+\pi)/4=1.9989\ldots$ fixed by the same
algebraic rules that determine $w_0$, $w_a$, and $\Omega_{m,\mathrm{dyn}}$. It is
not a free parameter and was not adjusted to the CMB comparison. We make no claim
of temporal priority over the public Planck measurement of $\Omega_m$: the relevant
point is structural, not chronological---because the value is set by the
construction before any comparison, the agreement below is a constraint the
framework had to satisfy, not a fit it was granted. Its cosmological role as the bridge between the dynamic
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item \textit{Structurally fixed, not fitted}: MIRA is determined by the
        algebraic construction of the constant dictionary, independently of the
        CMB data against which it is compared; it introduces no free parameter.
        We make no claim of temporal priority over public data (DESI, Planck),
        which predate this work---the strength of the assignment is its structural
        rigidity, not its chronology.
...
\begin{equation}
    \label{eq:bic_cobaya}
    \Delta\mathrm{BIC} = 2.875 + (2 - 6)\ln(6469) = -32.2.
\end{equation}
...
plik\_lite Cobaya & TTTEEE+lowl & 6469 & 2 & 6 & $-32.2$ (SSEE decisively favoured) \\
...
\begin{itemize}
  ...
  \item $\Delta\mathrm{BIC}=-32.2$ (full covariance, $k=2$ vs $k=6$), decisively favouring SSEE over $\Lambda$CDM.
