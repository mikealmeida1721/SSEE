...
$\chi^2_r=1.064$ vs.\ $\Lambda$CDM's $1.043$ over 1971 TT multipoles.
The full TT+TE+EE+lensing analysis yields $\chi^2_r = 1.064, 1.054, 1.040, 0.736$
for SSEE vs $1.043, 1.040, 1.039, 0.757$ for $\Lambda$CDM.
In the lensing sector (9 MV bandpowers), SSEE shows a marginal preference
over $\Lambda$CDM ($\Delta\chi^2_r = -0.021$).
...
\midrule
$\Lambda$CDM (Planck 2018) & 2055.8 & 1.043 & 0 \\
SSEE+MIRA & 2097.1 & 1.064 & $+41.3$ \\
...
\midrule
TT & 1971 & 1.064 & 1.043 & $+0.021$ \\
TE & 1967 & 1.054 & 1.040 & $+0.014$ \\
EE & 1967 & 1.040 & 1.039 & $+0.001$ \\
PP & 9    & 0.736 & 0.757 & $-0.021$ \\
\midrule
Combined$^{\dagger}$ & 5914 & 1.052 & 1.040 & $+0.012$ \\
\bottomrule
\end{tabular}
...
The TE residual is intermediate ($+0.014$), while TT shows the largest
but still modest deviation ($+0.021$).
Notably, in the lensing potential sector (PP), SSEE achieves
$\chi^2_r=0.736$ vs.\ $0.757$ for $\Lambda$CDM --- the dark energy modification
...
SSEE+MIRA & 2 & 1.052 & $+37.1$ \\
\bottomrule
\end{tabular}
\end{table}

Under the diagonal approximation, $\Lambda$CDM is preferred ($\Delta\mathrm{BIC}=+37.1$).
However, this result is driven by $N=5914$ amplifying a marginal $\Delta\chi^2_r\approx0.012$,
...
This diagonal cross-check is evaluated at the SSEE reference value
$H_0=66.66$~km\,s$^{-1}$\,Mpc$^{-1}$; the canonical \texttt{plik\_lite} MCMC
(\S\ref{sec:b1_mcmc}) instead fits $H_0=67.08\pm0.03$, and the small difference
does not affect the model-selection conclusion.
...
The diagonal result ($\Delta\mathrm{BIC}=+37.1$) is driven by $N\approx5914$ amplifying
a marginal $\Delta\chi^2_r\approx0.012$; the full covariance Cobaya result is the canonical evaluation.}
\label{tab:bic_comparison}
\begin{tabular}{llcccc}
\toprule
Method & Spectra & $N$ & $k_{\rm SSEE}$ & $k_{\Lambda\rm CDM}$ & $\Delta\mathrm{BIC}$ \\
\midrule
Diagonal (approx.) & TT+TE+EE+PP & 5914 & 2 & 6 & $+37.1$ ($\Lambda$CDM favoured) \\
...
\begin{itemize}
  ...
  \item TT: $\chi^2_r=1.064$ over 1971 multipoles ($\Lambda$CDM: $1.043$).
  \item TE/EE: $\chi^2_r=1.054/1.040$, essentially matching $\Lambda$CDM.
  \item Lensing (PP): $\chi^2_r=0.736$ vs.\ $0.757$ for $\Lambda$CDM — SSEE marginally favoured.
