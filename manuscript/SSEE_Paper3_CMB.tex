...
($s_m=0.160050$, $w_0=-0.839950$, $w_a=-0.669975$) fits DESI~DR2 BAO data
...
$s_m=0.160050$ (DESI EoS sector) and the CMB value $\Omega_{m,\mathrm{CMB}}=0.3089$
...
$s_m=0.160050$ is $1+w_0$, an equation-of-state parameter rather
...
$s_m=1+w_0=0.160050$ ($z\lesssim2.3$), validated independently in
...
parameters. There is one matter density in this model, $\Omega_m=0.308881$
...
s_{\mathrm{DE}} &= \frac{T_r}{M_v} = \frac{3\varphi+\pi}{2(\varphi+\pi)} = 0.839950, \label{eq:sDE}\\
s_m &= 1 - s_{\mathrm{DE}} = 0.160050. \label{eq:sm}
\end{align}
...
With $s_m=0.160050$, the Hubble parameter at recombination scales as
...
The naive background uses $s_m=0.160050$, which inflates both the comoving
...
The sector value $s_m=1+w_0=0.160050$ (DESI) and the CMB value
...
$s_m \equiv 1+w_0 = (M_v-T_r)/M_v = 0.160050$ is a
...
SSEE naive ($s_m=0.160050$) & 242 & 601 & 930 \\
...
Note that the raw SSEE \emph{dynamical} background ($s_m=0.160050$)
