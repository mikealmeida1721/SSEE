...
$N=2409$ data points, $\Delta\chi^2_{\rm BF}=-1.8$), while the parameter-count economy
yields $\Delta\mathrm{BIC}=-33.0$, decisively favouring SSEE on the Jeffreys scale.
...
This gives $k_\mathrm{SSEE} = 2$ model parameters ($\log A_s$, $\tau$ --- the only quantities genuinely sampled, with $H_0$ fixed at the algebraic anchor; the $\sim$20 \texttt{plik} nuisance parameters are common to both models and excluded from $\Delta k$), compared to the $\Lambda$CDM run with $k_\mathrm{\Lambda CDM} = 6$ free parameters ($H_0$, $\Omega_b h^2$, $\Omega_c h^2$, $n_s$, $\log A_s$, $\tau$). The $k=2$ vs $k=6$ BIC penalty over the $N=2409$ combined data points is $\Delta k \cdot \ln N = 4 \times \ln(2409) = 31.15$, yielding:
\begin{equation}
    \label{eq:bic_b1}
    \Delta\mathrm{BIC}_{k2} = \Delta\chi^2_\mathrm{BF} - 31.15,
\end{equation}
where $\Delta\chi^2_\mathrm{BF} = \chi^2_{\mathrm{SSEE,BF}} - \chi^2_{\Lambda\mathrm{CDM,BF}}$ is the difference in best-fit $\chi^2$ between the two models over the same $N=2409$ data points (evaluated through \texttt{clipy}; no \texttt{clik} binary required).
...
$\Delta\mathrm{BIC}$ ($k=2$ vs $k=6$, $N=2409$) & $-33.0$ & (reference) \\
$\Delta\mathrm{BIC}$ ($k=4$ vs $k=6$, $N=2409$) & $-17.4$ & (reference) \\
\bottomrule
\end{tabular}
\end{table}
...
Full \texttt{plik} MCMC   & TTTEEE+lowl+lensing  & 2409 & 2 & 6 & $\mathbf{-33.0}$ \emph{(canonical)} \\
Full \texttt{plik} (conserv.) & TTTEEE+lowl+lensing & 2409 & 4 & 6 & $-17.4$ (SSEE strongly favoured) \\
\bottomrule
\end{tabular}
...
Paper~6 two-sector gives the same $0.70\sigma$ (at RSD scales $k\ll k_{\rm fs}$
the $\varphi$-DM clusters as cold matter; the two-sector primarily resolves the
