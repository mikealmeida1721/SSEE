...
at $0.05\sigma$ and galaxy-cluster mass ratios with $\chi^2_r=0.122$.
...
\emph{conditional on the MIRA phenomenological mapping hypothesis}
(\S\ref{sec:mira}); a first-principles derivation of the factor $1.9989$
from the Israel--Stewart transport equation remains open.
...
A formal evaluation using the \texttt{plik\_lite} TTTEEE likelihood (Cobaya) at the
CMB-preferred $H_0=67.075$~km\,s$^{-1}$\,Mpc$^{-1}$ gives $\Delta\chi^2_\mathrm{eff}=+3.5$
and $\Delta\mathrm{BIC}=-40.3$ with $k=1$ (SSEE predicts; $\Lambda$CDM fits 6 parameters),
\textbf{strongly favouring SSEE} on the Jeffreys scale.
In a conservative diagonal BIC comparison ($k=2$ vs $k=6$), the combined spectra yield
$\Delta\mathrm{BIC}=+31.1$ favouring $\Lambda$CDM; isolating TT alone gives
$\Delta\mathrm{BIC}=-6.9$ favouring SSEE.
The primary model-comparison result of this paper is $\Delta\mathrm{BIC}=-40.3$;
the range $[-40.3,\,+31.1]$ brackets the full sensitivity to the assumed number of SSEE free parameters.
...
and the complete domain delineation are in Paper~1, \S\ref{sec:domain} and
...
A formal derivation of MIRA from the IS transport equation remains
an open problem.
...
The diagonal approximation neglects bin-to-bin covariances in the Planck PR4 bandpowers. Based on the Planck 2018 likelihood analysis \citep{PlanckLikelihood2020}, off-diagonal elements contribute corrections of $\lesssim5\%$ to $\chi^2$ for $\ell>30$, where the large majority of our 1971 multipoles reside. A direct cross-check is provided by the Cobaya/\texttt{plik\_lite} evaluation (\S\ref{sec:cobaya}), which uses the proper correlated likelihood: at the CMB-optimal $H_0=67.075$~km\,s$^{-1}$\,Mpc$^{-1}$, the \texttt{plik\_lite} likelihood yields $\Delta\chi^2_\mathrm{eff}=+3.5$ relative to \LCDM, compared to $\Delta\chi^2 \approx 37$ implied by the diagonal TT approximation at the same multipole range. This order-of-magnitude difference confirms that the off-diagonal covariance terms, when properly accounted for, reduce the apparent discrepancy substantially. The $\Delta\mathrm{BIC}=-40.3$ result from \S\ref{sec:cobaya} is therefore the primary quantitative result of this paper for model comparison; the diagonal $\chi^2_r$ values serve as qualitative indicators only.

\subsection{Cobaya \texttt{plik\_lite} evaluation and acoustic scale analysis}
\label{sec:cobaya}

\noindent\begin{tcolorbox}[colback=green!4,colframe=green!50!black,title=Reproducibility: Cobaya/plik\_lite Evaluation,width=\linewidth]
\textbf{Code:} Cobaya v3.5.4 \citep{Torrado2021} + CAMB v1.6.6 \citep{Lewis2000}.
\textbf{Likelihood:} \texttt{planck\_2018\_highl\_plik.TTTEEE\_lite} (official Planck 2018 release).\\[2pt]
\textbf{Fixed parameter vector} (all derived algebraically or from DESI posterior; none fitted to CMB):
\begin{align*}
  w_0 &= -T_r/M_v = -0.8399, \quad
  w_a = -P_{sc}/K_v = -0.6700, \\
  n_s &= 1 - \varphi^{-7} = 0.96556, \quad
  \Omega_{m,\mathrm{CMB}} = \Omega_{m,\mathrm{dyn}}\times\mathrm{MIRA} = 0.31993,\\
  \Omega_b h^2 &= 0.02237\ \text{(Planck 2018 prior)},\quad
  \ln(10^{10}A_s) = 3.044,\quad \tau = 0.054.
\end{align*}
\textbf{Free parameter:} $H_0$ scanned over $[66, 68]$ km\,s$^{-1}$\,Mpc$^{-1}$
($\Omega_c h^2$ derived consistently from $\Omega_{m,\mathrm{CMB}}$ at each $H_0$).\\[2pt]
\textbf{Reference run:} $\Lambda$CDM best-fit (6 free parameters) gives
$\chi^2_\mathrm{eff}=584.97$ (Table~\ref{tab:h0scan}).\\[2pt]
\textbf{Reproducibility:} the full parameter file and run script are archived at
Zenodo \texttt{10.5281/zenodo.19679049} alongside the SSEE codebase.
\end{tcolorbox}
...
information from Planck --- is addressed by the Zenodo timestamp and the
Predictive Register in Paper~1 \S1: all algebraic parameters were derived
before the CMB analysis reported here.

\paragraph{Sensitivity of \texorpdfstring{$\Delta$BIC}{ΔBIC} to the assumed number of free parameters.}
With $\Delta\chi^2_\mathrm{eff}=+3.5$, $N=6413$ \texttt{plik\_lite} bins ($\ln N=8.766$),
and $k_{\Lambda\mathrm{CDM}}=6$, the BIC verdict is:
\begin{equation}
  \Delta\mathrm{BIC}(k_{\SSEE}) = 3.5 + (k_{\SSEE}-6)\times 8.766.
\end{equation}
\begin{table}[H]
\centering
\caption{$\Delta$BIC sensitivity to the assigned number of free \SSEE\ parameters
($N=6413$ \texttt{plik\_lite} bins, $\Delta\chi^2=+3.5$, $k_{\Lambda\mathrm{CDM}}=6$).
Jeffreys scale: $\Delta\mathrm{BIC}<-10$ = strong evidence for SSEE;
$\Delta\mathrm{BIC}\in(-10,0)$ = moderate; $\Delta\mathrm{BIC}>0$ = inconclusive or favours \LCDM.}
\label{tab:bic_sensitivity}
\small
\begin{tabular}{crl}
\toprule
$k_{\SSEE}$ & $\Delta$BIC & Jeffreys verdict \\
\midrule
0 & $-49.1$ & strong evidence for \SSEE \\
\textbf{1} & $\mathbf{-40.3}$ & \textbf{strong (used; $H_0$ only free)} \\
2 & $-31.6$ & strong evidence for \SSEE \\
3 & $-22.8$ & strong evidence for \SSEE \\
4 & $-14.0$ & strong evidence for \SSEE \\
5 & $-5.3$  & moderate evidence for \SSEE \\
6 & $+3.5$  & inconclusive \\
\bottomrule
\end{tabular}
\end{table}
\SSEE\ is favoured on the Jeffreys scale for $k_{\SSEE}\leq 4$ --- even if one
generously assigns four additional free parameters beyond $H_0$.  The result
$\Delta\mathrm{BIC}=-40.3$ is therefore robust: it holds for any credible
accounting of the SSEE parameter count.  The conservative diagonal comparison
($\Delta\mathrm{BIC}=+31.1$, $k=2$; §\ref{sec:stats}) uses a different dataset
approximation ($N_{\rm diag}\approx5900$ multipoles, diagonal noise) and a more
conservative $k$ assignment; it brackets the upper end of the sensitivity range.
The full range is $[-40.3,\,+31.1]$, summarising all plausible counting choices.
...
solution of the IS growth ODE (Paper~1, Appendix~A) yields
$\gamma_{\mathrm{IS}} = 0.657\pm0.002$, lying 6\% above the algebraic
prediction (compared to $\gamma_\Lambda=0.55$ for $\Lambda$CDM).
...
The IS numerical solution from Paper~1, Appendix~A gives
$\gamma_\mathrm{IS}=0.657$, yielding $S_8=0.818$ (1.1\,$\sigma$ below Planck),
with the 6\% discrepancy between $\gamma_\mathrm{alg}$ and $\gamma_\mathrm{IS}$
translating to a $\sim 1\%$ change in $S_8$.
Both predictions exceed $\Lambda$CDM ($S_8^\Lambda=0.830$) in matching Planck
while maintaining the $\sim 3\sigma$ tension with KiDS-1000 and DES~Y3 that
persists in $\Lambda$CDM.
...
SSEE+IS ($\gamma_\mathrm{IS}=0.657$, Paper~1) & \textbf{0.792} & \textbf{0.818} \\
\hline
...
\caption{$f\sigma_8(z)$ predictions vs RSD data.
  SSEE: $\gamma_\mathrm{IS}=0.657$ (Paper~1, App.~A), $\sigma_{8,\mathrm{IS}}=0.792$.
...
13 RSD data points, compared to $0.596$ for $\Lambda$CDM — a modest but
...
\begin{enumerate}
  ...
  \item \textbf{Growth index:} The algebraic prediction is $\gamma_\mathrm{alg}=\phi^{-1}=0.618$;
    the IS numerical solution (Paper~1, App.~A) gives $\gamma_\mathrm{IS}=0.657\pm0.002$,
    yielding $S_8=0.818$ (see Table~\ref{tab:sigma8}).
    The RSD comparison (Table~\ref{tab:fsig8}) shows $\chi^2/N=0.524$
    consistently below $\Lambda$CDM across 13 surveys.
    A direct measurement of $\gamma<0.55$ or $\gamma>0.70$ at $>3\sigma$
