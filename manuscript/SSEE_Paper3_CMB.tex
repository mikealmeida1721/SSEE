...
($\Omega_{m,\mathrm{dyn}}=0.160050$, $w_0=-0.839950$, $w_a=-0.669975$) fits DESI~DR2 BAO data
...
is $\omega_m=\omega_b+\omega_c+\omega_\nu$, where $\omega_b=(\pi-\varphi)/[3(\varphi+\pi)^2]=0.022418$
and $\omega_c=\mathrm{KAL}_0\,\omega_b\,n_s=0.119514$ are forward identities of Paper~1
...
$\Omega_{m,\mathrm{dyn}}=0.160050$ (DESI) and the CMB value $\Omega_{m,\mathrm{CMB}}=0.3089$
...
No ``difference'' requires a physical carrier: there is a single matter
density $\Omega_m=\omega_m/h^2=0.308881$, and the quantity once written
$\Omega_{m,\rm dyn}=0.160050$ is $1+w_0$, an equation-of-state parameter rather
than a density. The two-sector structure formerly invoked here is retracted
(Paper~6; Paper~1 \S1.4).
...
$\Omega_{m,\rm dyn}=1+w_0=0.160050$ ($z\lesssim2.3$), validated independently in
...
$\mathrm{KAL}_0$ & $\beta+\pi$ & $5.52141\ldots$ & Structural retention \\
...
$H_0/(\mathrm{km\,s^{-1}\,Mpc^{-1}}) = 3(\varphi+\pi)^2 = M_v\times\Omega = 67.96214$
...
With $\Omega_m=0.160050$, the Hubble parameter at recombination scales as
...
The naive background uses $\Omega_m=0.160050$, which inflates both the comoving
...
\begin{equation}
  ...
  \omega_b = \frac{\pi-\varphi}{3(\varphi+\pi)^2} = 0.022418, \qquad
  \omega_c = \mathrm{KAL}_0\,\omega_b\,n_s = 0.119514,
...
\begin{equation}
  ...
  \qquad (H_0^{\rm alg} = 67.96214).
\end{equation}

\subsection{No matter factor: two independent predictions}
\label{sec:mira_timeline}

The dynamic value $\Omega_{m,\mathrm{dyn}}=1+w_0=0.160050$ (DESI) and the CMB value
...
\subsection{The two-sector reading, and why it is withdrawn}
\label{sec:two_sector_physics}

An earlier version of this section read the two densities as a \emph{partition}:
their difference,
\begin{equation}
  \Omega_{\phi\text{-DM}}
    \;=\; \Omega_{m,\mathrm{CMB}} - \Omega_{m,\mathrm{dyn}}
    \;=\; 0.3089 - 0.1601 \;=\; 0.149 ,
  \label{eq:withdrawn_subtraction}
\end{equation}
was taken to define a second, warm dark-matter component (Paper~6).
That reading is \textbf{withdrawn} (2026-08-01), for a reason internal to
the algebra rather than to any measurement.

The subtracted quantity is not a density. In SSEE,
$\Omega_{m,\mathrm{dyn}} \equiv 1+w_0 = (M_v-T_r)/M_v = 0.160050$ is a
\emph{saturation}: it is the complement of the equation-of-state ratio
$-w_0 = T_r/M_v$, and it carries no factor of $H$. Equation
(\ref{eq:withdrawn_subtraction}) therefore subtracts a number belonging to the
equation of state from a measured mass density. The result is dimensionally
well formed and physically empty: there was nothing for the inferred component
to be made of. Paper~6 gives the full retraction; the particle mass
$m_\phi$, the free-streaming scale $k_{\rm fs}$, and the two-sector
$\sigma_8$/$S_8$ values that followed from
Eq.~(\ref{eq:withdrawn_subtraction}) are withdrawn with it.

Two consequences for the present paper. First, the tension the partition was
introduced to relieve does not exist: measured against the \emph{raw}
KiDS-1000 $\xi_\pm$ data with $A_s$ free, a single matter sector gives
$S_8 = 0.7555 \pm 0.0192$, i.e.\ $0.11\sigma$ from KiDS-1000\footnote{Updated 2026-09-20. Against the later \textsc{KiDS-Legacy} release (357 raw $\xi_\pm$ points; Wright et al.\ 2025, arXiv:2503.19441 --- published sixteen months before this analysis, over which no temporal priority is claimed) the amplitude the shear asks for lies $0.49\sigma$ from the one the CMB fixes under the \emph{same} algebraic background, so $A_s$ need not be free either: fixed to that value, the growth sector carries \emph{no} free cosmological parameter and predicts $S_8=0.8273$ against $0.8265\pm0.0176$ measured. The three $\chi^2$ compared span $1.0$ over 357 points, so the shear does \emph{not} discriminate between the models; what separates them is the parameter count. See Paper~6.}
($0.759\pm0.024$). Second, and more importantly for what follows, nothing in
the CMB confrontation of this paper rested on the subtraction. The forward
identity of Eq.~(\ref{eq:omega_m_direct}) and its comparison with Planck are
untouched; they are restated below without it.

\paragraph{A mechanism withdrawn with the partition.}
The same earlier reading proposed a dissipative source
$\propto \mathrm{KAL}_0 H^3/8\pi G$ in the SSEE continuity equation as the
``geometric retention'' that linked the two sectors. It is withdrawn on two
counts: it links two sectors that are no longer claimed to exist, and its
normalisation is wrong. Friedmann fixes $8\pi G$ through
$\rho_{\rm crit}=3H^2/8\pi G$, leaving no free convention, and the resulting
viscous pressure is $\Pi_0 = -\mathrm{KAL}_0\,\rho_{\rm crit}$, i.e.\
$\Pi_0/\rho_{\rm DE} \simeq -8$ where $w_0=-0.839950$ is required --- too large by a
factor $9.5$. The bulk-viscosity sector that does survive is the
scale-factor-dependent one of Paper~1, $\zeta(a)\propto(1-a)\rho_{\rm DE}/H$,
which delivers $w_a$ and is normalised to $\rho_{\rm DE}$ rather than to
$\rho_{\rm crit}$.
...
history. In SSEE that total is a single quantity, $\Omega_m=0.308881$,
and it is the same quantity that enters the background geometry at every
redshift. In particular, the saturation $1+w_0=0.160050$ never occupies a density
...
\end{equation}
This result is what survives the withdrawal of the partition: it never used
Eq.~(\ref{eq:withdrawn_subtraction}), and it is unchanged by it.

\paragraph{Falsifiability.}
The withdrawn two-sector reading predicted a crossover --- $\Omega_m\approx0.160$
for datasets confined to $z\lesssim2$, $\Omega_m\approx0.309$ for datasets
integrating to $z\gtrsim100$, with a smooth interpolation between them. That
prediction is withdrawn, and what replaces it is stronger because it has no
free joint: SSEE now predicts a \emph{single} matter density,
$\Omega_m=0.308881$, at every redshift and for every probe. A weak-lensing or
BAO analysis that required a background matter density significantly different
from $0.3089$ --- in either direction, at any redshift --- would falsify the
framework, with no partition available to absorb the discrepancy.
...
\midrule
$H_0$ [km/s/Mpc] & 67.962137 & SSEE anchor $3(\varphi+\pi)^2$ (Paper~9) \\
$\Omega_b h^2$ & 0.02237 & Planck 2018 prior \\
$\Omega_{m,\mathrm{CMB}}$ & 0.3089 & $\omega_m/h^2$ (forward) \\
$\sum m_\nu^{\rm active}$ [eV] & 0.068490 & SSEE algebraic (Paper~4) \\
$w_0$ & $-0.839950$ & $-T_r/M_v$ \\
$w_a$ & $-0.669975$ & $-P_{sc}/I_g$ \\
...
= 0.022418$, agrees with this prior at $0.3\sigma$ ($0.02237\pm0.00015$); the $0.2\%$
...
posterior $H_0=67.79$), $0.91\sigma$ from the Planck value
$1.04109\pm0.00030$ --- the earlier $6.66\sigma$ ``tension'' was an artefact of
the cold sector $1+w_0=0.160050$ misplaced in $E(z)$, removed once the total
...
SSEE naive ($\Omega_m=0.160050$) & 242 & 601 & 930 \\
...
anchor $H_0=67.96214$ ($\omega_m$-direct, $\Omega_{m,\rm CMB}=0.308881$),
...
All SSEE dark energy parameters ($w_0, w_a$), the spectral index $n_s$, and --- under the 2026 $\omega_m$-direct reframe --- the baryon and cold-matter densities $\omega_b=(\pi-\varphi)/(3\Omega^2)=0.022418$ and $\omega_c=\mathrm{KAL}_0\,\omega_b\,n_s=0.119514$ are algebraically fixed. The dimensional anchor $H_0/(\mathrm{km\,s^{-1}\,Mpc^{-1}}) = 3(\varphi+\pi)^2 = 67.96214$ is itself \emph{derived} from the SH0ES--$f_{\rm screen}$ inversion (Paper~9), not fitted to the CMB; the amplitude $A_s$ and reionization $\tau$ are borrowed inputs, not derived from $\{\varphi,\pi\}$, and are \emph{fitted} here (earlier versions of this work held them at their external Planck values while still counting them as free; fitting them is the self-consistent choice and is what we do below). The two quantities the framework borrows rather than derives are therefore $A_s$ and $\tau$, and we conservatively assign $k=2=\{A_s,\tau\}$. The standard $\Lambda$CDM baseline has $k=6$: $H_0$, $\Omega_b h^2$, $\Omega_c h^2$, $n_s$, $A_s$, and $\tau$ \citep{Planck2018}, all six free in the CMB fit. (The companion full MCMC of \S\ref{sec:b1_mcmc} additionally lets $H_0$ \emph{float} as a validation check: the chain recovers $H_0$ at the algebraic anchor, confirming that fixing it costs no fit quality and that the $k=2$ count is sound.)

Evaluating at the single global anchor $H_0/(\mathrm{km\,s^{-1}\,Mpc^{-1}}) = 3(\varphi+\pi)^2 = 67.96214$ (which is also the \texttt{plik\_lite}-preferred value, $\Omega_{m,\rm CMB}=\omega_m/h^2=0.308881$), the effective $\chi^2$ for SSEE is $1003.586$, compared to $1003.769$ for $\Lambda$CDM, with $\{A_s,\tau\}$ minimized in both. The two models therefore fit \emph{equally well}: $\Delta\chi^2 = -0.183$, which is statistically zero over the $N=669$ data points ($613$ \texttt{plik\_lite} TTTEEE bandpowers plus $28$ lowT and $28$ lowE, counted directly from the likelihood files).

\paragraph{Definitive Bayesian Information Criterion.}
Applying the BIC penalty for the differing degrees of freedom:
\begin{equation}
    \label{eq:bic_cobaya}
    \Delta\mathrm{BIC} = -0.183 + (2 - 6)\ln(669) = -26.21.
\end{equation}
A $\Delta\mathrm{BIC} < -10$ constitutes decisive evidence on the Jeffreys scale. Even under a hyper-conservative penalty where SSEE is assigned $k=4$ (additionally counting two algebraically-fixed inputs, $\omega_b$ and $H_0$, as if they were fitted), the result remains $\Delta\mathrm{BIC} = -0.183 + (4-6)\ln(669) = -13.19$, still strongly favouring the SSEE framework. This evaluation confirms that the geometric constraint of the SSEE background reproduces the CMB temperature and polarisation spectra without relying on diagonal approximations. (The earlier legacy minimization, which found $H_0=67.037$ with $\Delta\chi^2=+2.875$ and $\Delta\mathrm{BIC}=-32.2$, is superseded by this $\omega_m$-direct evaluation.)
...
The point-estimate scan of \S\ref{sec:cobaya} evaluates $\chi^2$ at the fixed algebraic anchor $H_0/(\mathrm{km\,s^{-1}\,Mpc^{-1}}) = 3(\varphi+\pi)^2$ while holding all remaining SSEE quantities algebraically fixed. To provide complete posterior distributions and a fully rigorous parameter-counting comparison, we perform a dedicated MCMC analysis using \texttt{Cobaya}'s built-in sampler against the full off-diagonal \texttt{plik} TTTEEE + lowl\,TT + lowl\,EE + lensing likelihood suite (evaluated through \texttt{clipy}).

\paragraph{Free parameters.}
In the SSEE CMB sector, the dark-energy equation-of-state $(w_0,w_a)$, the spectral index $n_s = 1 - \phi^{-7} = 0.965558$, the baryon density $\Omega_b h^2 = (\pi-\varphi)/(3\Omega^2) = 0.022418$, and the CDM density $\Omega_c h^2 = \mathrm{KAL}_0\,\Omega_b h^2\,n_s = 0.119514$ ($\omega_m$-direct) are all algebraically fixed, and the geometry anchor $H_0/(\mathrm{km\,s^{-1}\,Mpc^{-1}}) = 3(\varphi+\pi)^2 = 67.96214$ is derived (Paper~9). The only two model parameters genuinely sampled are therefore:
...
\midrule
$H_0$ [km\,s$^{-1}$\,Mpc$^{-1}$] & $67.96214$ (fixed)   & $67.35 \pm 0.55$ \\
...
\begin{figure}[H]
  ...
  green line marks the SSEE algebraic prediction $H_0^\mathrm{alg}=67.96214$\,km\,s$^{-1}$\,Mpc$^{-1}$.
...
\texttt{plik\_lite} point est. & TTTEEE+lowl     & 669  & 2 & 6 & $-26.21$ (SSEE decisively favoured) \\
...
($w_0=-0.839950$, $w_a=-0.669975$, $H_0=67.96214$ km\,s$^{-1}$\,Mpc$^{-1}$,
...
conservative CLASS ceiling $S_8=0.827$),
...
two-sector $\varphi$-DM mechanism ($S_{8,\mathrm{eff}}=0.758$), \emph{now retracted} (Paper~6): with $A_s$ free the single sector gives $S_8=0.7555\pm0.0192$ against raw KiDS-1000 $\xi_\pm$.
...
SSEE+IS (single-sector ceiling, CLASS, Paper~5) & \textbf{0.8149} & \textbf{0.827} \\
\hline
...
The withdrawn Paper~6 two-sector variant gave $0.93\sigma$ (retired 2026-08-01,
together with the $\phi$-DM particle): the free-streaming that lowered $\sigma_8$
...
$S_8$ lensing tension. That resolution is \emph{retracted} (Paper~6): the
$3.5\sigma$ ceiling was an artefact of fixing $A_s$ to Planck, and with $A_s$
free the single sector already gives $S_8=0.7555\pm0.0192$ against the 225 raw
KiDS-1000 $\xi_\pm$ points.
...
$H_0=67.96214$~km\,s$^{-1}$\,Mpc$^{-1}$) and verified the
...
Note that the raw SSEE \emph{dynamical} background ($\Omega_{m,\rm dyn}=0.160050$)
...
\begin{enumerate}
  ...
  \item \textbf{BAO consistency:} the BAO geometry is governed by the total
    matter density $\Omega_m=0.308881$, not by the saturation
    $1+w_0=0.160050$; SSEE must continue to fit DESI DR2 BAO within $2\sigma$
    ...
    from $-0.839950$, or $w_a$ from $-0.669975$, would falsify the
    algebraic derivation. \emph{(An earlier version of this item placed
    $\Omega_{m,\mathrm{dyn}}=0.160050$ in the geometry slot; that is a
    saturation, not a density, and the reading is withdrawn.)}

  \item \textbf{Cosmic-shear amplitude:} with the background fixed by the
    algebra and only the primordial amplitude free, the model predicts
    $S_8=0.7555\pm0.0192$ against the raw KiDS-1000 $\xi_\pm$ data (Paper~6).
    A shear measurement placing $S_8$ more than $3\sigma$ from this value
    would falsify the growth sector. \emph{(The former free-streaming
    falsifier at $k_{\rm fs}$ is withdrawn together with the $\phi$-DM
    particle; see Paper~6.)}
  ...
  \item \textbf{Growth index:} The algebraic prediction is $\gamma_\mathrm{alg}=\phi^{-1}=0.618$
    ...
    yielding a single-sector ceiling $S_8=0.827$ in the CMB observational sector (Table~\ref{tab:sigma8}).
...
\begin{itemize}
  ...
  \item \textbf{\texttt{plik\_lite} TTTEEE${}+{}$low-$\ell$ ($\omega_m$-direct):} $\chi^2_{\mathrm{eff}} = 1003.586$ with $\{A_s,\tau\}$ minimized, at $H_0 = 67.96214$ km\,s$^{-1}$\,Mpc$^{-1}$ (the CMB-minimising value with SSEE-fixed $\omega_b,\omega_c$); $\Lambda$CDM gives $1003.769$ with six free parameters.
  \item $\Delta\mathrm{BIC}=-26.21$ ($k=2$ vs $k=6$, $N=669$), decisively favouring SSEE over $\Lambda$CDM at equal fit quality ($\Delta\chi^2=-0.183$).
...
The SSEE cascade, run on the measured SH0ES rate, returns the IR-partial
$H_0^{\mathrm{glob,IR}}=68.13$~km\,s$^{-1}$\,Mpc$^{-1}$
(Paper~9 IR cascade, $H_0^{\rm SH0ES}(1-f_{\rm scr}^{\rm IR})$; the canonical
full-cascade value is the pure number
$3(\varphi+\pi)^2=67.96214$) and addresses the persistent Hubble tension
