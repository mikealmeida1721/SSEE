...
\begin{table}[ht]
\centering
\caption{$\chi^2_r$ by spectrum vs.\ Planck PR4 ($\ell=30$--$2000$).
...
but still small deviation ($+0.019$).
All three residuals are well within the expected scatter for a non-parametric model.
...
When TT, TE, and EE spectra are combined under the independence approximation
($N_{\rm total}=5905$, $\chi^2_{r,\mathrm{SSEE}}=1.052$,
$\chi^2_{r,\Lambda\mathrm{CDM}}=1.041$), the joint $\Delta\mathrm{BIC}\approx+14$.
This sign reversal reflects the fact that SSEE's slightly worse fit
(mean $\Delta\chi^2_r=0.011$) accumulates over the larger combined dataset.
However, TT, TE, and EE are not statistically independent — a rigorous
...
result; the $+14$ value should be treated as an upper bound on the penalty.
