...
consistent with the algebraic estimate $H_0 = M_v\times\Omega = 67.96$ km\,s$^{-1}$\,Mpc$^{-1}$
($M_v = \varphi+\pi+K_v \approx 14.279$, $\Omega=\pi+\varphi\approx4.760$)
from the framework's dimensional constants \citep{AlmeidaSSEE2026a}.
...
\paragraph{MIRA as a depth ratio.}
The factor MIRA$=1.9989\approx2$ can be interpreted as the ratio between
the comoving depth of the CMB observational sector and the comoving depth
of the BAO dynamic sector:
\begin{equation}
  \mathrm{MIRA} \;\approx\; \frac{\chi(z_*)}{\chi(z_{\rm BAO,eff})}
  \;\approx\; \frac{r_{\rm CMB}}{r_{\rm BAO}},
\end{equation}
where $\chi(z_*)$ is the comoving distance to last scattering and
$\chi(z_{\rm BAO,eff})$ is the effective depth of the BAO data
(weighted mean $z_{\rm eff}\approx0.5$--$1.0$ for DESI DR2).
In $\Lambda$CDM, $\chi(z_*)\approx14{,}000$~Mpc and
$\chi(z_{\rm eff})\approx2{,}000$--$7{,}000$~Mpc; the ratio varies
between $2$ and $7$ depending on the effective redshift.
The algebraic coincidence of MIRA$\approx2$ with the ratio at the
DESI effective redshift is a prediction of the framework, not a free choice.
