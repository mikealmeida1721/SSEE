...
In a BIC comparison ($k=0$ for SSEE vs $k=6$ for $\Lambda$CDM), TT alone gives
$\Delta\mathrm{BIC}=-6.9$ favouring SSEE; treating TT+TE+EE as independent
spectra gives an upper bound of $\Delta\mathrm{BIC}\approx+14$ (see \S\ref{sec:bic}).
In the lensing sector (9 MV bandpowers, $L=8$--$400$), SSEE achieves
...
\end{align}
The Hubble constant $H_0=66.66^{+0.47}_{-0.46}$ km\,s$^{-1}$\,Mpc$^{-1}$
...
of this total at $H_0=66.66$~km/s/Mpc, leaving the remainder as a structural
...
Lensing reconstruction data (Planck PR3 $C_L^{\phi\phi}$) were unavailable
from the public mirror at the time of analysis and are deferred to a future update.
...
Applied to TT alone, SSEE+MIRA is \textbf{favoured} owing to its zero free parameters.

\paragraph{On the $k=0$ assignment.}
A referee might argue that MIRA itself constitutes a free parameter, raising $k$ to $1$.
We reject this for two reasons.
First, MIRA$=(\varphi+\beta)/2$ is a \emph{derived} algebraic constant within the SSEE
framework \citep{AlmeidaSSEE2026genesis}, with its numerical value fixed before any CMB
datum was examined (Genesis~5.12 commit \texttt{f8728c3}, 2026 January 28;
\S\ref{sec:mira_timeline}).
Second, MIRA was not chosen to minimise $\chi^2$ or optimise any fit statistic;
its role as an Observational Frequency was defined in a cosmological context that
predates this analysis.
If, despite this, the reader assigns $k=1$, then
$\Delta\mathrm{BIC}=2093.6 - (2100.5 - 7.587) = -0.3$, which is statistically
inconclusive but does not reverse the conclusion.

\paragraph{Note on combined spectra.}
When TT, TE, EE, and PP are combined under the independence approximation
($N_{\rm total}=5914$, $\chi^2_{r,\mathrm{SSEE}}=1.051$,
$\chi^2_{r,\Lambda\mathrm{CDM}}=1.040$), the joint $\Delta\mathrm{BIC}\approx+13.7$.
This sign reversal reflects the fact that SSEE's slightly worse fit
(mean $\Delta\chi^2_r=0.011$ across TT/TE/EE) accumulates over the larger combined dataset.
The lensing sector (PP) alone favours SSEE ($\Delta\chi^2_r=-0.027$),
partially offsetting the temperature penalty.
However, TT, TE, EE, and PP are not statistically independent — a rigorous
joint test requires the full Planck CMB likelihood with its off-diagonal
covariance terms.
The $\Delta\mathrm{BIC}=-6.9$ from TT alone remains the cleanest single-spectrum
result; the $+13.7$ value should be treated as an upper bound on the combined penalty.
...
\begin{itemize}
  ...
  \item $\Delta\mathrm{BIC}=-6.9$ (TT alone) favouring SSEE+MIRA over $\Lambda$CDM.
