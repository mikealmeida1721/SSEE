...
\caption{Cross-domain predictions of the \SSEE\ two-axiom system.
  ...
  \textbf{M}~=~MCMC-validated posterior,
  \textbf{P}~=~numerical coincidence (dimensionless value matches physical units;
  no first-principles dimensional derivation; see Paper~1, App.~B footnote~$\dagger$);
  see Paper~1~\S\,Domain for full protocol.}
\label{tab:cross}
...
\midrule
\textbf{Cosmology} & & & & & \\
\quad $H_0^{\rm alg}$ [km/s/Mpc] & $3(\varphi+\pi)^2$ & 67.962 & $67.4\pm0.5$ & $1.1\sigma$ & P$^\dagger$ \\
\quad $H_0^{\rm MCMC}$ [km/s/Mpc] & emcee posterior & 66.75 & $66.75\pm0.44$ & $0.0\sigma$ & M \\
\quad $H_{0,\rm local}$ [km/s/Mpc] & $3(\varphi+\pi)^2 + K$ & 73.48  & $73.0\pm1.0$ & $0.5\sigma$ & P$^\dagger$ \\
...
\label{eq:mnu}
\end{equation}
Three of the four factors are determined by $\varphi$ and $\pi$; the
fourth introduces \emph{two} external observational anchors: the scale
$94.07\,\mathrm{eV}$ set by $T_{\rm CMB}$ and Fermi--Dirac neutrino
statistics, and the integer $\Delta\ell = 22$ — the measured offset
between the naive \SSEE\ first CMB acoustic peak ($\ell_1 = 242$,
derived from $\Omega_{m,\rm dyn}=0.160$) and the Planck-observed value
($\ell_1 = 220$).  The formula thus operates with two observational
inputs, not zero; it is \emph{self-consistent} within the \SSEE\
framework but not purely algebraic in the strict zero-parameter sense
used for $w_0$, $w_a$, and $H_0$.
