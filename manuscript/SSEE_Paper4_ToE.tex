...
verified by a convergence theorem — nine structurally distinct algebraic paths converge to the same numerical value (two degenerate pairs arise from SOLAR$=$MAR and
IGNIS$=$KRYSTOS\_V; see Appendix~\ref{app:independence}), which is consistent with, but does not by itself prove, the absence of post-hoc tuning;
...
\midrule
$P_1$      & $\mathrm{SOLAR}+\mathrm{PYROS}$
          & $(\varphi+2\pi)+(2\varphi+\pi)$
          & 14.278880 \\[2pt]
$P_2$   & $\mathrm{PHITA}+\mathrm{AURA}$
          & $\tfrac{3\varphi+5\pi}{2}+\tfrac{3\varphi+\pi}{2}$
          & 14.278880 \\[2pt]
$P_3$      & $\mathrm{IGNIS}+\Omega$
          & $2(\varphi+\pi)+(\varphi+\pi)$
          & 14.278880 \\[2pt]
$P_4$    & $\mathrm{MIKA}+\pi$
          & $(3\varphi+2\pi)+\pi$
          & 14.278880 \\[2pt]
$P_5$      & $\mathrm{BIAL}+\mathrm{KAL}+\mathrm{PYROS}$
          & $\tfrac{\varphi+\pi}{2}+\tfrac{\varphi+3\pi}{2}+(2\varphi+\pi)$
          & 14.278880 \\[2pt]
$P_6$ & $\mathrm{MAR}+\mathrm{PYROS}$
          & $(\varphi+2\pi)+(2\varphi+\pi)$
          & 14.278880 \\[2pt]
$P_7$   & $\mathrm{PYROS}+\Omega+\pi$
          & $(2\varphi+\pi)+(\varphi+\pi)+\pi$
          & 14.278880 \\[2pt]
$P_8$    & $\mathrm{MIKA}+\mathrm{KAL}-\mathrm{BIAL}$
          & $(3\varphi+2\pi)+\tfrac{\varphi+3\pi}{2}-\tfrac{\varphi+\pi}{2}$
          & 14.278880 \\[2pt]
$P_9$ & $\varphi+\pi+\mathrm{KRYSTOS\_V}$
...
KRYSTOS\_V & $2(\varphi+\pi)$                    & $2\Omega$      & 9.519253 \\
VITA  & $(\varphi+5\pi)/2$                      & $\pi+\mathrm{KAL}$ & 8.663001 \\
PHITA & $(3\varphi+5\pi)/2$                     & $\mathrm{VITA}+\varphi$ & 10.281034 \\
...
\begin{equation}
\boxed{H_0 = \Sigma_{\rm Sov} \times \Omega_{\rm DNAV} = 3(\varphi+\pi)^2 = 67.962}
\label{eq:H0}
\end{equation}
...
excess of $\Sigma_{\rm Sov}$ over the three-dimensional matter manifold (TRIAL).
...
\begin{align}
  ...
  K_v    &= 2(\varphi+\pi)             \approx  9.5192 \quad \text{(KRYSTOS\_V — Structural Constraint)}.
\end{align}
...
(no dimensional derivation; see Paper~1, App.~B). The deviation column uses Planck~2018 error bars.}
\label{tab:params}
\begin{tabular}{lllll}
\toprule
...
motivates the connection $H_0 = \Sigma_{\rm Sov} \times \Omega_{\rm DNAV}$.
...
\mathrm{IGNIS} = \pi+\mathrm{PYROS} &= 2(\varphi+\pi) = \mathrm{KRYSTOS\_V}
\end{align}
where SOLAR$=$MAR and IGNIS$=$KRYSTOS\_V hold algebraically.
Consequently:
\begin{itemize}
  \item $P_1$ ($\mathrm{SOLAR}+\mathrm{PYROS}$) and $P_6$
        ($\mathrm{MAR}+\mathrm{PYROS}$) expand identically.
  \item $P_3$ ($\mathrm{IGNIS}+\Omega$) and $P_9$ ($\varphi+\pi+\mathrm{KRYSTOS\_V}$)
        expand identically.
\end{itemize}
...
Although SOLAR and MAR expand identically ($\varphi+2\pi$), and IGNIS and KRYSTOS\_V
both equal $2(\varphi+\pi)$, the two elements in each pair occupy distinct nodes in
the named-constant hierarchy graph.  Retaining them documents nine conceptually
distinct traversal routes; the algebraic coincidences are a structural property
of the hierarchy, not a defect in its design.
