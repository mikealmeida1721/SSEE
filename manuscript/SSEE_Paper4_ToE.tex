...
$n_s = 1-\varphi^{-7} = 0.965558$ ($0.16\sigma$ from Planck~2018),
...
dark-energy parameters $w_0=-0.839950$, $w_a=-0.669975$ derived in the
...
The value
\begin{equation}
M_v \equiv \varphi+\pi+K_v = 3\,\Omega = 14.278879927\ldots
\label{eq:sigma9}
\end{equation}
...
$\Omega = \varphi+\pi$:
\begin{equation}
\boxed{H_0 = M_v \times \Omega = 3(\varphi+\pi)^2 = 67.962}
\label{eq:H0}
\end{equation}
...
correction is applied, and the two agree to $0.04\sigma$.
...
\boxed{\Omega_c h^2 = K_{\rm AL}\,\Omega_b h^2\,n_s
      = 5.521406\times 0.02242\times 0.96556 = 0.11951}
\label{eq:omch2_IS}
\end{equation}
...
\begin{equation}
\boxed{n_s = 1 - \varphi^{-7} = 1 - \frac{1}{\varphi^7} = 0.965558}
\label{eq:ns}
\end{equation}
...
Its value, $N_\ast = 58.068884$, lies within the standard window $[50, 65]$ for
...
matching $2\varphi^7 = 58.068884$ to within $O(1/N_\ast)\approx 0.002$ e-folds.
...
\begin{align}
  ...
  M_v    &= \varphi+\pi+K_v            \approx 14.2789 \quad \text{(Maximal Dimensional Invariant $= M_v$)}, \\
  P_{sc} &= 2\varphi+\pi               \approx  6.3777 \quad \text{(Dynamical Evolution Scalar)}, \\
  K_v    &= \varphi+\pi+\Omega         \approx  9.5193 \quad \text{(Structural Constraint)}.
\end{align}
...
\begin{equation}
  \boxed{w_0 = -\frac{3\varphi+\pi}{2(\varphi+\pi)} = -0.839950},
  \qquad
  \boxed{w_a = -\frac{2\varphi+\pi}{2(\varphi+\pi)} = -0.669975}.
  \label{eq:w0wa_p4}
\end{equation}
...
$\Omega_{m,\rm CMB}$ & 0.308881 & $0.3153\pm0.0073$   & $0.88\sigma$ & $\omega_m/h^2$ ($\omega_m$-direct$^\ddagger$) \\
...
(with $\Omega=\pi+\varphi=4.759627$, $\mathrm{KAL}=5.521406$, $\mathrm{TRIAL}=11.993542$),
...
of Paper~6 ($m_\phi=\Sigma m_\nu^{\rm active}\times(\mathrm{SOLAR}^2\cdot\mathrm{KRYSTOS}_V)$),
where the multiplier $\mathrm{SOLAR}^2\cdot\mathrm{KRYSTOS}_V=594.28$ is a
...
\begin{itemize}
  ...
  \item $n_s = 1-\varphi^{-7} = 0.965558$ ($0.16\sigma$ from Planck)
...
deviates from $M_v = 14.278879927019064$ by more than floating-point
