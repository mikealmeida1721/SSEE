...
verified by a Nine Sovereignties theorem — nine structurally distinct algebraic paths converge to the same numerical value (two degenerate pairs arise from SOLAR$=$MAR and
...
From Theorem~2, the Nine Sovereignties equal $\Sigma_{\rm Sov} = 3(\varphi+\pi)$.
...
\begin{itemize}
  ...
  \item MIKE ($\mathrm{IGNIS}+\Omega$) and MIKAEL\_V ($\varphi+\pi+\mathrm{KRYSTOS\_V}$)
        expand identically.
\end{itemize}
...
(Forja / chaotic force) and KRYSTOS\_V (Cristal / ordering force).
...
at the same magnitude, in balance at $\Sigma_{\rm Sov}$.
