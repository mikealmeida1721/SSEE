...
$H_{0,\rm local} = H_0^{\rm alg}/(1-f_{\rm scr}) = 72.86$~km\,s$^{-1}$\,Mpc$^{-1}$
($0.17\sigma$ from \citet{Riess2022}; see Section~\ref{sec:hubble_tension}).
\end{enumerate}
...
\quad $H_{0,\rm local}$ [km/s/Mpc] & $H_0^{\rm alg}/(1-f_{\rm scr})$ & 72.86$^{\rm P}$  & $73.0\pm1.0$ & $0.17\sigma$ \\
...
\bottomrule
\multicolumn{5}{l}{\footnotesize $^{\rm P}$~Numerical coincidence (Type~P): both $3(\varphi+\pi)^2$ and $K=(\varphi+3\pi)/2$}\\
\multicolumn{5}{l}{\footnotesize are dimensionless numbers whose sum matches $H_{0,\rm local}$ in km\,s$^{-1}$\,Mpc$^{-1}$;}\\
\multicolumn{5}{l}{\footnotesize no dimensional derivation from first principles exists. Consistent with Paper~1 policy.}\\
\end{tabular}
\end{table}
...
$H_{0,\rm local}$ [km/s/Mpc]  & 72.86  & $73.0\pm1.0$  & $0.17\sigma$ & algebraic (Type~P$^\dagger$) \\
...
yields $H_{0,\rm local} = H_0^{\rm alg}/(1-f_{\rm scr}) = 72.86$~km\,s$^{-1}$\,Mpc$^{-1}$
($0.17\sigma$ from \citet{Riess2022}). Paper~10 provides a UV-completion
self-consistency check, $H_0^{\rm UV} = 73.04$~km\,s$^{-1}$\,Mpc$^{-1}$,
conditional on the UV--IR separability postulate. The canonical local value
is the Paper~9 result $72.86$~km\,s$^{-1}$\,Mpc$^{-1}$.
...
The active neutrino mass sum follows from the acoustic residual operator
$\mathcal{R} = 4\,\mathrm{KAL} - 22 = 0.0856$ combined with the Israel--Stewart
relaxation time $\tau_\Pi H_0 = \mathrm{KAL}/(3\,\Omega_{\rm DE}) = 2.191$:
\begin{equation}
  \Sigma m_\nu^{\rm active}
    = \mathcal{R}\,\Omega_b h^2\,\frac{94.07~\mathrm{eV}}{\tau_\Pi H_0}
    = 0.0824~\mathrm{eV},
\end{equation}
...
\emph{Status.} This relation is classified \textbf{Type~P} (phenomenological),
not a zero-parameter algebraic result. The integer subtraction in
$\mathcal{R}=4\,\mathrm{KAL}-22$ is a nearest-integer (fractional-part) operation
on $4\,\mathrm{KAL}=22.086$ and has no independent $\varphi,\pi$ derivation; the
conversion factor $94.07$~eV is the standard $\Sigma m_\nu=94.07~\mathrm{eV}\times
\Omega_\nu h^2$ relation, a Standard-Model input rather than an SSEE quantity.
The value $\Sigma m_\nu^{\rm active}=0.0824$~eV is therefore a calibrated ansatz,
consistent with its Type-P classification in the Paper~1 predictive register.
...
\begin{itemize}
  ...
  \item $H_{0,\rm local} = H_0^{\rm alg}/(1-f_{\rm scr}) = 72.86$ ($0.17\sigma$ from Riess~2022)
