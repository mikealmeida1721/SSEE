...
($0.5\sigma$ from Riess~2022; numerical coincidence Type~P — no dimensional
...
Dynamical dark energy alternatives — quintessence \citep{Ratra1988,Copeland2006},
k-essence \citep{ArmendarizPicon2001}, and phantom models \citep{Caldwell2002} —
...
merely fit them \citep{Weinberg2013review}.
...
\textbf{Physical derivation from Sakharov conditions.}
The formula $\Omega_b h^2 = (\pi-\varphi)/H_0^{\rm SSEE}$ is not obtained by scanning;
its structure is dictated by the Sakharov conditions for baryogenesis
\citep{Sakharov1967}. The three conditions require that the baryon asymmetry
$\eta_B \propto \delta_{\rm CP}/\Gamma_{\rm exp}$, i.e., CP-violation divided by the
expansion rate at the electroweak transition. In the SSEE framework:
\begin{itemize}
...
\end{itemize}
The baryon-to-photon ratio $\eta_B \propto \delta_{\rm CP}/H_0$ identifies
$(\pi-\varphi)$ as the numerator and $H_0^{\rm SSEE} = 3(\varphi+\pi)^2$ as the
denominator, yielding $\Omega_b h^2 = (\pi-\varphi)/H_0^{\rm SSEE}$ as the natural
SSEE form. A scan of 96 algebraic candidates confirms this is the unique SSEE
expression within $1\sigma$ of Planck (see \texttt{ssee\_op1\_baryon\_density.py}).
The quantitative derivation from the sphaleron rate in the SSEE background is deferred
to Paper~B.
...
generated during inflation \citep{Mukhanov1992,Starobinsky1980,Guth1981,Linde1982}.
...
SSEE quintessential inflation model with $\alpha = \varphi^4/3$ and gravitational
reheating at $T_{\rm rh}\sim10^{-4}$~\text{GeV}.
\end{conjecture}
...
inflation followed by gravitational reheating (consistent with, but not proven by,
the framework). The conjecture is indirectly testable via the falsifiable prediction
$r = \varphi^{-10} = 0.00813$ (Sec.~\ref{sec:r}): a CMB-S4 or LiteBIRD detection
inconsistent with this value would refute Conjecture~B.1. Full derivation is
deferred to Paper~B (quintessential inflation and reheating in SSEE).
