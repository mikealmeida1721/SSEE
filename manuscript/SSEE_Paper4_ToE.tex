...
$\Omega_b h^2$ (BBN) & $3(\pi-\varphi)/2$      & —              & 2.285338 \\
...
The baryon-buffer constant $\mathcal{B} = 3(\pi-\varphi)/2$ measures the geometric
excess of $\Sigma_{\rm Sov}$ over the three-dimensional matter manifold.
...
If instead the \SSEE\ algebraic prediction $\Omega_b h^2 = 3(\pi-\varphi)/200 = 0.02285$
is substituted, the IS formula gives $K_{\rm AL}\times0.02285\times n_s = 0.12182$
($+1.5\sigma$), and the static formula gives $K_{\rm AL}\times0.02285 = 0.12617$
($+5.1\sigma$). The $-0.6\sigma$ result therefore relies on the Planck observed baryon
density, not the algebraic prediction; both inputs and tensions are disclosed here
for transparency.
...
The spectral tilt is phenomenologically modeled via the golden-ratio harmonic $n_s = 1 - \varphi^{-7}$. We motivate the exponent 7 by mapping it to the seven sequential combinations of $\{\varphi, \pi\}$ defined in Table~4, though we acknowledge this mapping is currently an algebraic ansatz rather than a derived consequence of an inflationary action.
...
\begin{equation}
  \eta_{\rm SSEE} = 2.74\times10^{-8}\times\frac{3(\pi-\varphi)}{200}
    = 2.74\times10^{-8}\times0.02285 = 6.26\times10^{-10}.
\end{equation}
...
The functional form $n_s = 1-\varphi^{-7}$ is a phenomenological ansatz, not a
derived consequence of an inflationary Lagrangian (see Section~\ref{sec:ns}).
The exponent~7 is motivated \emph{a priori} by the count of seven non-redundant
structural registers in the \SSEE\ hierarchy (Table~\ref{tab:hierarchy}),
providing a theoretical basis for the choice, but the connection to inflationary
dynamics remains an open problem.  Predictions independent of $n_s$ are
unaffected by this ansatz.
