...
\normalsize SSEE-V3.6 — Paper~4 (Peer-Review Corrected)}
...
where $\Sigma_{\rm Sov}=3\Omega$ is the maximal structural invariant of the dictionary, reached by twenty distinct additive routes under the no-self-sum construction law (Appendix~\ref{app:independence}); this multiplicity is consistent with, but does not by itself prove, the absence of post-hoc tuning;
...
$n_s = 1-\varphi^{-7} = 0.96556$ ($0.24\sigma$ from Planck~2018),
...
($0.17\sigma$ from \citet{Riess2022}), using the single global anchor
$H_0^{\rm alg}=3(\varphi+\pi)^2=67.962$ (2026 reframe); the earlier
CMB-mapped value $71.87$ ($1.12\sigma$, via $H_0^{\rm MIRA}=67.037$) is superseded;
...
dark-energy parameters $w_0=-0.840$, $w_a=-0.670$ derived in the
...
\subsection{The Maximal Structural Invariant \texorpdfstring{$\Sigma_{\rm Sov}$}{Sigma_Sov}}
\label{sec:sovereigns}

The value
\begin{equation}
\Sigma_{\rm Sov} \equiv 3(\varphi+\pi) = 3\,\Omega = 14.278879927\ldots
\label{eq:sigma9}
\end{equation}
...
A direct enumeration under this law shows that $\Sigma_{\rm Sov}$ is reached by
...
\end{equation}
each equal to $\Sigma_{\rm Sov}$ to 16 significant digits. This multiplicity is a
consequence of $\Sigma_{\rm Sov}$ being the maximal invariant of the two-dimensional
...
\quad $H_{0,\rm local}$ [km/s/Mpc] & $H_0^{\rm alg}/(1-f_{\rm scr})$ & 72.86  & $73.0\pm1.0$ & $0.17\sigma$ \\
\quad $\Omega_b$ & $\frac{100(\pi-\varphi)}{6(\varphi+\pi)^4}$ & 0.0495 & $0.0493\pm0.0004$ & $0.53\sigma$ \\
\quad $n_s$ (inflation) & $1-\varphi^{-7}$ & 0.96556 & $0.9649\pm0.0042$ & $0.24\sigma$ \\
\textbf{Hubble tension} & & & & \\
\quad Fractional correction & $K/H_0 = \frac{\varphi+3\pi}{6(\varphi+\pi)^2}$ & $8.12\%$ & $\approx 8\%$ & — \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^{\rm P}$~Canonical value (2026 reframe): $f_{\rm scr}$ applied to the single global anchor $H_0^{\rm alg}=3(\varphi+\pi)^2=67.962$ gives $72.86$ ($0.17\sigma$).}\\
\multicolumn{5}{l}{\footnotesize The earlier CMB-mapped register ($H_0^{\rm MIRA}=67.037\to71.87$, $1.12\sigma$) is superseded.}\\
...
From Section~\ref{sec:sovereigns}, the maximal structural invariant is $\Sigma_{\rm Sov} = 3(\varphi+\pi)$.
...
$H_0^{\rm MCMC}=67.95\pm0.40$~km\,s$^{-1}$\,Mpc$^{-1}$ (Paper~2, under
the $\omega_m$-direct CMB anchor as prior, $H_0^{\rm alg}=67.962$, Paper~3).
With the total gravitating matter density $\Omega_m=\omega_m/h^2=0.30889$ in the
background geometry, the BAO-corrected posterior coincides with the algebraic
anchor to $0.04\sigma$, and the \SSEE\ sound horizon matches \LCDM\
($r_d^{\rm SSEE}=148.2$~Mpc vs.\ $r_d^{\rm \Lambda CDM}=147.8$~Mpc): DESI~DR2
confirms the algebraic value. The $\sim$0.8~km\,s$^{-1}$\,Mpc$^{-1}$ downshift
reported in earlier drafts was an artefact of using the cold sector
$1+w_0=0.160$ as the background density, now corrected.
...
agreement with Planck~2018 ($\Omega_m = 0.3153\pm0.007$). There is \emph{no}
matter factor: $\Omega_{m,\rm dyn}=0.160$ (DESI) and $\omega_m$ (CMB) are two
independent SSEE predictions. (The earlier MIRA mapping
$\mathrm{MIRA}\times\Omega_{m,\rm dyn}=0.31993$ and the geometric route
$(\pi-\varphi)/(\pi+\varphi)=0.3201$ are both superseded.)
This is the
\emph{CMB-sector} (total matter) value, applicable to early-universe observables where $\phi$-DM
...
is an \emph{independent} algebraic identity ($=1+w_0$), not obtained from the
CMB-sector density by any factor (OP-8 dissolved).
...
\boxed{\Omega_c h^2 = K_{\rm AL}\,\Omega_b h^2\,n_s
      = 5.5214\times 0.02242\times 0.96556 = 0.11951}
\label{eq:omch2_IS}
\end{equation}
...
$H_{0,\rm local}$ [km/s/Mpc]  & 72.86  & $73.0\pm1.0$  & $0.17\sigma$ & $H_0^{\rm alg}$-screened (canonical) \\
$\Omega_b$          & 0.0495 & $0.0493\pm0.0004$    & $0.53\sigma$ & algebraic \\
$\Omega_b h^2$      & 0.02242& $0.02237\pm0.00015$ & $0.32\sigma$ & algebraic \\
$\Omega_{m,\rm CMB}$ & 0.30889 & $0.3153\pm0.0073$   & $0.88\sigma$ & $\omega_m/h^2$ ($\omega_m$-direct$^\ddagger$) \\
$\Omega_c h^2$ (IS) & 0.11951& $0.1200\pm0.0012$   & $0.41\sigma$ & algebraic (forward) \\
$Y_p$               & 0.2472 & $0.2471\pm0.0003$   & $0.2\sigma$ & BBN (no fit) \\
$n_s$               & 0.96556& $0.9649\pm0.0042$   & $0.24\sigma$& algebraic \\
...
\multicolumn{5}{l}{\footnotesize The dynamical sector $\Omega_{m,\rm dyn}=(\pi-\varphi)/[2(\varphi+\pi)]\approx 0.160$ governs late-time}\\
\multicolumn{5}{l}{\footnotesize BAO and cluster dynamics; see Paper~1, Sec.~1.4 (Two-$\Omega_m$ Criterion). $\Omega_{m,\rm dyn}$ and}\\
\multicolumn{5}{l}{\footnotesize $\omega_m$ are two independent SSEE predictions (the earlier MIRA mapping to $0.31993$ and}\\
...
is applied multiplicatively to the global background anchor
$H_0^{\rm alg}=3(\varphi+\pi)^2=67.962$~km\,s$^{-1}$\,Mpc$^{-1}$, which the
$\omega_m$-direct reframe (Paper~3) establishes as both the background $H$
and the CMB-preferred value, yielding the canonical local value
$H_{0,\rm local} = H_0^{\rm alg}/(1-f_{\rm scr}) = 72.86$~km\,s$^{-1}$\,Mpc$^{-1}$
($0.17\sigma$ from \citet{Riess2022}). (The earlier MIRA anchor
$H_0^{\rm MIRA}=67.037$ gave $71.87$ at $1.12\sigma$, now superseded.)
Paper~10 provides a UV-completion self-consistency check, the canonical
$H_0^{\rm UV} = 73.04$~km\,s$^{-1}$\,Mpc$^{-1}$ ($0.00\sigma$; the earlier
MIRA-anchored $72.05$ at $0.96\sigma$ is superseded),
...
of Paper~6 ($m_\phi=\Sigma m_\nu^{\rm active}\times(\mathrm{SOLAR}^2\cdot\mathrm{KRYSTOS})$),
where the multiplier $\mathrm{SOLAR}^2\cdot\mathrm{KRYSTOS}=594.28$ is a
...
\begin{itemize}
  ...
  \item $n_s = 1-\varphi^{-7} = 0.96556$ ($0.24\sigma$ from Planck)
  \item $\Omega_c h^2 = K_{\rm AL}\,\Omega_b h^2\,n_s = 0.11951$ ($0.41\sigma$ from Planck, forward)
  ...
  \item $H_{0,\rm local} = H_0^{\rm alg}/(1-f_{\rm scr}) = 72.86$ ($0.17\sigma$ from Riess~2022, via global anchor $67.962$; earlier $71.87$ at $1.12\sigma$ superseded)
  \item $r = \varphi^{-10} = 0.00813$ (falsifiable by LiteBIRD~2032)
\end{itemize}

The identification of $\Sigma_{\rm Sov}=3\Omega$ as the maximal structural
...
connection $H_0 = \Sigma_{\rm Sov} \times \Omega$.
...
% ===========================================================================
\appendix
\section{The Twenty Additive Routes to \texorpdfstring{$\Sigma_{\rm Sov}$}{Sigma_Sov}}
...
ceiling---the maximal invariant $\Sigma_{\rm Sov}=3(\varphi+\pi)$ is reached by
...
$\Sigma_{\rm Sov}$ being the closure ceiling of the dictionary, not evidence of
...
deviates from $\Sigma_{\rm Sov} = 14.278879927019064$ by more than floating-point
