...
The numerator $(\pi-\varphi)$ is the CP-asymmetry parameter of the baryonic sector:
it measures the geometric excess of the gauge-boson loop sector ($\pi$) over the
scalar-field sector ($\varphi$). The denominator $3\Omega^2 = H_0^{\rm SSEE}$ is the
cosmological Hubble scale; the ratio $(\pi-\varphi)/H_0$ expresses the baryon fraction
as CP-violation divided by expansion rate --- a structure consistent with the Sakharov
conditions for baryogenesis (B-violation via sphaleron at $T_{\rm EW}$; CP-violation
$\delta_{\rm CP} = (\pi-\varphi)/\Omega$; non-equilibrium from gravitational reheating
with $T_{\rm rh}\sim10^{-4}$~GeV). The derivation from the sphaleron rate in the SSEE
background is deferred to Paper~B.
...
Section~\ref{sec:r} below). The derivation of $N_\ast = 2\varphi^7$ from the
quintessential inflation potential $V(\varphi_{\rm inf})$ with gravitational reheating
is deferred to Paper~B.
