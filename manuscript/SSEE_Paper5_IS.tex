...
$\phi$-DM mass and the MIRA dynamical mechanism --- tracked as OP-8/9/11/14
...
For Q3, we compute the IS growth index $\gamma_{\mathrm{IS}} = 0.5503 \pm 0.0003$
(consistent with $\gamma_{\Lambda\mathrm{CDM}} \simeq 0.55$), the growth
factor $\mathcal{G} = D_1^{\mathrm{SSEE}}/D_1^{\Lambda\mathrm{CDM}} = 1.0109$
(growth essentially identical to $\Lambda$CDM once the correct cosmological
background $\Omega_{m,\mathrm{cosm}} = 0.320$ is used in the Poisson source;
...
Using the Planck-normalised amplitude, we find
$S_8^{\mathrm{SSEE}} = 0.8466 \pm 0.0063$, which is $1.0\sigma$ above Planck~2018,
$3.85\sigma$ above KiDS-1000 ($0.766\pm0.020$), and $3.90\sigma$ above
DES-Y3 ($0.776\pm0.017$).
The $f\sigma_8$ mean tension is $0.74\sigma$, essentially identical to $\Lambda$CDM
($0.73\sigma$): SSEE does not suppress or enhance structure formation relative to
$\Lambda$CDM once $\Omega_{m,\mathrm{cosm}}=0.320$ governs the growth equation.
...
$\Omega_{m,\mathrm{cosm}} = 0.3199$ is the gravitational background density
(MIRA-enhanced, equal to the CMB-inferred value), and primes
denote $d/d\ln a$.
...
The growth index is indistinguishable from $\Lambda$CDM; with $\Omega_{m,\mathrm{cosm}}=0.320$
...
\caption{Growth rate $f = d\ln D_1/d\ln a$ at selected redshifts,
  comparing SSEE (IS dynamics, $\Omega_{m,\mathrm{cosm}}=0.320$) with
...
With $\Omega_{m,\mathrm{cosm}} = 0.320$ in the Poisson source, the gravitational
clustering environment is the same as $\Lambda$CDM; differences arise only from
the DE equation-of-state history $w(z)$.
...
We find
\begin{equation}
  \mathcal{G} = 1.0109 \pm 0.005.
  \label{eq:G_value}
\end{equation}
SSEE grows \emph{marginally faster} than $\Lambda$CDM ($\sim$1\%).
...
\begin{equation}
  \sigma_8^{\mathrm{SSEE}} = 0.811 \times 1.0109 = 0.820 \pm 0.006.
...
$\sigma_8^{\mathrm{SSEE}} = 0.820$:
\begin{equation}
  \boxed{S_8^{\mathrm{SSEE}}
    = 0.820 \times \sqrt{\frac{0.3199}{0.3}}
    = 0.820 \times 1.033
    = 0.847 \pm 0.006.}
  \label{eq:S8_SSEE}
\end{equation}
This is $1.0\sigma$ above Planck~2018, $3.85\sigma$ above KiDS-1000, and
$3.90\sigma$ above DES-Y3 ($S_8 = 0.776\pm0.017$, 3$\times$2pt).
SSEE predicts slightly \emph{more} large-scale structure than $\Lambda$CDM
($S_8^{\Lambda\mathrm{CDM}} = 0.831$), placing it in tension with
weak-lensing surveys at the same level as $\Lambda$CDM or mildly worse.

\begin{table}[h!]
\centering
\caption{$S_8$ comparison: SSEE prediction versus observations.
  Tensions are quoted in units of $\sigma$.
  SSEE uses $\Omega_{m,\mathrm{CMB}} = 0.3199$ and $\sigma_8 = 0.820$.
...
\midrule
Planck 2018 & $0.832 \pm 0.013$ & 0.014 & 1.02 & $1.0\sigma$ \\
KiDS-1000   & $0.766 \pm 0.020$ & 0.021 & 3.86 & $3.9\sigma$ \\
DES-Y3      & $0.776 \pm 0.017$ & 0.018 & 3.92 & $3.9\sigma$ \\
\midrule
SSEE (this work) & $0.847 \pm 0.006$ & — & — & — \\
\bottomrule
\end{tabular}
\end{table}

The SSEE prediction $S_8 = 0.847$ is $1.0\sigma$ above Planck 2018 and in $3.9\sigma$
tension with both KiDS-1000 and DES-Y3.
This places SSEE in a similar position to $\Lambda$CDM ($S_8^{\Lambda\mathrm{CDM}}=0.831$,
tensions $3.3\sigma$/$3.3\sigma$), marginally worse.
The IS growth sector with the correct cosmological background $\Omega_{m,\mathrm{cosm}}=0.320$
...
($m_\phi = 36.95\,\mathrm{eV}$, $k_{\rm fs} = 0.659\,h/\mathrm{Mpc}$).
The two-sector effective transfer function suppresses power at $k > k_{\rm fs}$,
reducing $\sigma_8$ to $0.742$ and yielding $S_8^{\rm WDM} = 0.766$
...
The single-sector IS result of this paper ($S_8 = 0.847$) is the upper bound;
...
\begin{figure}[h!]
  ...
  \caption{$S_8$: from the single-sector challenge to the two-sector
    ...
    ($S_8 = 0.847 \pm 0.006$, red) sits $3.9\sigma$ above KiDS-1000 ---
    the honest baseline tension reported in Table~\ref{tab:S8}. The
    two-sector $\phi$-DM extension of Paper~6 (green), in which
    free-streaming below $k_{\rm fs} = 0.659\,h/\mathrm{Mpc}$
    ($m_\phi = 36.95$\,eV, forward prediction with no parameter fitted to
    lensing data) suppresses small-scale power, lands at $S_8 = 0.766$ ---
    $0.01\sigma$ from KiDS-1000 (green band). Grey points: Planck 2018,
...
redshifts, because with $\Omega_{m,\mathrm{cosm}}=0.320$ in the growth equation
...
\begin{figure}[h!]
  ...
  \caption{$f\sigma_8(z)$ for SSEE-V3.6 IS (solid blue) and $\Lambda$CDM
    ...
    The SSEE prediction ($\Omega_{m,\mathrm{cosm}}=0.320$ in growth equation)
    is essentially identical to $\Lambda$CDM at $z < 1$, with a mean tension
    of $0.74\sigma$ across six data points (vs.\ $0.73\sigma$ for $\Lambda$CDM).}
  \label{fig:fsigma8}
\end{figure}
...
\midrule
\multicolumn{5}{l}{Mean tension} & $0.74\sigma$ & $0.73\sigma$ \\
\bottomrule
\end{tabular}
\end{table}

The mean tension of $0.74\sigma$ across six surveys is indistinguishable
from the $\Lambda$CDM baseline ($0.73\sigma$).
SSEE and $\Lambda$CDM produce virtually identical $f\sigma_8(z)$ curves when
$\Omega_{m,\mathrm{cosm}}=0.320$ governs the Poisson source.
...
$\phi$-DM transfer function break at $k_{\rm fs} = 0.659\,h/\mathrm{Mpc}$
(Paper~6).
...
$S_8$ prediction & $0.847 \pm 0.006$ & $\sim 0.811$ & (unstable) \\
...
Paper~6 of this series provides the physical basis: the MIRA identity reflects a \emph{two-sector} dark matter structure in which a $\phi$-dark matter component with $\Omega_{\phi\text{-DM}}=(\mathrm{MIRA}-1)\,\Omega_{m,\mathrm{dyn}}=0.160$ and algebraically fixed mass $m_\phi=36.95\,\mathrm{eV}$ contributes at scales below the free-streaming cutoff $k_{\rm fs}=0.659\,h\,\mathrm{Mpc}^{-1}$. The near-equality $\Omega_{\phi\text{-DM}}\approx\Omega_{m,\mathrm{dyn}}$ produces $\mathrm{MIRA}\approx2$; the precise value $(3\varphi+\pi)/4$ traces back to the algebraic mass constraint $m_\phi=\Sigma m_\nu^{\rm active}\times(\Omega_{\rm DNAV}^4+\mathrm{AURA}\cdot\mathrm{KAL})$. This phenomenological resolution is subject to the Ly-$\alpha$ and free-streaming-scale ($k_{\rm fs}$, DESI~Y3/Euclid) falsification criteria described in Paper~6 --- $\phi$-DM has no Standard-Model portal, so its mass is probed only through the matter power spectrum, not neutrino-mass experiments; a first-principles derivation from quantum field theory remains an open problem.
...
With the correct cosmological background $\Omega_{m,\mathrm{cosm}} = 0.320$
...
\begin{itemize}
  \item \textbf{$S_8$ (weak-lensing amplitude):}
        SSEE predicts $S_8 = 0.847 \pm 0.006$, $1.0\sigma$ above Planck~2018 and
        $3.85\sigma$/$3.90\sigma$ above KiDS-1000/DES-Y3 — marginally worse than
        $\Lambda$CDM ($3.3\sigma$/$3.3\sigma$).
        The single-sector IS framework does not resolve the weak-lensing tension.

  \item \textbf{$f\sigma_8$ (RSD growth rate):}
        SSEE predicts $f\sigma_8(z=0.5) = 0.4785$, matching the
        BOSS DR12 value of $0.458 \pm 0.038$ at $0.5\sigma$.
        The mean tension across six RSD surveys is $0.74\sigma$, essentially
        identical to $\Lambda$CDM ($0.73\sigma$).
\end{itemize}

The two observables are consistent: once $\Omega_{m,\mathrm{cosm}}=0.320$
sets the gravitational background, SSEE differs from $\Lambda$CDM only
through the DE equation-of-state history $w(z)$ with $w_0=-0.840$, $w_a=-0.670$.
This drives a $\sim$1\% enhancement of $\mathcal{G}$ and $\sigma_8$, which
translates to marginally higher $S_8$.
...
reduces $\sigma_8$ from $0.820$ to $0.742$, recovering $S_8 \approx 0.766$
($<0.1\sigma$ of DES-Y3).
...
$S_8 = 0.847$ (single-sector IS) & $0.847 \pm 0.006$ & $S_8 > 0.86$: favours SSEE over $\Lambda$CDM & KiDS-1000, DES-Y3 / current \\
$\gamma_{\rm IS} = 0.5503$ & $0.5503 \pm 0.0003$ & $\gamma$ degenerate with $\Lambda$CDM at current precision & DESI full shape, Euclid / 2026--2028 \\
$f\sigma_8(z=0.5) = 0.4785$ & $0.4785 \pm 0.010$ & $0.5\sigma$ BOSS; mean $0.74\sigma$ (6 surveys) & DESI full-shape / 2026 \\
$D_1$ ratio $\mathcal{G} = 1.011$ & $1.011 \pm 0.005$ & $1\%$ enhancement; distinguishable at $2\%$ precision & Euclid weak lensing / 2027 \\
...
We conclude that the MIRA factor does not emerge from classical Israel-Stewart fluid mechanics. Paper~6 proposes that the physical basis is instead a two-sector dark matter structure (CDM + $\phi$-DM with $m_\phi=36.95\,\mathrm{eV}$), whose near-equal densities produce $\mathrm{MIRA}\approx2$ algebraically; a first-principles derivation from quantum field theory remains open.

\paragraph{Q3 — Growth rate and $S_8$ (SSEE tracks $\Lambda$CDM closely).}
The IS growth index $\gamma_{\mathrm{IS}} = 0.5503 \pm 0.0003$
is numerically indistinguishable from $\gamma_{\Lambda\mathrm{CDM}} \simeq 0.55$.
With $\Omega_{m,\mathrm{cosm}} = 0.320$ governing the Poisson source,
the growth factor ratio is $\mathcal{G} = 1.0109$, giving
$\sigma_8^{\mathrm{SSEE}} = 0.820 \pm 0.006$ and
$S_8^{\mathrm{SSEE}} = 0.847 \pm 0.006$.
This is $1.0\sigma$ above Planck~2018 and $3.9\sigma$ above KiDS-1000 and DES-Y3,
marginally worse than $\Lambda$CDM ($3.3\sigma$).
The mean $f\sigma_8$ tension across six RSD surveys is $0.74\sigma$,
...
(ii) predictive of a growth history essentially identical to $\Lambda$CDM
    once $\Omega_{m,\mathrm{cosm}}=0.320$ governs the growth equation (Q3),
    with $f\sigma_8$ tension of $0.74\sigma$ (vs.\ $0.73\sigma$ for $\Lambda$CDM), and
(iii) strictly bounded: the MIRA factor cannot be derived from IS dynamics (Q2); the physical basis is provided by Paper~6's two-sector $\phi$-dark matter construction ($\Omega_{\phi\text{-DM}}\approx\Omega_{m,\mathrm{dyn}}$ gives MIRA$\approx2$; exact value from the algebraic mass $m_\phi=36.95\,\mathrm{eV}$), with a first-principles QFT derivation remaining open.
The primary falsifiable prediction in the growth sector is
$S_8 = 0.847$ (single-sector IS upper bound), with the two-sector WDM
correction bringing it to $S_8 \approx 0.766$ (Paper~6).
