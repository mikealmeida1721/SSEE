...
sound speed, with no residual factors.\footnote{An earlier version of this paper
wrote $\ztilde\equiv\zeta/(\rho_{\mathrm{DE}}H_0)$ and correspondingly set
$\rho=\rho_{\mathrm{DE}}$ when solving the dispersion relation, dropping a
factor $(1+\wz)$ between the Euler equation and the result. With that reading
the same $\ztilde$ would give $c^2_{s,\mathrm{eff}}=+4.41$, i.e.\ superluminal.
No numerical result of this paper changes: what changes is the meaning of the
symbol $\ztilde$, and hence the physical $\zeta$, which is
$\rho_{\mathrm{DE}}(1+\wz)H_0\ztilde = 0.2946\,\rho_{\mathrm{DE}}H_0$ ---
a factor $1/(1+\wz)=6.25$ smaller than previously stated.}
