...
exactly marginally stable, and SSEE is a zero-free-parameter model
...
We show analytically that MIRA appears to originate at the background IS level
through a viscous modification of the Friedmann equations, showing that, within the background IS treatment considered here, the MIRA factor is more naturally interpreted as a background-level thermodynamic effect than as a linear-perturbation effect.
...
The IS growth sector has been fully explored; the $S_8$ weak-lensing tension
is characterised but not resolved at this stage (tension remains at $\geq 2\sigma$).
...
The CPL parameters~\eqref{eq:w0_wa} \citep{ChevallierPolarski2001,Linder2003} are consistent with DESI DR2 at $0.05\sigma$
...
\begin{equation}
  ...
  \label{eq:S8_SSEE}
\end{equation}
This is $7.7\sigma$ below Planck, $2.0\sigma$ below KiDS-1000, and
$2.8\sigma$ below DES-Y3 ($S_8 = 0.776\pm0.017$, 3$\times$2pt).
While the amplitude is lower than Planck, tension with DES/KiDS remains at a significant level.
...
The SSEE prediction $S_8 = 0.725$ remains in $2.0\sigma$ and $2.8\sigma$ tension with KiDS-1000 and DES-Y3 respectively.
The IS growth sector has been explored, but does not yet convincingly resolve the lensing tension.
...
($m_\phi = 5.71\,\mathrm{eV}$, $k_{\rm fs} = 0.493\,h/\mathrm{Mpc}$),
reducing the mean $f\sigma_8$ tension from $2.56\sigma$ to $0.50\sigma$.
Applying a CLASS WDM transfer function yields $S_8^{\rm WDM} = 0.761$
($0.09\sigma$ DES-Y3).
An additional HMcode-2020 baryonic feedback suppression ($B_{\rm eff} = 0.9447$)
further gives $S_8^{\rm bar} = 0.758$ ($0.06\sigma$ DES-Y3; Paper~6, eq.~12).
The single-sector IS result of this paper thus serves as the conservative upper bound
on $S_8$; the full two-sector plus baryonic treatment constitutes the nominal SSEE prediction.
...
has since been published, yielding $f\sigma_8$ measurements at
five effective redshifts in $z\in[0.3,1.3]$ that are broadly consistent
with $\Lambda$CDM predictions and systematically higher than the SSEE
values in Table~\ref{tab:fsigma8}; a dedicated comparison incorporating
SSEE transfer functions is deferred to a companion paper.
We report the tension transparently as a concrete falsifiable
...
\subsection{Physical origin of the MIRA factor}
...
Paper~6 of this series provides the physical basis: the MIRA identity reflects a \emph{two-sector} dark matter structure in which a $\phi$-dark matter component with $\Omega_{\phi\text{-DM}}=(\mathrm{MIRA}-1)\,\Omega_{m,\mathrm{dyn}}=0.160$ and algebraically fixed mass $m_\phi=5.71\,\mathrm{eV}$ contributes at scales below the free-streaming cutoff $k_{\rm fs}=0.493\,h\,\mathrm{Mpc}^{-1}$. The near-equality $\Omega_{\phi\text{-DM}}\approx\Omega_{m,\mathrm{dyn}}$ produces $\mathrm{MIRA}\approx2$; the precise value $(3\varphi+\pi)/4$ traces back to the algebraic mass constraint $m_\phi=\Sigma m_\nu^{\rm active}\times H_{0,\rm alg}$. This phenomenological resolution is subject to the Ly-$\alpha$ and KATRIN/PTOLEMY falsification criteria described in Paper~6; a first-principles derivation from quantum field theory remains an open problem.
...
We have performed a linear Israel-Stewart causal viscous perturbation analysis in the sub-horizon regime
...
This leaves SSEE in $2.0\sigma$ and $2.8\sigma$ tension with KiDS-1000 and DES-Y3 respectively.
The IS growth sector is explored at the linear sub-horizon level considered in this paper; the two-sector $\phi$-DM construction of Paper~6 provides the physical basis for MIRA.
The lowered amplitude arises from a structural consequence of IS dynamics with
$\Omega_{m,\mathrm{dyn}} = 1 + w_0 = 1 - T_r/M_v \simeq 0.160$.
...
(ii) predictive of an $S_8$ value within $2.0\sigma$ and $2.8\sigma$ of KiDS and DES respectively, exploring the growth tension without free parameter adjustment (Q3), and
(iii) strictly bounded: the MIRA factor cannot be derived from IS dynamics (Q2); the physical basis is provided by Paper~6's two-sector $\phi$-dark matter construction ($\Omega_{\phi\text{-DM}}\approx\Omega_{m,\mathrm{dyn}}$ gives MIRA$\approx2$; exact value from the algebraic mass $m_\phi=5.71\,\mathrm{eV}$), with a first-principles QFT derivation remaining open.
...
surveys. Like the $H(z)$ chronometer tension, this $f\sigma_8$ suppression is a direct physical consequence of the low geometric matter density ($\Omega_{m,\mathrm{dyn}}=0.160$). It functions as a diagnostic suggesting that a future theoretical extension, for example via kinetic braiding in an EFT framework, may be required to dynamically enhance the effective gravitational clustering force without altering the underlying geometric axioms.
This tension is reported transparently; a confirmed $>5\sigma$ discrepancy
from DESI Year-1 full-shape analysis would directly falsify the current unbraided SSEE-V3.6 formulation.
