...
number $s_m=0.160050$, or is there nothing to source (the former interpretation
...
$\Omega_{m,\mathrm{eff}}/\Omega_m = 0.9897\pm 0.0167 \to 1$, not~$\approx2$:
linear IS perturbations generate no such link --- and there is no second matter
sector for them to link to.
...
once stated this rule is itself \emph{withdrawn}, because $s_m=0.160050$ was
...
\begin{equation}
  ...
  \qquad \Omega_m = 0.308881,\ \ \Omega_{\mathrm{DE}} = 0.691119,
  \label{eq:Omeff}
\end{equation}
with the \emph{densities} in both slots. Until 2026-09-26 this equation carried
$\Omega_{m,\mathrm{dyn}}=0.160050$ and $\Omega_{\mathrm{DE}}=0.839950$, which are the
saturations $s_m=1+w_0$ and $s_{\rm DE}=|w_0|$ --- equation-of-state numbers in a
density slot, inside a Poisson source. The substitution does not move the
measured $r$, which is the output of the perturbation integration and never
touched these two constants; it moves only the normalisation of the ratio below.
and we write $\mathcal{R} \equiv \Omega_{m,\mathrm{eff}}/\Omega_{m,\mathrm{dyn}}$ for the
enhancement this clustering would produce.

\paragraph{What this section does and does not test (reframed 2026-09-26).}
Earlier versions posed $\mathcal{R}$ as a test of whether the perturbations
\emph{reproduce} the algebraic $\mathrm{MIRA}=1.9989$, under the mapping
$\Omega_{m,\mathrm{CMB}} = \mathrm{MIRA}\times\Omega_{m,\mathrm{dyn}} = 0.3199$ then in force.
That mapping was withdrawn on 2026-06-18 when OP-8 closed: there is no matter
factor in the model, $\Omega_{m,\mathrm{CMB}}=\omega_m/h^{2}=0.308881$ is derived, and
$\Omega_{m,\mathrm{dyn}}=1+w_0$ is a saturation rather than a density. So there is no
longer a factor of two for the perturbations to supply, and a
``$50\%$ discrepancy'' from a withdrawn target is not a result.
What the integration measures is unchanged and stands on its own: the level of
dark-energy clustering, reported below as a \emph{bound}. The figure
$r^*=0.4464$ is retained only as the scale that a two-fold enhancement would
have demanded, which is what makes the smallness of the measured $r$ meaningful:
\begin{equation}
  r^* = \frac{(\mathrm{MIRA}-1)\,\Omega_m}{\Omega_{\mathrm{DE}}}
  = \frac{0.998924 \times 0.308881}{0.691119}
  \simeq 0.4464.
  \label{eq:r_required}
\end{equation}
This corresponds to DE perturbations $\delta_{\mathrm{DE}} \approx 0.45\,\delta_m$
at $z=0$ — a $45\%$-level DE clustering signal, which would be plainly
...
\caption{Linear IS perturbation growth results from $z=19$ to $z=0$.
  ...
  The column ``IS regime'' marks the modes on which the bound is quoted,
  $k \geq 10\,H_0/c$ --- well inside both the IS-stabilised and the sub-horizon
  regimes. The three modes below it are listed for completeness and are
  \emph{excluded} from the bound: their $|r|$ is large and the quasi-static
  reduction used here does not hold there.}
\label{tab:MIRA}
\begin{tabular}{cccccc}
\toprule
$k\,[H_0/c]$ & $\delta_m(z=0)$ & $r = \delta_{\mathrm{DE}}/\delta_m$
  & $\Omega_{m,\mathrm{eff}}$ & $\mathcal{R}$ & IS regime \\
\midrule
2   & 0.2105 & $-41.05$   & $-28.0595$ & $-90.8425$ & no  \\
3   & 0.1537 & $-11.63$   & $-7.7321$  & $-25.0326$ & no  \\
5   & 0.1711 & $-0.88612$ & $-0.3035$  & $-0.9827$  & no  \\
10  & 0.2918 & $-0.01753$ & $0.2968$   & $0.9608$   & yes \\
20  & 0.5426 & $-0.00072$ & $0.3084$   & $0.9984$   & yes \\
50  & 1.2958 & $-0.00008$ & $0.3088$   & $0.9998$   & yes \\
100 & 2.5510 & $-0.00002$ & $0.3089$   & $1.0000$   & yes \\
\midrule
\multicolumn{4}{l}{Mean ($k \geq 10\,H_0/c$, IS-stabilised):}
  & $0.9897 \pm 0.0167$ & \\
\multicolumn{4}{l}{Would require ($r = r^* = 0.4464$):}
  & $1.9989$ & \\
\multicolumn{4}{l}{$\Rightarrow$ bound: $|r| \leq 0.0175$ for $k\geq10$, i.e.\ $3.9\%$ of $r^*$}
  &  & \\
\bottomrule
\end{tabular}
\end{table}

The measured enhancement is $\mathcal{R} = 0.9897 \pm 0.0167$ at $k \geq 10\,H_0/c$:
consistent with unity, i.e.\ \emph{no} enhancement. The content of that number is
a bound on dark-energy clustering. Inside the IS-stabilised sub-horizon regime
the ratio $r = \delta_{\mathrm{DE}}/\delta_m$ is small and \emph{negative},
$|r| \leq 0.0175$ at $k = 10\,H_0/c$ and falling as $k^{-2}$ to
$2\times10^{-5}$ by $k = 100\,H_0/c$ --- at most $3.9\%$ of the $r^*=0.4464$
that a two-fold enhancement would have needed. The restriction to
$k \geq 10\,H_0/c$ is deliberate: the three lowest modes of
Table~\ref{tab:MIRA} lie outside that regime and their $|r|$ is \emph{not}
small, so folding them into the bound would overstate it.
Dark energy in this model does not cluster at the scales where the IS
suppression operates, and a measurement of $\delta_{\rm DE}/\delta_m$ above the
percent level at $z=0$ and $k \gtrsim 10\,H_0/c$ would falsify it.
...
\caption{$\mathcal{R}(k) \equiv \Omega_{m,\mathrm{eff}}/\Omega_{m,\mathrm{dyn}}$
  (blue points with error bars) versus $k/(H_0/c)$; earlier versions of this
  text, and of the figure axis, wrote this quantity $\mathrm{MIRA}_\mathrm{num}$
  and divided by $\Omega_{m,\mathrm{dyn}}$ in the label while the code already
  divided by $\Omega_m$.
  The former target $\mathrm{MIRA} = (3\varphi+\pi)/4 \simeq 1.999$ (red dashed)
  is not approached: IS suppression drives $\mathcal{R} \to 1$
  for $k \gg H_0/c$.
  ...
  The conclusion stands: MIRA is \emph{not} reproduced by IS dynamics, at either
  level --- linear perturbations give $\mathcal{R}\approx1$ and the background
  IS Friedmann integration a ratio $\approx3.04$, neither of them $1.9989$.
  Both figures are recorded here as the \emph{historical} closing of a route
  that is no longer open: the target $1.9989$ was the matter factor
  $\Omega_{m,\mathrm{CMB}} = \mathrm{MIRA}\times\Omega_{m,\mathrm{dyn}}$, withdrawn on
  2026-06-18 when OP-8 closed. MIRA survives as the screening entity of
  Paper~9, where it is not a matter factor and nothing here bears on it.
  See Section~\ref{subsec:MIRA_background}.}
\label{fig:MIRA_test}
\end{figure}
...
This is $52\%$ higher than the algebraic MIRA factor ($1.9989$). We therefore establish that $\mathrm{MIRA}$ \emph{cannot} be derived from the background Israel--Stewart viscous pressure; the classical IS route is ruled out at both the linear-perturbation and background levels. \textbf{Historical note (2026-09-26).} The quantity being chased here, the matter factor $\Omega_{m,\mathrm{CMB}} = \mathrm{MIRA}\times\Omega_{m,\mathrm{dyn}}=0.3199$, was withdrawn on 2026-06-18 when OP-8 closed: $\Omega_{m,\mathrm{CMB}}=\omega_m/h^{2}=0.308881$ is derived with no factor, and $\Omega_{m,\mathrm{dyn}}=1+w_0$ is a saturation, not a density. The $3.04$ and the $52\%$ are therefore the record of a route closing, not a live tension --- there is no longer a factor of two to derive. The negative result is kept because it is what shut the route.
...
model --- not because $c^2_{s,\mathrm{eff}}=0$, which alone would make the
fluid cluster everywhere, but because the IS friction
$\mathcal{F}(k,a)=(1-3c_s^2)+\tilde z\,(k/aH)^2$ grows as $k^2$
(\S\ref{subsec:damping}) and drives $\delta_{\rm DE}/\delta_m\to0^-$.
On that smooth background the spherical-collapse threshold is
$\delta_c^{\mathrm{SSEE}} = 1.67634$ at $z_c=0$ and $1.68647$ at $z_c=10$,
against $\Lambda$CDM's $1.67599 \to 1.68646$: the two are the same to
$0.02\%$. The value $\delta_c^{\mathrm{SSEE}}=1.6284$ carried by earlier
...
($\sigma_8=0.81530$ for SSEE, $0.811$ for $\Lambda$CDM) and with the
...
deficit grows with mass and redshift, reaching $0.892$ at
$3\times10^{12}M_\odot$, $z=10$ and $0.778$ at the same mass at $z=15$, and it
is not an amplitude effect: \SSEE\ carries the \emph{larger} $\sigma_8$. It is
the growth history --- a smaller $\Omega_m$ ($0.308881$ against $0.3153$) and
the CPL background grow less between $z\sim10$ and today. At the masses JWST
actually probes ($M\sim10^{10.8}M_\odot$) the ratio is $0.99$: indistinguishable
...
$\mathcal{R} = 0.9897 \pm 0.0167$ versus the former target
...
Furthermore, numerical integration of the background IS Friedmann equations yields an effective ratio of $3.04$, failing to reproduce the matter factor at the background level --- a figure retained as the historical record of that route closing, since the factor it targeted was itself withdrawn in 2026-06 (OP-8).
