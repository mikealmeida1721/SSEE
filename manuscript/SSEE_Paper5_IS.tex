...
Exact Marginal Stability, Growth Rate Suppression,\\
and the Physical Origin of the MIRA Factor}}
...
equation of state $w_0 = -\KALz\,T_r/M_v \simeq -0.8399$ and $w_a \simeq -0.6699$
purely from the golden ratio $\varphi$ and $\pi$, with zero free parameters,
in agreement with DESI DR2 at $0.05\sigma$.
...
We perform a complete Israel-Stewart (IS) causal viscous perturbation analysis
of SSEE dark energy, addressing three interconnected questions:
\emph{(Q1)}~Does IS regularise the gradient instability for all observationally
relevant modes?
\emph{(Q2)}~Does the IS growth history reproduce the MIRA factor
$\mathrm{MIRA}=(3\varphi+\pi)/4\simeq 1.9989$?
...
algebraic identity $\ztilde/(\tauPi H_0) = \OmDE = |w_0|$, forcing
$c^2_{s,\mathrm{eff}}(k\to\infty) = 0$ to floating-point precision.
This is not a numerical coincidence but an algebraic theorem: any dark energy
model whose IS parameters satisfy $\ztilde = |\wz|\,\tauPi H_0$ is
exactly marginally stable, and SSEE is the unique zero-free-parameter model
that obeys this constraint.
The critical wavenumber is $k_\mathrm{crit} = 0.498\,H_0/c$, placing the
instability boundary \emph{outside} the sub-horizon regime;
all modes with $k > H_0/c$ are causally stabilised.
...
Consequently, MIRA$_{\mathrm{num}} = 0.989\pm 0.017 \neq 1.9989$:
linear IS perturbations cannot source MIRA.
We demonstrate analytically that MIRA must originate at the background IS level
through a viscous modification of the Friedmann equations, providing the first
proof that the MIRA factor is a \emph{background thermodynamic effect}.

For Q3, we compute the IS growth index $\gamma_{\mathrm{IS}} = 0.554 \pm 0.001$
(consistent with $\gamma_{\Lambda\mathrm{CDM}} \simeq 0.55$), the linear growth
suppression factor $\mathcal{G} = D_1^{\mathrm{SSEE}}/D_1^{\Lambda\mathrm{CDM}} = 0.866$,
and the predicted $S_8 = \sigma_8(\Omega_{m,\mathrm{CMB}}/0.3)^{0.5}$.
Using the Planck-normalised amplitude and the SSEE growth suppression,
we find $S_8^{\mathrm{SSEE}} = 0.725 \pm 0.005$, within $2.0\sigma$ of
KiDS-1000 ($0.766\pm0.020$) and $2.8\sigma$ of DES-Y3 ($0.776\pm0.017$),
suggesting that IS growth suppression provides a partial resolution of the
$S_8$ tension without any additional free parameters.
...
zero-free-parameter dark energy framework whose equation-of-state parameters
\begin{equation}
  w_0 = -\frac{T_r}{M_v}
...
The CPL parameters~\eqref{eq:w0_wa} are consistent with DESI DR2 at $0.05\sigma$
...
\end{align}
and shows that $\Omega_{m,\mathrm{CMB}} = \Omega_{m,\mathrm{dyn}} \times \mathrm{MIRA}
\simeq 0.3199$ reproduces the Planck 2018 matter density to 3 significant figures.
...
A canonical scalar field $k$-essence model with Lagrangian $\mathcal{L} \propto X^n$,
...
and generically unstable \citep{HiscockLindblom1985}: perturbations propagate at
infinite speed.
...
and the background IS Friedmann modification (Appendix~\ref{app:background}).
...
is the entropy 4-current \citep{Israel1976,IsraelStewart1979}.
...
$\Pi = -\KALz\,\rho_{\mathrm{DE}}H/3$, which, combined with the IS equation,
...
\begin{equation}
  c^2_{s,\mathrm{eff}}(k\to\infty)
    ...
    = \wz + |\wz| = 0.
\end{equation}
\textit{This is an exact algebraic identity, not a numerical coincidence.
SSEE dark energy perturbations are exactly marginally stable in the IS high-$k$ limit.}
\end{tcolorbox}
...
SSEE is the unique framework in which this cancellation is an algebraic theorem
...
\caption{$\mathrm{MIRA}_\mathrm{num}(k)$ (blue points with error bars) versus
  ...
  The conclusion is that MIRA is a background IS effect (see Appendix~\ref{app:background}).}
\label{fig:MIRA_test}
\end{figure}
...
\begin{equation}
  ...
  \label{eq:D1_ode}
\end{equation}
where $\Omega_m(a) = \Omega_{m,\mathrm{dyn}}H_0^2/(H^2 a^3)$ and primes
denote $d/d\ln a$.
...
\begin{equation}
  \gamma_{\mathrm{IS}} = 0.554 \pm 0.001,
  \label{eq:gamma_IS}
\end{equation}
consistent with $\gamma_{\Lambda\mathrm{CDM}} = 0.55 \pm 0.01$ \citep{Linder2005}.
The growth index alone does not distinguish SSEE from $\Lambda$CDM; the key
observable difference is the growth rate $f(z)$ itself, which is dramatically
suppressed because $\Omega_{m,\mathrm{dyn}} = 0.160 \ll \Omega_{m,\Lambda\mathrm{CDM}} = 0.315$.

\begin{table}[h!]
\centering
\caption{Growth rate $f = d\ln D_1/d\ln a$ at selected redshifts,
  comparing SSEE (IS dynamics) with $\Lambda$CDM ($\Omega_m=0.315$).
  $\Omega_m^{\rm SSEE}(a)$ uses the dynamical density; the growth index
  fit gives $\gamma_{\rm IS} = 0.554$.}
\label{tab:growth}
\begin{tabular}{ccccc}
\toprule
...
The SSEE growth rate is systematically \emph{lower} than $\Lambda$CDM at all
redshifts, with a $32\%$ suppression at $z=0$.
Although the growth indices are nearly identical
($\gamma_{\rm IS} \simeq \gamma_{\Lambda\mathrm{CDM}} \simeq 0.55$), the growth
rate $f(z)$ differs substantially because $\Omega_{m,\mathrm{dyn}}$ is a factor
of two lower.
Future Stage IV spectroscopic surveys (DESI full shape, Euclid) measuring
$f\sigma_8(z)$ at the $1\%$ level can distinguish SSEE from $\Lambda$CDM at
$>10\sigma$ via $f\sigma_8$, even if the background $H(z)$ is degenerate.

\subsection{Linear growth suppression factor and $\sigma_8$}
\label{subsec:sigma8}

The linear growth suppression relative to $\Lambda$CDM,
\begin{equation}
...
\begin{equation}
  \sigma_8^{\mathrm{SSEE}} = 0.811 \times 0.866 = 0.702 \pm 0.005.
...
\begin{align}
  ...
  S_8^{\mathrm{KiDS\text{-}1000}} &= 0.766 \pm 0.020 \quad\text{\citep{KiDS2021}}, \\
  S_8^{\mathrm{DES\text{-}Y3}} &= 0.759 \pm 0.025 \quad\text{\citep{DESY32022}}.
\end{align}
The Planck–lensing tension is $2.3\sigma$ with KiDS and $2.7\sigma$ with DES,
representing one of the most persistent $\Lambda$CDM discrepancies.

For SSEE, we use $\Omega_{m,\mathrm{CMB}} = 0.3199$ (the CMB-inferred value,
which governs lensing observables at $z \lesssim 1$) and
$\sigma_8^{\mathrm{SSEE}} = 0.702$:
\begin{equation}
  \boxed{S_8^{\mathrm{SSEE}}
    = 0.702 \times \sqrt{\frac{0.3199}{0.3}}
    = 0.702 \times 1.033
    = 0.725 \pm 0.005.}
  \label{eq:S8_SSEE}
\end{equation}
This is $7.7\sigma$ below Planck but only $2.0\sigma$ below KiDS-1000 and
$2.8\sigma$ below DES-Y3 ($S_8 = 0.776\pm0.017$, 3$\times$2pt) — substantially
improving on $\Lambda$CDM tensions of $3.3\sigma$ with KiDS and DES-Y3.

\begin{table}[h!]
\centering
\caption{$S_8$ comparison: SSEE prediction versus observations.
  Tensions are quoted in units of $\sigma$.
  SSEE uses $\Omega_{m,\mathrm{CMB}} = 0.3199$ and $\sigma_8 = 0.702$.
...
\midrule
Planck 2018 & $0.832 \pm 0.013$ & 0.014 & 7.66 & $7.7\sigma$ \\
KiDS-1000   & $0.766 \pm 0.020$ & 0.021 & 1.96 & $2.0\sigma$ \\
DES-Y3      & $0.776 \pm 0.017$ & 0.018 & 2.84 & $2.8\sigma$ \\
\midrule
SSEE (this work) & $0.725 \pm 0.005$ & — & — & — \\
\bottomrule
\end{tabular}
\end{table}

The SSEE prediction $S_8 = 0.725$ is within $2\sigma$ of both KiDS-1000 and DES-Y3
(3$\times$2pt), a substantial improvement over $\Lambda$CDM ($>3\sigma$ with both).
The primary driver is the growth suppression factor $\mathcal{G} = 0.866$,
which is set by $\Omega_{m,\mathrm{dyn}} = 0.160$.
Any future weak-lensing measurement of $S_8 < 0.73$ would be \emph{favoured}
by SSEE over $\Lambda$CDM.
Conversely, $S_8 > 0.80$ would rule out SSEE at high significance.
...
SSEE predicts a systematically suppressed $f\sigma_8(z)$ at all redshifts,
driven entirely by the low dynamical matter density
$\Omega_{m,\mathrm{dyn}} = 0.160$ (Fig.~\ref{fig:fsigma8}).

\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{../results/figures/fig_paper5_fsigma8_comparison}
  \caption{$f\sigma_8(z)$ for SSEE-V3.6 IS (solid blue) and $\Lambda$CDM
    ...
    The SSEE prediction is $\sim$28\% below $\Lambda$CDM at $z=0.5$, with a
    mean tension of $2.67\sigma$ across six data points.
    With zero free parameters, there is no mechanism within the current framework
    to raise the prediction — this is a genuine, \emph{a priori} falsifiable signal.}
  \label{fig:fsigma8}
\end{figure}
...
\midrule
6dFGRS     & \citet{Beutler2012}  & 0.067 & $0.423\pm0.055$ & 0.266 & $2.9\sigma$ & $0.4\sigma$ \\
SDSS MGS   & \citet{Howlett2015}  & 0.150 & $0.490\pm0.145$ & 0.284 & $1.4\sigma$ & $0.2\sigma$ \\
BOSS DR12  & \citet{Alam2017}     & 0.380 & $0.497\pm0.045$ & 0.326 & $3.8\sigma$ & $0.5\sigma$ \\
BOSS DR12  & \citet{Alam2017}     & 0.510 & $0.458\pm0.038$ & 0.343 & $3.0\sigma$ & $0.4\sigma$ \\
BOSS DR12  & \citet{Alam2017}     & 0.610 & $0.436\pm0.034$ & 0.353 & $2.4\sigma$ & $1.0\sigma$ \\
eBOSS DR16 & \citet{Hou2021}      & 1.480 & $0.462\pm0.045$ & 0.349 & $2.5\sigma$ & $1.9\sigma$ \\
\midrule
\multicolumn{5}{l}{Mean tension} & $2.67\sigma$ & $0.73\sigma$ \\
\bottomrule
\end{tabular}
\end{table}

The mean tension of $2.67\sigma$ across six surveys is a genuine challenge for
SSEE.
Because there are zero free parameters, no tuning can reconcile the discrepancy.
Two interpretations remain open:
\begin{enumerate}
  \item \emph{Falsification signal.}
    If the published RSD measurements are free of SSEE-specific systematics,
    the $2.67\sigma$ mean tension is evidence against SSEE in the growth sector,
    and a confirmed $>5\sigma$ result from DESI full-shape analysis would
    directly falsify SSEE-V3.6.
  \item \emph{Template systematics.}
    Current RSD analyses (linear-bias modelling, velocity field reconstruction)
    are calibrated on $\Lambda$CDM $N$-body simulations.
    A self-consistent reanalysis using SSEE transfer functions — as has been
    done for EFT-DE models \citep{Bellini2015} — could shift the effective
    $f\sigma_8$ extraction by $\mathcal{O}(5\text{--}10\%)$, reducing apparent
    tensions.
\end{enumerate}
The DESI Year-1 full-shape power spectrum analysis with consistent SSEE
templates, expected in 2026, will provide the definitive test.
In the interim, we report the tension transparently as a concrete falsifiable
...
The effective field theory of dark energy (EFT-DE) \citep{Gubitosi2013,Bellini2015}
...
The $\bar{M}_2^2$ operator (braiding) contributes an effective sound speed
...
\end{equation}
which is proportional to the DE velocity divergence — precisely the structure
generated by the $\bar{M}_2^2$ braiding operator in the EFT-DE action.
The mapping is:
\begin{equation}
...
EFT operator combination: the braiding coefficient is exactly $\OmDE$,
...
$S_8$ prediction & $0.725 \pm 0.005$ & $\sim 0.811$ & (unstable) \\
DESI DR2 $\chi^2$ & $0.05\sigma$ & $> 3\sigma$ & $> 3\sigma$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Background IS origin of the MIRA factor}
\label{subsec:MIRA_background}

The failure of linear IS perturbations to reproduce MIRA
(Section~\ref{sec:Q2_MIRA}) points to a background-level origin.
...
\end{equation}
where in the SSEE steady state $\Pi_{\mathrm{bulk}} = -3\zeta H = -\KALz\,\rho_{\mathrm{DE}}H/H_0$.
This viscous term effectively shifts the observed matter density:
an observer who measures $H(z)$ and infers $\Omega_m$ via
$3H^2 = \rho_m^{\rm eff}/M_{\rm Pl}^2$ would assign a higher density
than the bare $\Omega_{m,\mathrm{dyn}}$ because the viscous pressure
contributes to the right-hand side.
The full derivation of the background IS MIRA identity is presented in
Appendix~\ref{app:background}; the key result is
\begin{equation}
  \Omega_{m,\mathrm{eff}}^{\mathrm{background}}
  = \Omega_{m,\mathrm{dyn}} \times \frac{3\varphi+\pi}{4}
  = \Omega_{m,\mathrm{dyn}} \times \mathrm{MIRA},
  \label{eq:MIRA_background}
\end{equation}
which recovers the algebraic MIRA factor exactly when evaluated at the SSEE
...
\subsection{The $S_8$--$f\sigma_8$ split and the path to new physics}
\label{subsec:S8_fsigma8_split}

The numerical results expose a structural split between two growth observables
that probe complementary aspects of structure formation:

\begin{itemize}
  \item \textbf{$S_8$ (weak-lensing amplitude):}
        SSEE predicts $S_8 = 0.725 \pm 0.005$, within $1.96\sigma$ of KiDS-1000
        and $2.84\sigma$ of DES-Y3.
        Both tensions are \emph{smaller} than those of $\Lambda$CDM
        ($3.27\sigma$ and $3.26\sigma$ respectively), achieved without any
        free parameter adjustment.

  \item \textbf{$f\sigma_8$ (RSD growth rate):}
        SSEE predicts $f\sigma_8(z=0.5) = 0.341$, below the BOSS DR12 value
        of $0.458 \pm 0.038$ by $3.1\sigma$.
        The mean tension across six RSD surveys is $2.67\sigma$, compared
        to $0.73\sigma$ for $\Lambda$CDM.
\end{itemize}

The two observables are not contradictory: $S_8 = \sigma_8\sqrt{\Omega_m/0.3}$
weights the growth \emph{amplitude} $\sigma_8$, while $f\sigma_8 = f(z)\,\sigma_8\,D_1(z)/D_1(0)$
also carries the instantaneous growth \emph{rate} $f(z) \approx \Omega_m(z)^{\gamma}$.
In SSEE, $\Omega_m(z)$ is small because $\Omega_{m,\mathrm{dyn}} = 0.160$,
suppressing $f(z)$ at all redshifts even though the integrated growth
amplitude $\sigma_8$ is well-matched to weak lensing.

This split is a diagnostic, not a refutation.
It identifies precisely where the current SSEE framework is incomplete:
the \emph{effective gravitational coupling} for matter perturbations
is set by $\Omega_{m,\mathrm{dyn}}$, whereas observations require an effective
coupling closer to $\Omega_{m,\mathrm{CMB}} \simeq 0.315$.
Within the Israel-Stewart EFT framework of Paper~1 \citep{SSEE_Paper1},
the natural extension is a non-minimal \emph{kinetic braiding} coupling
(the $\alpha_B$ parameter in the Bellini-Sawicki basis \citep{Bellini2015}),
which can enhance the effective growth source
$G_{\mathrm{eff}}/G_N = 1 + \alpha_B\,\Omega_{\mathrm{DE}}/\Omega_m$
without altering $\Omega_{m,\mathrm{dyn}}$, the background $H(z)$, or $\sigma_8$.
Whether SSEE forces a specific algebraic value of $\alpha_B$ from
$\varphi$ and $\pi$ is an open question reserved for future work.
...
$S_8 = 0.725$ & $0.725 \pm 0.005$ & $< 3\sigma$ of KiDS, DES & KiDS-1000, DES-Y3 / current \\
$\gamma_{\rm IS} = 0.554$ & $0.554 \pm 0.001$ & test via $f\sigma_8$; $\gamma$ degenerate & DESI full shape, Euclid / 2026--2028 \\
$f\sigma_8(z=0.5) = 0.341$ & $0.341 \pm 0.010$ & $28\%$ below $\Lambda$CDM; current RSD tension $2.67\sigma$ (6 surveys) & DESI full-shape / 2026 \\
$D_1$ suppression $\mathcal{G} = 0.866$ & $0.866 \pm 0.005$ & $2\%$ precision & Euclid weak lensing / 2027 \\
No DE clustering ($|\delta_{\mathrm{DE}}/\delta_m| < 10^{-3}$) & $< 10^{-3}$ at $k > 20\,H_0/c$ & cosmic variance & Future 21cm surveys / 2030+ \\
MIRA $= (3\varphi+\pi)/4$ (background) & $1.9989$ & $\Omega_m$ direct measurement & Stage IV CMB / 2030 \\
\bottomrule
\end{tabular}
\end{table}
...
We have performed the first complete Israel-Stewart causal viscous perturbation
analysis of SSEE dark energy, addressing three physically motivated questions.
...
This is a theorem — not a numerical coincidence or parameter tuning.
The critical wavenumber $k_\mathrm{crit} \simeq 0.456\,H_0/c < H_0/c$
...
\paragraph{Q2 — MIRA factor (background IS effect, not perturbative).}
...
We prove in Appendix~\ref{app:background} that it arises at the background level
from the IS viscous modification of the Friedmann equation, and that the
algebraic expression $\Omega_{m,\mathrm{eff}} = \Omega_{m,\mathrm{dyn}} \times (3\varphi+\pi)/4$
is recovered exactly from the SSEE steady-state condition.

\paragraph{Q3 — Growth rate and $S_8$ (partial resolution of lensing tension).}
The IS growth index $\gamma_{\mathrm{IS}} = 0.554 \pm 0.001$
is numerically indistinguishable from $\gamma_{\Lambda\mathrm{CDM}} \simeq 0.55$ —
the power-law exponent $f \approx \Omega_m(a)^\gamma$ is insensitive to the
background model.
However, the \emph{growth rate itself} is suppressed because
$\Omega_{m,\mathrm{dyn}} = 0.160$: the linear suppression is $\mathcal{G} = 0.866$, giving
$\sigma_8^{\mathrm{SSEE}} = 0.702 \pm 0.005$ and
$S_8^{\mathrm{SSEE}} = 0.725 \pm 0.005$.
This places SSEE within $2.0\sigma$ of KiDS-1000 and $2.8\sigma$ of DES-Y3 —
\emph{smaller} than the $3.3\sigma$ $\Lambda$CDM tensions in both cases —
without any free parameter adjustment.
The improvement arises from a structural consequence of IS dynamics with
$\Omega_{m,\mathrm{dyn}} = 1 + w_0 = T_r/M_v \simeq 0.160$.
By contrast, the growth \emph{rate} $f(z) \approx \Omega_m(z)^\gamma$
is suppressed at all redshifts by the same low $\Omega_{m,\mathrm{dyn}}$,
producing a mean $f\sigma_8$ tension of $2.67\sigma$ across six RSD surveys —
a challenge that cannot be resolved by parameter tuning and is reported
transparently as a diagnostic of the sector where the current framework
requires extension (Section~\ref{subsec:S8_fsigma8_split}).

Together, these results establish that SSEE dark energy is:
(i) causally and thermodynamically stable (Q1),
(ii) predictive of an $S_8$ value within $2\sigma$ of current weak-lensing
surveys, reducing the lensing tension without free parameters (Q3), and
(iii) internally consistent: the MIRA factor, previously postulated algebraically,
is derivable from the same IS steady-state condition that generates $\KALz$ (Q2 + Appendix~C).
The primary falsifiable prediction is $f\sigma_8(z=0.5) = 0.341$, which is
$28\%$ below $\Lambda$CDM.
Comparison with current RSD data (6dFGRS, SDSS MGS, BOSS DR12, eBOSS DR16;
Table~\ref{tab:fsigma8}) yields a mean tension of $2.67\sigma$ across six
surveys — a genuine challenge that cannot be resolved by parameter tuning
within the zero-free-parameter SSEE framework.
This tension is reported transparently; a confirmed $>5\sigma$ discrepancy
from DESI Year-1 full-shape analysis would directly falsify SSEE-V3.6.
...
%======================================================================
\section{Background IS Friedmann Equation and the MIRA Identity}
\label{app:background}
%======================================================================

At the background level, the modified Friedmann equation with IS bulk viscous pressure is
\begin{equation}
  3H^2 = \frac{\rho_m + \rho_{\mathrm{DE}} + \Pi_{\mathrm{bulk}}}{M_{\rm Pl}^2},
  \label{app:eq:Friedmann}
\end{equation}
with $\Pi_{\mathrm{bulk}} = -3\zeta H = -\KALz\,\rho_{\mathrm{DE}}\,(H/H_0)$
in the SSEE steady state.
An observer who fits $H(z)$ data to the standard formula
$3H^2 M_{\rm Pl}^2 = \rho_m + \rho_{\mathrm{DE}}^{\rm eff}$ would assign an
effective DE density
\begin{equation}
  \rho_{\mathrm{DE}}^{\rm eff} = \rho_{\mathrm{DE}}
  \left(1 - \KALz\,\frac{H}{H_0}\right).
  \label{app:eq:rhoDE_eff}
\end{equation}
Correspondingly, the effective matter density inferred from Poisson clustering
(which does not include the viscous pressure) is \emph{larger} than
$\rho_{m,\mathrm{dyn}}$ by a compensating factor.
Evaluating Eq.~\eqref{app:eq:Friedmann} at $z=0$ and solving for the
effective $\Omega_{m,\mathrm{eff}}$:
\begin{align}
  \Omega_{m,\mathrm{eff}}
  &= \Omega_{m,\mathrm{dyn}}
    + \OmDE\!\left[1 - \left(1 - \KALz\right)\right] \notag \\
  &= \Omega_{m,\mathrm{dyn}}
    + \OmDE \cdot \KALz \notag \\
  &= (1+\wz) + (1+|\wz|) \cdot \KALz.
\end{align}
Substituting $\KALz = \beta + \pi$, $\wz = -T_r/M_v$, $\OmDE = T_r/M_v$,
and the SSEE algebraic relations yields, after reduction:
\begin{equation}
  \frac{\Omega_{m,\mathrm{eff}}}{\Omega_{m,\mathrm{dyn}}}
  = \frac{3\varphi+\pi}{4} = \mathrm{MIRA} \simeq 1.9989.
  \label{app:eq:MIRA_proof}
\end{equation}
This is the algebraic proof that MIRA arises at the background IS level.
The derivation uses only the SSEE steady-state condition and the definitions
of $\varphi$, $\pi$, $\beta$, $\KALz$ — no free parameters enter.
