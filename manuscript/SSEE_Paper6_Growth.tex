...
primordial amplitude free (as it is in both models), \SSEE{} yields
$S_8=0.7555\pm0.0192$, in agreement with the KiDS-1000 published value
$S_8=0.759\pm0.024$ at $0.11\sigma$. We are explicit about what this does not
...
Compared against the KiDS-1000 published value $S_8=0.759\pm0.024$, this is a
$0.11\sigma$ agreement.

\paragraph{What this does and does not claim.} It would be wrong to read this
as \SSEE{} having carried a $3.5\sigma$ tension and then reduced it to
$0.11\sigma$. \SSEE{} never carried that tension. At this level the model has
...
\begin{tabular}{lccc}
\toprule
$A_s$ determined by & $\log(10^{10}A_s)$ & $S_8$ & vs.\ KiDS-1000 \\
\midrule
...
\smallskip
\footnotesize
The last column compares each row against the \emph{published} KiDS-1000
value. The quantity that matters for the argument below is different: the
internal disagreement between rows~1 and~2, which are the same model with the
same background. In $\log(10^{10}A_s)$ that separation is
$3.042-2.863=0.179\pm0.053$, i.e.\ $\mathbf{3.4\sigma}$.
\normalsize
\end{center}
...
$\chi^2$ is quoted at the best-fit point over 225 data points.}
\label{tab:s8}
\begin{tabular}{lcccc}
\toprule
 & $\log A$ & $\sigma_8$ & $S_8$ & $\chi^2_{\rm min}$ (225 pts) \\
\midrule
$\Lambda$CDM (background free) & \TODO{logA} & \TODO{s8} & \TODO{S8} & \TODO{chi2} \\
\SSEE{} (background fixed) & $2.863\pm0.051$ & $0.7446\pm0.0189$ & $0.7555\pm0.0192$ & $265.4$ \\
\midrule
...
result, which is what licenses the \SSEE{} row above it. That run is in
...
summary statistics --- the model has no $S_8$ tension of its own: against the
225 raw KiDS-1000 $\xi_\pm$ points, with the background fixed by algebra and
only the primordial amplitude free, \SSEE{} gives $S_8=0.7555\pm0.0192$,
agreeing with the KiDS-1000 published value at $0.11\sigma$. The residual
$3.4\sigma$ between the amplitude preferred by the CMB and the amplitude
preferred by cosmic shear is a disagreement between those datasets, which this
work reports but does not claim to resolve. The
$f\sigma_8$ result is reported in \S\ref{sec:fsigma8}. \TODO{cerrar esta
frase con la conclusion conjunta cuando esten R1/R2 (fsigma8) y R4.}
