...
\title{SSEE Paper 6: The Growth Sector Re-examined --- No Second Component,\\
No $S_8$ Tension, on Raw Survey Data}
...
retracted here. Two independent findings drive the retraction. First,
...
measured density with a dimensionless equation-of-state number, and carried no
physical content despite being dimensionally well formed. Second, and
independently, evaluating \SSEE{} directly against the \emph{raw} KiDS-1000
$\xi_\pm$ correlation functions --- rather than against the compressed $S_8$
summary statistic, which is derived under a $\Lambda$CDM background --- shows
no tension to resolve: with the primordial amplitude free (as it is in both
models), $S_8=\TODO{S8 SSEE}$ against $S_8=\TODO{S8 SSEE err}$ measured
directly, compared to $S_8=\TODO{S8 LCDM}$ for $\Lambda$CDM fit to the same raw
data. The particle was introduced to explain a tension that only existed once
the public summary statistic, itself conditioned on $\Lambda$CDM, was
compared against \SSEE{}'s background. On the data it was meant to explain, it
is not required, and independently it is excluded by it: at the mass required
by the model's own algebraic lineage ($m_\phi=40.70$~eV), free-streaming
suppresses $\sigma_8$ well beyond what the raw shear data allow. We report
this as a completed, falsified line of reasoning rather than omit it, and
present the growth sector as it now stands: a single sector, $\Omega_m$ fixed
by the CMB-calibrated background with no partition, tested against raw KiDS-1000
cosmic shear and raw BOSS DR12 redshift-space multipoles.
\end{abstract}
...
Alcock--Paczynski effect for redshift-space clustering, and the non-linear
matter power spectrum template for lensing. Both official pipelines assumed
$\Lambda$CDM. Comparing a $\Lambda$CDM-conditioned number against a background
that is \emph{not} $\Lambda$CDM (\SSEE{} has $w_0=-0.840$, not $-1$) imports
part of the $\Lambda$CDM background into the comparison itself.
...
dynamical cold sector (equation of state)'' in the same breath as summing it
with $\Omega_{\phi\rm DM}$ to recover $\Omega_{m,\rm CMB}$ --- an internal
inconsistency once both statements are read together.
...
cosmic shear surveys under $\Lambda$CDM: parameters fitted to the CMB, carried
over unchanged to lensing, land outside the lensing-preferred region.
...
(negative control: $\chi^2=261.27$ vs.\ 260.32 official).
...
via MCMC (Cobaya, Metropolis sampler, 4 independent chains,
Gelman--Rubin $R-1<0.03$, Cobaya \citep{Cobaya}), not minimized, for both \SSEE{} (background fixed,
...
used at this stage of the analysis (see \S\ref{sec:limitation-kaiser}); a
one-loop/effective-theory model is the target for the production run.

% ============================================================================
\section{$S_8$: no tension on raw cosmic shear}
\label{sec:s8}
% ============================================================================
