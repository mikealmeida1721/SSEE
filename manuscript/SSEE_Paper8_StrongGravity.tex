...
\begin{abstract}
\noindent The physical interpretation of $m_\phi=36.95$~eV that this paper
requires is now settled: Paper~6 fixes $\phi$-dark matter as a free
massive cosmological scalar (mass term written into the action, with the
coefficient $m_\phi=\Sigma m_\nu^{\rm active}\,(\Omega_{\rm DNAV}^4+\AURA\cdot\KAL)
= 36.95$~eV fixed forward by the $(\varphi,\pi)$ algebra). This selects the
canonical EFT Bellini--Sawicki limit (b) below as the physical realisation,
so the canonical strong-lensing prediction is \emph{unconditional}, not
deferred.\footnote{What remains open in OP-9 is only the \emph{UV origin} of
the multiplier $\Omega_{\rm DNAV}^4+\AURA\cdot\KAL$ from first principles;
the present paper does not depend on that residual. Selecting limit~(b)
requires only that $m_\phi$ be a settled free-scalar dark-matter mass, which
Paper~6 supplies.} We retain the two-limit analysis (alternative MOND-like
\emph{vs.}\ canonical EFT) because each limit is rigorous and the contrast is
itself falsifiable, but limit~(b) is the canonical model of Papers~1, 6, 7,
and~10, and limit~(a) is kept as a derivation of pedagogical interest. The
predictions are checked against current observations (solar-system tests,
SLACS/BELLS lensing) and are consistent with them.
...
\begin{tcolorbox}[colback=green!4,colframe=green!50!black,title=Scope of falsifiability]
\textbf{The predictions enumerated below are pre-committed falsifiable
observables of the canonical model.} The interpretation of
$m_\phi=36.95$~eV is fixed by Paper~6 as the mass of a free cosmological
scalar (gravitationally produced $\phi$-dark matter, no Standard-Model
portal); this single interpretation selects the canonical EFT
Bellini--Sawicki limit~(b) and therefore a unique canonical lensing
prediction. The earlier ambiguity among a $V(\phi)$ condensate, a
phenomenological HDM particle, and a laboratory-accessible excitation is
resolved in favour of the free-scalar realisation. The only residual open
item (UV origin of the multiplier $\Omega_{\rm DNAV}^4+\AURA\cdot\KAL$,
tracked as OP-9) does \emph{not} enter the predictions below, which depend
only on $m_\phi$ being a settled free-scalar mass.
\end{tcolorbox}
