...
\begin{abstract}
\noindent The physical interpretation of $m_\phi=41.02$~eV that this paper
...
= 41.02$~eV fixed forward by the $(\varphi,\pi)$ algebra). This selects the
...
the multiplier $\mathrm{SOLAR}^2\cdot\mathrm{KRYSTOS}$ from first principles;
...
\Omega_{\phi\rm DM}=0.149$) and the Bellini--Sawicki structure of Paper~7
...
$k\lessgtr k_{\mathrm{fs}}=0.762\,h\,\mathrm{Mpc}^{-1}$, identifying
...
actual SSEE model with $\Omega_{\rm CDM}=0.160$, $\Omega_{\phi\rm DM}=0.149$ and EFT
...
amplitude} of Paper~9 ($f_{\rm screen}=\alpha_K/(3\,\MIRA)$). The
CMB-to-dynamical matter ratio
$\Omega_{m,\mathrm{CMB}}/\Omega_{m,\mathrm{dyn}}=0.30889/0.160\approx1.93$ is
\emph{numerically close} to $\MIRA$ but is \emph{not} identified with it: like
$\sqrt{\AURA}$, it lands near $2$ by near-coincidence, not by structural
identity (the two matter densities are independent algebraic predictions;
Paper~1, Two-$\Omega_m$ Criterion). In the lensing sector the quantity
...
two-sector limit (Paper~6, with $\Omega_{\rm CDM}=0.160$, $\Omega_{\phi\rm DM}=0.149$
...
On scales $k < k_{\mathrm{fs}} = 0.762\,h\,\mathrm{Mpc}^{-1}$, both the
CDM sector ($\Omega_{\mathrm{CDM}} = 0.160$) and the $\phi$-DM sector
($\Omega_{\phi\mathrm{DM}} = 0.149$) contribute, and the effective
lensing mass fraction is $\Omega_{m,\mathrm{eff}} = \Omega_{\mathrm{CDM}}+\Omega_{\phi\mathrm{DM}}
= 0.30889 = \Omega_{m,\mathrm{CMB}}$ --- the standard CMB-sector value
(within $2\%$ of the $\Lambda$CDM $\Omega_m=0.315$).
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    For $k < k_{\rm fs}=0.762\,h/\mathrm{Mpc}$ (blue dashed), the $\phi$-DM sector
    ...
    This is the pedagogical \emph{limit~(a)} (disformal scalar-force enhancement);
    the canonical \emph{limit~(b)} of the actual SSEE model recovers GR-with-DM
    lensing on all scales (Sec.~\ref{sec:double_protection}).
  }
...
\begin{itemize}
  \item Paper~9: the Hubble screening fraction $f_{\rm scr} = \alpha_K/(3\,\MIRA) = (\pi-\varphi)/\Omega^2$
...
the CMB matter sector: $\Omega_{m,\mathrm{CMB}}=\omega_m/h^2=0.30889$ with no
...
\begin{equation}
  f_{\mathrm{lens}}(k) \;=\;
  \begin{cases}
    1 & k < k_{\mathrm{fs}} = 0.762\,h\,\mathrm{Mpc}^{-1} \\
    \Omega_{\mathrm{CDM}}/\Omega_{m,\mathrm{CMB}} \approx 0.518 & k > k_{\mathrm{fs}}
  \end{cases}\,,
  \label{eq:flens}
\end{equation}
because on small scales only the CDM sector ($\Omega_{\mathrm{CDM}}=0.160$)
contributes, while the $\phi$-DM sector ($\Omega_{\phi\mathrm{DM}}=0.149$) is
free-streaming.
The ratio of CDM to total is $\Omega_{\mathrm{CDM}}/\Omega_{m,\mathrm{CMB}}=0.160/0.30889 = 0.518$,
close to $1/\MIRA=0.500$ by the same near-coincidence (\S\ref{sec:mira}).
...
\begin{enumerate}
  ...
  \item \textbf{Sub-Mpc scale ($k \gtrsim k_{\mathrm{fs}}$, compact lenses)}: The $\phi$-DM
        ...
        $\Lambda$CDM expectation for the same total $\Omega_m=0.30889$.
...
$m_\phi=41.02$~eV is fixed by Paper~6 as the mass of a free cosmological
...
item (UV origin of the multiplier $\mathrm{SOLAR}^2\cdot\mathrm{KRYSTOS}$,
...
\begin{enumerate}
  ...
  \item \textbf{Free-streaming cutoff} ($k_{\mathrm{fs}} = 0.762\,h\,\mathrm{Mpc}^{-1}$,
        ...
        $k = 0.762\,h\,\mathrm{Mpc}^{-1}$ would falsify the $\phi$-DM mass
        $m_\phi = 41.02\,\mathrm{eV}$.
  ...
  \item \textbf{Scalar mass $m_\phi = 41.02\,\mathrm{eV}$} (Paper~6):
        ...
        scale $k_{\rm fs}(m_\phi) = 0.762\,h/\mathrm{Mpc}$ imprinted on the
        ...
        (\mathrm{SOLAR}^2\cdot\mathrm{KRYSTOS})$ is falsified.
...
\begin{enumerate}
  ...
  \item \textbf{Two-sector signature}: SSEE predicts a scale-dependent
        lensing suppression near $k_{\mathrm{fs}} = 0.762\,h\,\mathrm{Mpc}^{-1}$,
