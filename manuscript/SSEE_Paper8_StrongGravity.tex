...
stated that the physical interpretation required here was settled, because
Paper~6 fixed a $\phi$-dark-matter mass $m_\phi$ forward from the
$(\varphi,\pi)$ algebra, and that this selected the canonical EFT
Bellini--Sawicki limit~(b) below, making the strong-lensing prediction
\emph{unconditional}. \textbf{Paper~6 has since retracted that particle}, on
two independent grounds: the subtraction defining its density mixed a measured
density with an equation-of-state number, and the $S_8$ tension it was built to
close does not exist in the raw data. The selection of limit~(b) therefore
loses its external justification, and the strong-lensing prediction of this
paper reverts to \emph{conditional} on that limit --- which is why the
two-limit analysis is retained in full below rather than collapsed to a single
branch. Nothing in the lensing calculation itself is affected; what changes is
that its premise is no longer supplied from outside.\footnote{
...
\textbf{Canonical limit (\S\ref{sec:vainshtein}, \S\ref{sec:double_protection}):}
the model carries real dark matter as a \emph{single} unpartitioned component,
$\Omega_m=0.308881$ (the two-sector split of the earlier Paper~6 is retracted),
and the Bellini--Sawicki structure of Paper~7
...
the EFT suppression. The scale-dependent two-sector discussion that followed (contributions on
either side of a free-streaming scale $k_{\mathrm{fs}}$) is likewise withdrawn
with the particle; the matter sector is single and unpartitioned. The
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    \emph{Retracted figure (retained for the record).} The scale dependence
    shown here was generated by a $\phi$-DM free-streaming scale that is
    retracted with the particle (Paper~6): below it the ratio approached
    $\sqrt{\bc}=\sqrt{\mathcal{A}}\approx1.9995$, above it lensing recovered the
    GR baseline. With a single unpartitioned matter sector there is no such
    transition.
...
\begin{tcolorbox}[colback=orange!5,colframe=orange!60!black,title=Scope of falsifiability --- revised 2026-08]
\textbf{The predictions enumerated below are pre-committed falsifiable
observables, but their premise is now conditional rather than supplied.}
An earlier version argued that Paper~6 had fixed the interpretation of a
$\phi$-dark-matter mass as a free cosmological scalar, and that this single
interpretation selected the canonical EFT Bellini--Sawicki limit~(b),
yielding a unique \emph{unconditional} lensing prediction. \textbf{Paper~6
has retracted that particle}, so the selection of limit~(b) is no longer
externally justified and the two-limit analysis below is retained in full.
