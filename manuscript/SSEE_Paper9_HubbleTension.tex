...
$H_0^{\rm MIRA}=67.068$\,km\,s$^{-1}$\,Mpc$^{-1}$ (Paper~3),
\[
  H_0^{\rm local} \;=\; \frac{H_0^{\rm MIRA}}{1-\fsc}
                  \;=\; \frac{67.068}{1-0.06725}
                  \;\approx\; 71.90 \text{ km/s/Mpc}\,,
\]
which lies $1.10\sigma$ from SH0ES (Riess et al.\ 2022) with \emph{zero free parameters}.
...
completion of Paper~10 gives a physical $H_0^{\rm UV} = 72.077$\,km/s/Mpc
($0.93\sigma$ from SH0ES), tightening the canonical residual from $1.10\sigma$
to $0.93\sigma$; its Type~P evaluation lands on $73.040$\,km/s/Mpc, a last-digit
...
SSEE-self-consistent CMB-derived value $H_0^{\rm MIRA}=67.068$~km/s/Mpc,
this predicts $H_0^{\rm local} = 71.90$ km/s/Mpc ($1.10\sigma$ from SH0ES).
...
\midrule
$H_0^{\rm MIRA}$ (canonical) & Paper~3 CMB+MIRA & $67.068\pm0.54$ km/s/Mpc & Paper~3 \\
$\Hzero^{\rm alg}$ (Type~P)  & $3\Omega^2$    & $67.96$ km/s/Mpc & Paper~1 \\
$H_0^{\rm local}$ (canonical, mult.) & $H_0^{\rm MIRA}/(1-\fsc)$  & $71.90$ km/s/Mpc & Eq.~\eqref{eq:H0local} \\
...
Tension vs SH0ES (canonical) & ---            & $1.10\sigma$   & --- \\
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    $H_0^{\rm MIRA}=67.068$) is $H_0^{\rm local}=71.90$ km/s/Mpc, $1.10\sigma$
...
$H_0^{\rm MIRA}=67.068$~km/s/Mpc (Paper~3, the canonical physical early-Universe
Hubble scale in this suite):
\begin{equation}
  H_0^{\rm local} \;=\; \frac{H_0^{\rm MIRA}}{1-\fsc}
  \;=\; \frac{67.068}{1 - (\pi-\phiG)/(\phiG+\pi)^2}
  \;\approx\; 71.90\;\text{km/s/Mpc}
...
\begin{equation}
  \Delta_{\rm mult} = 1.10\sigma\,, \qquad \Delta_{\rm add} = 1.40\sigma\,.
  \label{eq:tensions}
\end{equation}
...
prediction $H_0^{\rm local} = 71.90$\,km/s/Mpc at $\sim 1.26\sigma$ from this
anchor.
...
\begin{equation}
  ...
  = \frac{67.068}{1-0.06725} = 71.90\;\text{km/s/Mpc}\,,
  \label{eq:prediction}
\end{equation}
where $H_0^{\rm MIRA}=67.068$~km/s/Mpc is derived in Paper~3 from
Planck~\texttt{plik\_lite} under the SSEE background.
\textit{Current status}: $1.10\sigma$ from SH0ES \citep{Riess2022}.
...
$H_0$ (km/s/Mpc)       & $67.4$    & $71$--$72.5$ & $70$--$71.8$    & $\sim 72$ & $\mathbf{71.90}$ \\
Residual vs SH0ES      & $5\sigma$ & $1.6$--$2\sigma$ & $2$--$3\sigma$ & $\sim 1\sigma$ & $\mathbf{1.10\sigma}$ \\
...
\begin{enumerate}[label=(\roman*),itemsep=2pt]
  ...
  \item \textbf{Small residual tension, zero added parameters.}  The canonical
        (physical, MIRA-derived) SSEE prediction sits at $1.10\sigma$ from SH0ES,
...
$H_0^{\rm local} = 71.90$\,km/s/Mpc ($1.10\sigma$, physical $H_0^{\rm MIRA}$) is robust.
...
$H_0^{\rm UV} = 72.077$\,km/s/Mpc ($0.93\sigma$ from SH0ES), tightening the canonical IR
residual from $1.10\sigma$ to $0.93\sigma$.
...
$H_0^{\rm MIRA}=67.068$\,km/s/Mpc, this gives the canonical prediction
\begin{equation}
  H_0^{\rm local} = \frac{H_0^{\rm MIRA}}{1-(\pi-\phiG)/(\phiG+\pi)^2}
  \approx 71.90\;\text{km/s/Mpc}\,,
\end{equation}
which is $1.10\sigma$ from SH0ES \citep{Riess2022} with zero free parameters.
...
using the canonical (physical) value $H_0^{\rm local} = 71.90$\,km/s/Mpc, the dynamical-sector matter density
...
$H_0^{\rm MIRA}=67.068$~km/s/Mpc (Paper~3, fixed by $r_d$ and $\theta_*$) and
applying the derived screening, the UV completion of Paper~10 yields
$H_0^{\rm local}=72.077$~km/s/Mpc, $0.93\sigma$ from SH0ES. This is a
...
of the body ($0.93\sigma$) remains the canonical claim; this appendix shows only
