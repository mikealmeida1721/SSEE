...
\title{\textbf{SSEE-V3.6 and the Hubble Tension:\\
       An Algebraic Screening Fraction from First Principles}}
...
The Hubble tension \citep{Verde2019,Riess2022,Kamionkowski2023} is the
$4$--$6\sigma$ discrepancy between measurements of the present expansion
...
\begin{itemize}
  \item \textbf{Early-universe inference}: CMB anisotropies (Planck~PR4,
        ...
        $\Lambda$CDM.
  \item \textbf{Local distance-ladder}: Type~Ia supernovae calibrated with
        Cepheids in the SH0ES programme \citep{Riess2022} give
...
$\fsc \approx 0.0673$ from first principles, predicting
...
The fundamental action of SSEE-V3.6 is a $k$-essence scalar minimally
...
matter to the disformal metric
\begin{equation}
...
$H_0^{\rm local}$  & $\Hzero^{\rm alg}/(1-\fsc)$ & $72.86$ km/s/Mpc & Eq.~\eqref{eq:H0local} \\
...
The fractional contribution of scalar kinetic energy to the local
expansion rate at $z < 0.15$ is
\begin{equation}
  \frac{\delta H}{H}\bigg|_{\rm local}
  \;=\; \frac{\rho_{\rm kin}}{2\Mpl^2 H^2\,(3\MIRA)}
  \;=\; \frac{\alphaK}{2\cdot 2 \cdot 3\MIRA}
  \;=\; \frac{\alphaK}{12\MIRA}
  \;=\; \frac{\fsc}{2}\,.
  \label{eq:localcorr_1st}
\end{equation}
To first order, $H_{\rm local} = H_{\rm cosm}(1 + \fsc/2)$.
In the exact (multiplicative) treatment appropriate to the Vainshtein
transition \citep{Vainshtein1972,Babichev2013}:
\begin{equation}
  ...
  \label{eq:H0local}
\end{equation}
The disformal coupling factor $\MIRA$ in the denominator has a physical
interpretation: it is the same optical coupling that controls how dark
matter sources the scalar gradient (Paper~8).
A stronger optical coupling ($\MIRA$ larger) implies a weaker screening
fraction (larger denominator), consistent with Vainshtein suppression.
...
\begin{enumerate}
  ...
  \item \textbf{Growth factor $\sigma_8$}: not modified.
        ...
        The $\sigma_8^{\rm eff} = 0.737$ result of Paper~6 is unaffected.
\end{enumerate}

\paragraph{Note on the EDE comparison.}
Early dark energy (EDE) models resolve the Hubble tension by reducing $r_d$
...
5 (IS viscosity) & $\sigma_8 = 0.702$, $G = D_1^{\rm SSEE}/D_1^{\Lambda{\rm CDM}} = 0.866$ & No \\
...
\paragraph{Relation to other tension-resolution proposals.}
The present mechanism is distinct from EDE (modifies $r_d$ at $z > 1000$),
from modified recombination (shifts CMB peaks), and from late dark energy
transitions (require $w(z) \neq$ CPL).
The SSEE mechanism operates at $z < 0.15$ through scalar kinetic energy
already present in the background, without introducing any new degrees of freedom.
...
We identify three items that lie beyond the scope of this paper.

\paragraph{1.~Derivation of the multiplicative form.}
Equation~\eqref{eq:H0local} adopts the multiplicative (Vainshtein-type)
correction $H_0^{\rm local} = H_0^{\rm alg}/(1-\fsc)$ rather than the
additive form $H_0^{\rm alg}(1+\fsc)$ (which would give 72.53 km/s/Mpc,
$0.49\sigma$ from SH0ES).
The $\sim 0.3$\,km/s/Mpc difference between the two conventions is of
order $\fsc^2$ and requires a full perturbative expansion of the
Friedmann equation in the scalar kinetic sector to resolve.

\paragraph{2.~Dynamical derivation of the local--global split.}
Section~\ref{sec:mechanism} identifies the scalar kinetic energy as the
source of the correction; a complete derivation would compute the
perturbative correction to the distance-luminosity relation for a
SH0ES-type calibration and show that the result is exactly $\fsc \times H_0^{\rm alg}$.
This requires integrating the perturbed Hubble flow over $z \in [0, 0.15]$
in the SSEE background.

\paragraph{3.~UV completion of $M$.}
Papers~7 and~8 treat $M$ (the UV cutoff in $K(X) = X/\KAL + X^2/M^4$)
as a reference parameter set to $M \approx \Mpl$.
The algebraic identity derived here does not depend on the value of $M$
(it cancels in the ratio $\alphaK/3\MIRA$), so the Hubble correction is
robust under changes to $M$.
However, a complete UV completion deriving $M$ from $\phiG$ and $\pi$
would further constrain the Vainshtein radius $r_V$ and refine the
physical mechanism of Section~\ref{sec:mechanism}.
...
Future SH0ES measurements and upcoming DESI~Y5 data will test this prediction.
...
Kamionkowski, M.\ \& Riess, A.G.\ 2023, \textit{Annu.\ Rev.\ Nucl.\ Part.\ Sci.}, 73, 153.
