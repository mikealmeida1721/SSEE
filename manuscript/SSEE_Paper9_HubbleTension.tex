...
Applied as a local correction to the global background anchor
$H_0 = 3(\phiG+\pi)^2 = 67.962$\,km\,s$^{-1}$\,Mpc$^{-1}$ (Postulate~D, Paper~1),
\[
  H_0^{\rm local} \;=\; \frac{H_0}{1-\fsc}
                  \;=\; \frac{67.962}{1-0.06725}
                  \;\approx\; 72.86 \text{ km/s/Mpc}\,,
\]
which lies $0.17\sigma$ from SH0ES (Riess et al.\ 2022) with \emph{zero free parameters}.
This adopts the single global anchor $H_0=3(\phiG+\pi)^2$ shared by every sector
...
$H_0^{\rm alg}=3(\varphi+\pi)^2=67.962$ as the global background shared by all
...
The UV completion of Paper~10 gives $H_0^{\rm UV} = 73.040$\,km/s/Mpc
($0.00\sigma$ from SH0ES), the small-scale refinement of the same cascade.
\end{abstract}
...
\begin{equation}
  \Hzero^{\rm alg} \;=\; 3(\phiG+\pi)^2 \;\approx\; 67.962\;\text{km/s/Mpc}\,,
  \label{eq:H0alg}
\end{equation}
...
global background anchor $H_0 = 3(\phiG+\pi)^2 = 67.962$~km/s/Mpc,
this predicts $H_0^{\rm local} = 72.86$ km/s/Mpc ($0.17\sigma$ from SH0ES).
...
\begin{align}
  ...
  \sDE &= -w_0 = \frac{\AURA}{\phiG+\pi} = 0.839950\,. \label{eq:OmDE}
\end{align}
...
the SSEE action is $\alphaK = 3(1-\Omega_m)(5-3w_0) = 15.591335$ (Paper~7,
verified against \textsc{hi\_class}); it depends on $K(X)$ and it evolves.
The constant $0.403302$ is $\sK$, and the distinction is what makes
...
$\Omega_{m,\rm CMB}/\Omega_{m,\rm dyn}\approx1.93$, a numerical near-coincidence);
...
\begin{equation}
  \boxed{
    \sK = \frac{3\AURA(\pi-\phiG)}{2(\phiG+\pi)^2} = 0.403302458888\,.
  }
...
$\sDE$      & $\AURA/\Omega$         & $0.83994977$   & Eq.~\eqref{eq:OmDE} \\
$1+w_0$      & $(\pi-\phiG)/(2\Omega)$& $0.16005023$   & Eq.~\eqref{eq:Omega_minus_AURA} \\
$\sK$    & $3\AURA(\pi-\phiG)/(2\Omega^2)$ & $0.403302$ & Eq.~\eqref{eq:alphaK_algebraic} \\
$3\MIRA$     & $3\AURA/2$             & $5.9967$       & Eq.~\eqref{eq:3MIRA} \\
$\fsc = \sK/(3\MIRA)$ & $(\pi-\phiG)/\Omega^2$ & $0.067253$ & Eq.~\eqref{eq:fscreen_identity} \\
\midrule
$H_0 = \Hzero^{\rm alg}$ (global anchor) & $3\Omega^2$ & $67.962$ km/s/Mpc & Paper~1 (Post.~D) \\
$H_0^{\rm local}$ (canonical, mult.) & $H_0/(1-\fsc)$  & $72.86$ km/s/Mpc & Eq.~\eqref{eq:H0local} \\
$H_0^{\rm local}$ (canonical, add.)  & $H_0(1+\fsc)$   & $72.53$ km/s/Mpc & --- \\
$H_0^{\rm SH0ES}$            & ---            & $73.04\pm1.04$ km/s/Mpc & \citet{Riess2022} \\
$H_0^{\rm Freedman}$         & ---            & $69.96\pm1.54$ km/s/Mpc & \citet{Freedman2024} \\
Tension vs SH0ES (canonical) & ---            & $0.17\sigma$   & --- \\
Tension vs Freedman (canonical) & ---         & $1.88\sigma$   & --- \\
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    The SSEE canonical multiplicative IR prediction (applied to the global
    anchor $H_0=3(\phiG+\pi)^2=67.962$) is $H_0^{\rm local}=72.86$ km/s/Mpc,
    $0.17\sigma$ from SH0ES and $1.88\sigma$ from Freedman 2024 \citep{Freedman2024}.
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    with $w_0=-0.839950$, $w_a=-0.669975$.  At $z=0$, $f_{\rm screen}=0.06725$
    ...
    and the algebraic background value $H_0^{\rm alg}=67.962$ km/s/Mpc (dotted blue).
  }
  \label{fig:fscreen_z}
\end{figure}
...
Applied to the global background anchor
$H_0=3(\phiG+\pi)^2=67.962$~km/s/Mpc (Postulate~D, the single dimensionful
input shared by every sector of the suite):
\begin{equation}
  H_0^{\rm local} \;=\; \frac{H_0}{1-\fsc}
  \;=\; \frac{67.962}{1 - (\pi-\phiG)/(\phiG+\pi)^2}
  \;\approx\; 72.86\;\text{km/s/Mpc}
  \quad[\text{multiplicative}]\,,
  \label{eq:H0local}
\end{equation}
\begin{equation}
  H_0^{\rm local} \;=\; H_0\,(1+\fsc)
...
$H_0^{\rm alg}=3(\varphi+\pi)^2=67.962$ as the global background --- with no
matter factor ($\Omega_{m,\rm CMB}=\omega_m/h^2$; Paper~1) --- so the cascade is
anchored on $67.962$ and the MIRA-mapped value is retired as canonical.
...
prediction $H_0^{\rm local} = 72.86$\,km/s/Mpc at $\sim 1.88\sigma$ from this
anchor.
...
\begin{enumerate}
  ...
  \item \textbf{Growth factor $\sigma_8$}: not modified.
        ...
        The $\sigma_8^{\rm eff} = 0.747$ result of Paper~6 is unaffected.
\end{enumerate}
...
\midrule
1 (Framework) & $H_0^{\rm alg} = 3(\phiG+\pi)^2$ & No — $H_0^{\rm alg}=67.962$ is
               the global background anchor (Postulate~D); the superseded
               MIRA-mapped $67.037$ is retired \\
2 (MCMC DESI) & MCMC posterior $\Hzero = 67.79 \pm 0.35$ (prior $H_0^{\rm alg}$) & No —
...
6 (growth) & $S_8=0.7555\pm0.0192$ vs.\ raw KiDS-1000 & No \\
7 (EFT)       & $s_K = 0.403302$, $c_s^2 = 0.021284$ & Provides $s_K$
               (input to Paper~9) \\
...
\textbf{SSEE predicts:}
\begin{equation}
  H_0^{\rm local} = \frac{H_0}{1-(\pi-\phiG)/(\phiG+\pi)^2}
  = \frac{67.962}{1-0.06725} = 72.86\;\text{km/s/Mpc}\,,
  \label{eq:prediction}
\end{equation}
where $H_0=3(\phiG+\pi)^2=67.962$~km/s/Mpc is the global background anchor
(Postulate~D, Paper~1), shared with the CMB sector as the same global value---the
...
\begin{enumerate}[label=(\roman*),itemsep=2pt]
  ...
  \item \textbf{Sub-sigma residual, zero added parameters.}  The canonical
        SSEE prediction, anchored on the global background $H_0=3(\phiG+\pi)^2$,
        sits at $0.17\sigma$ from SH0ES --- better than every finalist of the
...
$H_0^{\rm local} = 72.86$\,km/s/Mpc ($0.17\sigma$, global anchor $H_0=3(\phiG+\pi)^2$) is robust.
...
$\sK^{\rm full} = 0.41691$ ($+3.37\%$ over the IR value) and the UV value
$H_0^{\rm UV} = 73.040$\,km/s/Mpc ($0.00\sigma$ from SH0ES), the small-scale
refinement of the same cascade that lands on the SH0ES central value.
...
Applied as a local correction to the global background anchor
$H_0=3(\phiG+\pi)^2=67.962$\,km/s/Mpc, this gives the canonical prediction
\begin{equation}
  H_0^{\rm local} = \frac{H_0}{1-(\pi-\phiG)/(\phiG+\pi)^2}
  \approx 72.86\;\text{km/s/Mpc}\,,
\end{equation}
...
the single global anchor under the 2026 reframe (Paper~1).
...
using the canonical value $H_0^{\rm local} = 72.86$\,km/s/Mpc, the dynamical-sector matter density
...
The body reports the canonical prediction anchored on the global background
$H_0=3(\phiG+\pi)^2=67.962$~km/s/Mpc: applying the derived screening gives
$H_0^{\rm local}=72.86$ (IR, $0.17\sigma$) and, with the UV completion of
Paper~10, $73.040$~km/s/Mpc ($0.00\sigma$ from SH0ES) --- reducing the
$\sim 5\sigma$ Planck--SH0ES discrepancy to below the percent level with zero
free parameters.

This appendix supplies the independent self-consistency check that motivates
adopting $67.962$ as the global anchor: it works the screening chain
...
\end{equation}
where $\Omega_{\rm DNAV}=\pi+\phiG=4.759627$ is the Stability Metric and
$\textsc{mikael}=3\Omega_{\rm DNAV}=14.278880$ coincides in value with the Maximal
Dimensional Invariant $M_v$ (one of the 25 Sovereign paths to $3\Omega$), generated in
the lineage from $\textsc{mika}+\pi$ with $\textsc{mika}=K_v+\phiG=11.1373$. The agreement with
the inverse-chain value~\eqref{eq:Hbase_value} is $-0.0001\%$.

\paragraph{Status.}
The background that SH0ES requires for exact closure ($67.962$) coincides, to
...
$67.962$ as the single global background anchor of the suite rather than the
