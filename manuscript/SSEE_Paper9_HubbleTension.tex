...
Tension (mult.)    & ---            & $0.17\sigma$   & --- \\
Tension (add.)     & ---            & $0.49\sigma$   & --- \\
\bottomrule
\end{tabular}
\end{table}
...
The multiplicative form~\eqref{eq:H0local} is suggestive of a
geometric correction to the distance-luminosity relation: if the
scalar kinetic energy contributes a fraction $\fsc$ to the local
Friedmann equation at $z < 0.15$ but is absent from the CMB-era
Friedmann equation (where $\Omde \sim 10^{-9}$), the ratio of the
two inferred expansion rates is $1/(1-\fsc)$ to leading order.
A complete perturbative derivation of this split --- computing the
correction to the apparent luminosity distance for SH0ES calibrators
in the SSEE background --- is deferred to future work (see
...
\paragraph{1.~Perturbative derivation of the local--global split.}
Section~\ref{sec:localH0} presents both multiplicative
($72.86$\,km/s/Mpc, $0.17\sigma$) and additive ($72.53$\,km/s/Mpc,
$0.49\sigma$) forms; the difference is of order $\fsc^2 \approx 0.33$\,km/s/Mpc.
Resolving which form applies requires computing the perturbative correction
to the apparent luminosity distance for SH0ES calibrators integrated over
$z \in [0, 0.15]$ in the SSEE background.
Both forms are consistent with SH0ES, so the ambiguity does not affect
the main result.
