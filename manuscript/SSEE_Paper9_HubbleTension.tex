...
Applied as a local correction to the SSEE-self-consistent CMB-derived value
$H_0^{\rm MIRA}=67.037$\,km\,s$^{-1}$\,Mpc$^{-1}$ (Paper~3),
\[
  H_0^{\rm local} \;=\; \frac{H_0^{\rm MIRA}}{1-\fsc}
                  \;=\; \frac{67.037}{1-0.06725}
                  \;\approx\; 71.87 \text{ km/s/Mpc}\,,
\]
which lies $1.12\sigma$ from SH0ES (Riess et al.\ 2022) with \emph{zero free parameters}.
We adopt the MIRA-derived value as the canonical IR prediction for methodological
consistency with the rest of the suite (Papers~2--3,~6). Inverting the screening
chain shows that SH0ES requires a background $H_0=67.96$\,km/s/Mpc, which coincides
with the Type~P algebraic value $3(\phiG+\pi)^2$; this is discussed, without being
adopted as a physical prediction, in Appendix~\ref{app:postulateP}.
...
As a separate self-consistency check (not an independent prediction), the UV
completion of Paper~10 gives a physical $H_0^{\rm UV} = 72.05$\,km/s/Mpc
($0.96\sigma$ from SH0ES), tightening the canonical residual from $1.12\sigma$
to $0.96\sigma$; its Type~P evaluation lands on $73.040$\,km/s/Mpc, a last-digit
rounding coincidence (conditional on Postulate~C.1), not a significant agreement.
\end{abstract}
...
$\fsc \approx 0.06725$ from the SSEE algebra. Applied to the
SSEE-self-consistent CMB-derived value $H_0^{\rm MIRA}=67.037$~km/s/Mpc,
this predicts $H_0^{\rm local} = 71.87$ km/s/Mpc ($1.12\sigma$ from SH0ES).
...
\midrule
$H_0^{\rm MIRA}$ (canonical) & Paper~3 CMB+MIRA & $67.037\pm0.54$ km/s/Mpc & Paper~3 \\
$\Hzero^{\rm alg}$ (Type~P)  & $3\Omega^2$    & $67.96$ km/s/Mpc & Paper~1 \\
$H_0^{\rm local}$ (canonical, mult.) & $H_0^{\rm MIRA}/(1-\fsc)$  & $71.87$ km/s/Mpc & Eq.~\eqref{eq:H0local} \\
$H_0^{\rm local,\,alg}$ (Type~P comparison) & $\Hzero^{\rm alg}/(1-\fsc)$ & $72.86$ km/s/Mpc & --- \\
$H_0^{\rm SH0ES}$            & ---            & $73.04\pm1.04$ km/s/Mpc & \citet{Riess2022} \\
$H_0^{\rm Freedman}$         & ---            & $69.96\pm1.54$ km/s/Mpc & \citet{Freedman2024} \\
Tension vs SH0ES (canonical) & ---            & $1.12\sigma$   & --- \\
Tension vs SH0ES (Type~P)    & ---            & $0.17\sigma$   & App.~\ref{app:postulateP} \\
Tension vs Freedman (canonical) & ---         & $1.26\sigma$   & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    The SSEE canonical multiplicative IR prediction (applied to
    $H_0^{\rm MIRA}=67.037$) is $H_0^{\rm local}=71.87$ km/s/Mpc, $1.12\sigma$
    from SH0ES and $1.26\sigma$ from Freedman 2024 \citep{Freedman2024}.
    The Type~P value $H_0^{\rm local,alg}=72.86$ obtained from
    $H_0^{\rm alg}=67.96$ ($0.17\sigma$ SH0ES; Appendix~\ref{app:postulateP})
...
Applied to the SSEE-self-consistent CMB-derived value
$H_0^{\rm MIRA}=67.037$~km/s/Mpc (Paper~3, the canonical physical early-Universe
Hubble scale in this suite):
\begin{equation}
  H_0^{\rm local} \;=\; \frac{H_0^{\rm MIRA}}{1-\fsc}
  \;=\; \frac{67.037}{1 - (\pi-\phiG)/(\phiG+\pi)^2}
  \;\approx\; 71.87\;\text{km/s/Mpc}
  \quad[\text{multiplicative}]\,,
  \label{eq:H0local}
\end{equation}
\begin{equation}
  H_0^{\rm local} \;=\; H_0^{\rm MIRA}\,(1+\fsc)
  \;\approx\; 71.55\;\text{km/s/Mpc}
  \quad[\text{additive}]\,.
  \label{eq:H0local_add}
\end{equation}

The difference between the two forms is of order $\fsc^2 \approx 0.32$ km/s/Mpc.
Tension against SH0ES \citep{Riess2022} ($73.04\pm1.04$):
\begin{equation}
  \Delta_{\rm mult} = 1.12\sigma\,, \qquad \Delta_{\rm add} = 1.44\sigma\,.
  \label{eq:tensions}
\end{equation}

\paragraph{Comparison with the Type-P algebraic value.}
The same mechanism applied to the Type~P algebraic value
$H_0^{\rm alg}=3(\phiG+\pi)^2=67.96$ sharpens the agreement further, but we
do not adopt it as a physical prediction; this comparison is treated
separately in Appendix~\ref{app:postulateP}, which inverts the screening
chain to identify the background value SH0ES requires. Throughout the body
we use $H_0^{\rm MIRA}$ as the canonical input.

\paragraph{Comparison with TRGB/JWST.}
The TRGB+JWST measurement of \citet{Freedman2024} yields
$H_0 = 69.96\pm1.54$\,km/s/Mpc, placing the canonical SSEE multiplicative
prediction $H_0^{\rm local} = 71.87$\,km/s/Mpc at $\sim 1.26\sigma$ from this
anchor.
...
\begin{enumerate}
  ...
  \item \textbf{Growth factor $\sigma_8$}: not modified.
        ...
        The $\sigma_8^{\rm eff} = 0.742$ result of Paper~6 is unaffected.
\end{enumerate}
...
\midrule
1 (Framework) & $H_0^{\rm alg} = 3(\phiG+\pi)^2$ & No — $H_0^{\rm alg}$ is a
               Type-P numerical coincidence, not the physical background
               (which is $H_{\rm MIRA}$) \\
2 (MCMC DESI) & MCMC posterior: $\Hzero \approx 66.53$ km/s/Mpc & No —
...
6 ($\phi$-DM) & $\sigma_8^{\rm eff}=0.742$, $k_{\rm fs}=0.659\,h$/Mpc & No \\
...
\begin{equation}
  H_0^{\rm local} = \frac{H_0^{\rm MIRA}}{1-(\pi-\phiG)/(\phiG+\pi)^2}
  = \frac{67.037}{1-0.06725} = 71.87\;\text{km/s/Mpc}\,,
  \label{eq:prediction}
\end{equation}
where $H_0^{\rm MIRA}=67.037$~km/s/Mpc is derived in Paper~3 from
Planck~\texttt{plik\_lite} under the SSEE background.
\textit{Current status}: $1.12\sigma$ from SH0ES \citep{Riess2022}.
\textit{Type-P comparison}: the algebraic value $H_0^{\rm alg}=67.96$ tightens
this further; see Appendix~\ref{app:postulateP}.
\end{quote}
...
$H_0$ (km/s/Mpc)       & $67.4$    & $71$--$72.5$ & $70$--$71.8$    & $\sim 72$ & $\mathbf{71.87}$ \\
Residual vs SH0ES      & $5\sigma$ & $1.6$--$2\sigma$ & $2$--$3\sigma$ & $\sim 1\sigma$ & $\mathbf{1.12\sigma}$ \\
...
\begin{enumerate}[label=(\roman*),itemsep=2pt]
  ...
  \item \textbf{Small residual tension, zero added parameters.}  The canonical
        (physical, MIRA-derived) SSEE prediction sits at $1.12\sigma$ from SH0ES,
        competitive with the best finalists of the $H_0$ Olympics, \emph{and}
        achieves this with zero added parameters.  The algebraic ($H_0^{\rm alg}$)
        evaluation tightens the agreement further (Appendix~\ref{app:postulateP}),
        but is not adopted as the headline.
\end{enumerate}
...
depend on $M$ (it cancels in the ratio), so the canonical prediction
$H_0^{\rm local} = 71.87$\,km/s/Mpc ($1.12\sigma$, physical $H_0^{\rm MIRA}$) is robust.
...
$\alphaK^{\rm full} = 0.41691$ ($+3.37\%$ over the IR value) and a physical UV value
$H_0^{\rm UV} = 72.05$\,km/s/Mpc ($0.96\sigma$ from SH0ES), tightening the canonical IR
residual from $1.12\sigma$ to $0.96\sigma$.
The corresponding Type~P evaluation ($H_0^{\rm alg}$) gives $73.040$\,km/s/Mpc, which
lands on the SH0ES central value; we flag this as a last-digit rounding coincidence
(conditional on Postulate~C.1, Paper~10), not a statistically significant agreement.
...
\begin{equation}
  \fsc = \frac{\alphaK}{3\,\MIRA} = \frac{\pi-\phiG}{(\phiG+\pi)^2} \approx 0.06725\,,
\end{equation}
...
Applied as a local correction to the physical (MIRA-derived) Hubble rate
$H_0^{\rm MIRA}=67.037$\,km/s/Mpc, this gives the canonical prediction
\begin{equation}
  H_0^{\rm local} = \frac{H_0^{\rm MIRA}}{1-(\pi-\phiG)/(\phiG+\pi)^2}
  \approx 71.87\;\text{km/s/Mpc}\,,
\end{equation}
which is $1.12\sigma$ from SH0ES \citep{Riess2022} with zero free parameters.
Evaluated instead on the algebraic background $H_0^{\rm alg}=3(\phiG+\pi)^2$ the
same expression yields $72.86$\,km/s/Mpc ($0.17\sigma$); we report this as a
Type~P numerical coincidence, not as the headline.
...
using the canonical (physical) value $H_0^{\rm local} = 71.87$\,km/s/Mpc, the dynamical-sector matter density
$\Omega_m = \Omega_{m,\rm dyn} = 0.160$ (Paper~1, Sec.~1.4, Two-$\Omega_m$
Criterion: late-time $E(z)$ in CPL form uses the dynamical value), and the CPL
...
\begin{equation}
  t_0^{\rm SSEE} = 15.52\;\text{Gyr}
  \qquad (\Lambda\text{CDM}: 13.80\;\text{Gyr})\,.
\end{equation}
The SSEE universe is $1.72$\,Gyr \emph{older} than $\Lambda$CDM, a direct
...
The body of this paper reports the \emph{canonical} prediction in the physical
direction: starting from the CMB-anchored background
$H_0^{\rm MIRA}=67.037$~km/s/Mpc (Paper~3, fixed by $r_d$ and $\theta_*$) and
applying the derived screening, the UV completion of Paper~10 yields
$H_0^{\rm local}=72.05$~km/s/Mpc, $0.96\sigma$ from SH0ES. This is a
\emph{partial} resolution of the tension --- reducing the $\sim 5\sigma$
Planck--SH0ES discrepancy to below $1\sigma$ with zero free parameters --- and
it is the result we stand behind physically.

This appendix records a separate, weaker observation, presented in the
\emph{inverse} direction so that no algebraic value is assumed in advance.
...
exactly?} With $\fsc^{\rm UV}=0.069522$ from the $X^2/M^4$ correction of
Paper~10,
\begin{equation}
  H_0^{\rm base} \;=\; 73.040 \times (1 - 0.069522)
                 \;=\; 67.962\;\text{km/s/Mpc}\,.
...
Only now do we ask whether $67.962$~km/s/Mpc has structure in the closed
...
\end{equation}
where $\Omega_{\rm DNAV}=\pi+\phiG=4.7596$ is the Stability Metric and
$\textsc{mikael}=3\Omega_{\rm DNAV}=14.2788$ is the Maximal Dimensional Invariant
$\Sigma_9$ (Paper~4, one of the Nine Sovereignties), generated in the lineage
from $\textsc{mika}+\pi$ with $\textsc{mika}=K_v+\phiG=11.1373$. The agreement with
the inverse-chain value~\eqref{eq:Hbase_value} is $-0.0001\%$.

\paragraph{Status (Postulate P).}
We classify equation~\eqref{eq:double_identity} as a Type~P coincidence: the
algebraic value was \emph{recognised a posteriori}, not fitted, and it is
conditional on Postulate~C.1 (Paper~10). The physical, MIRA-anchored prediction
of the body ($0.96\sigma$) remains the canonical claim; this appendix shows only
that the background SH0ES \emph{would} require for exact closure coincides, to
five figures, with an entity already present in the SSEE dictionary.
