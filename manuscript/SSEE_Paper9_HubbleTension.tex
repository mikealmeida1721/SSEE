...
screening mechanism resolves the SH0ES tension, the full picture of
local $H_0$ measurements remains internally inconsistent at the $\sim 2\sigma$
level --- a known observational puzzle that is independent of the SSEE framework.
We report both comparisons in Table~\ref{tab:constants}.
...
$< 0.05$\,km/s/Mpc, confirming that equation~\eqref{eq:H0local} is the canonical prediction.
...
\paragraph{Multiplicative vs.\ additive form (resolved in Appendix~\ref{app:hubble_integral}).}
Section~\ref{sec:localH0} presents both forms; the ambiguity is now resolved.
Appendix~\ref{app:hubble_integral} shows that the disformal path compression
of Paper~8 (equation~\eqref{eq:chi_compress}) yields
$H_0^{\rm local} = H_0^{\rm alg}/(1-\fsc)$ \emph{exactly} at leading order in $z$.
The additive form is a $(1-\fsc)^{-1}\approx 1+\fsc$ linearisation carrying
a $\fsc^2 \approx 0.33$\,km/s/Mpc second-order error.
The variation of $\fsc(z)$ over $z\in[0,0.15]$ shifts $H_0^{\rm local}$ by
less than $0.05$\,km/s/Mpc (Appendix, Step~4), negligible at the SH0ES precision.
...
dark energy equation of state $(w_0, w_a) = (-0.840, -0.670)$:
\begin{equation}
...
This is more accommodating of the oldest globular clusters
($t_{\rm GC} \gtrsim 12.5$\,Gyr, e.g.\ \citealt{Valcin2021})
...
\end{equation}
This is the multiplicative form, derived from first principles.
