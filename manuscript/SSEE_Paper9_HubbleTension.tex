...
i.e.\ $0.17\sigma$, with \emph{zero free parameters}. The comparison is a
...
$3(\varphi+\pi)^2=67.96214$ as the algebraic target shared by all
...
\begin{equation}
  \Hzero^{\rm alg} \;=\; 3(\phiG+\pi)^2 \;\approx\; 67.96214\;\text{km/s/Mpc}\,,
  \label{eq:H0alg}
\end{equation}
...
background rate $H_0^{\rm glob} = 68.13$ km/s/Mpc, to be compared with the
algebraic number $3(\phiG+\pi)^2 = 67.96214$ ($0.17\sigma$).
...
$H_0^{\rm glob}$ (canonical, mult.) & $H_0^{\rm SH0ES}(1-\fsc)$ & $68.13$ km/s/Mpc & Eq.~\eqref{eq:H0local} \\
$H_0^{\rm glob}$ (canonical, add.)  & $H_0^{\rm SH0ES}/(1+\fsc)$ & $68.44$ km/s/Mpc & --- \\
$3\Omega^2$ (pure number, target) & $3(\phiG+\pi)^2$ & $67.96214$ & Paper~1 (Post.~D) \\
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    The SSEE canonical multiplicative IR cascade takes the measured local
    rate as input and returns $H_0^{\rm glob}=68.13$ km/s/Mpc from SH0ES
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    and the dimensionless algebraic target $3(\phiG+\pi)^2=67.96214$ (dotted blue).
  }
  \label{fig:fscreen_z}
\end{figure}
...
\begin{itemize}
  \item The CMB infers $H_0$ via the angular scale $\theta_* = r_d/D_A(z_*)$
        ...
        The inferred value is $H_0^{\rm glob} = 3(\phiG+\pi)^2$.
...
$3(\varphi+\pi)^2=67.96214$ as the algebraic target --- with no
...
cascade at $\sim 1.88\sigma$: run on the Freedman datum the lens returns
$H_0^{\rm glob} = 69.96\times(1-\fsc) = 65.26$\,km/s/Mpc, low by $2.70$
against $3(\phiG+\pi)^2$.
...
2 (MCMC DESI) & MCMC posterior $\Hzero = 67.79 \pm 0.35$ (prior $H_0^{\rm glob}$) & No —
...
$H_0^{\rm glob} = 68.13$\,km/s/Mpc ($0.17\sigma$ from $3(\phiG+\pi)^2$) is robust.
...
Applied to the measured local rate, the lens gives the canonical result
\begin{equation}
  H_0^{\rm glob} = H_0^{\rm SH0ES}\left[1-\frac{\pi-\phiG}{(\phiG+\pi)^2}\right]
...
using the measured local rate $H_0^{\rm SH0ES} = 73.04$\,km/s/Mpc, the dynamical-sector matter density
...
The body reports the canonical cascade run on the measured local rate
$H_0^{\rm SH0ES}=73.04\pm1.04$~km/s/Mpc: applying the derived screening gives
$H_0^{\rm glob}=68.13$ (IR, $0.17\sigma$) and, with the UV completion of
Paper~10, $67.96214$~km/s/Mpc ($4.3\times10^{-6}\sigma$) --- reducing the
...
adopting $67.96214$ as the algebraic target: it works the screening chain
...
$67.96214$ as the single algebraic target of the suite rather than the
