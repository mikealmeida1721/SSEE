...
superseded, as the matter factor $\pi/\varphi$ pairs the CMB sector with the single
anchor $67.96$ along the geometric degeneracy (Paper~1).
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    % TODO Fase B figura: regenerar fig_paper9_h0_tension.pdf con ssee_paper9_figures.py (cascada 72.86 canónica)
  }
...
the 2026 reframe the matter factor $\pi/\varphi$ pairs the CMB sector with the
single global anchor $67.96$ along the geometric degeneracy (Paper~1), so the
cascade is anchored on $67.962$ and the MIRA value is retired as canonical.
...
\begin{enumerate}
  ...
  \item \textbf{Growth factor $\sigma_8$}: not modified.
        ...
        The $\sigma_8^{\rm eff} = 0.748$ result of Paper~6 is unaffected.
\end{enumerate}
...
2 (MCMC DESI) & MCMC posterior $\Hzero$ (prior $H_0^{\rm alg}$; PENDIENTE Fase B re-fit) & No —
               MCMC measures the global background, not local \\ % TODO Fase B: posterior con prior 67.96 (P2)
...
6 ($\phi$-DM) & $\sigma_8^{\rm eff}=0.748$, $k_{\rm fs}=0.762\,h$/Mpc & No \\
...
(Postulate~D, Paper~1), shared with the CMB sector via the matter factor
$\pi/\varphi$.
