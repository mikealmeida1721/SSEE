...
Paper~7 derives $\alphaK = 0.403302$ numerically from CLASS/EFTCAMB; in
...
\end{equation}
Numerically: $\alphaK = 3\times3.998\times1.524/(2\times22.65) = 0.403302$,
...
$\alphaK$    & $3\AURA(\pi-\phiG)/(2\Omega^2)$ & $0.403302$ & Eq.~\eqref{eq:alphaK_algebraic} \\
$3\MIRA$     & $3\AURA/2$             & $5.9967$       & Eq.~\eqref{eq:3MIRA} \\
$\fsc = \alphaK/(3\MIRA)$ & $(\pi-\phiG)/\Omega^2$ & $0.067253$ & Eq.~\eqref{eq:fscreen_identity} \\
\midrule
...
2 (MCMC DESI) & MCMC posterior $\Hzero = 67.95 \pm 0.40$ (prior $H_0^{\rm alg}$) & No —
...
6 ($\phi$-DM) & $\sigma_8^{\rm eff}=0.747$, $k_{\rm fs}=0.754\,h$/Mpc & No \\
7 (EFT)       & $\alphaK = 0.403302$, $\bc = -\AURA$ & Provides $\alphaK$
               (input to Paper~9) \\
...
$\alphaK^{\rm full} = 0.41691$ ($+3.37\%$ over the IR value) and the UV value
