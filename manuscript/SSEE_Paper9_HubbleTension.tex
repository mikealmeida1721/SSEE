...
Applied instead to the algebraic value $H_0^{\rm alg}=3(\phiG+\pi)^2=67.96$ (a Type~P
coincidence; Paper~1) the same mechanism gives $H_0^{\rm local,\,alg}=72.86$~km/s/Mpc
($0.17\sigma$ from SH0ES); we adopt the MIRA-derived value as the canonical IR prediction
for methodological consistency with the rest of the suite (Papers~2--3,~6).
The improved agreement obtained with the algebraic value is therefore a numerical
coincidence rather than a physical prediction.
...
completion, closing the residual $1.10\sigma$ gap.
\end{abstract}
...
Tension vs SH0ES (Type~P)    & ---            & $0.17\sigma$   & numerical coincidence \\
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    The numerical-coincidence value $H_0^{\rm local,alg}=72.86$ obtained from
    $H_0^{\rm alg}=67.96$ ($0.17\sigma$ SH0ES) is shown for comparison.
...
Applying the same mechanism to $H_0^{\rm alg}=3(\phiG+\pi)^2=67.96$
(a Type~P numerical coincidence; Paper~1) yields
$H_0^{\rm local,alg}=72.86$~km/s/Mpc (multiplicative) and $72.53$
(additive), with SH0ES tensions of $0.17\sigma$ and $0.49\sigma$ respectively.
The sharper agreement obtained from $H_0^{\rm alg}$ is a numerical coincidence
rather than a physical prediction; we report it for transparency but use
$H_0^{\rm MIRA}$ as the canonical input throughout this paper.
...
\textit{Type-P comparison}: applied to $H_0^{\rm alg}=67.96$ the same mechanism
gives $H_0^{\rm local,alg}=72.86$ km/s/Mpc ($0.17\sigma$ SH0ES); see
\S\ref{sec:localH0} for discussion.
\end{quote}
...
using $H_0^{\rm local} = 72.86$\,km/s/Mpc, the dynamical-sector matter density
