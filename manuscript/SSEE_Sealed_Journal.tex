...
% ----------------------------------------------------------------------------
\title{\bfseries Fixing the Dark-Energy Equation of State with Two
Dimensionless Constants, $\varphi$ and $\pi$:\\
A Falsifiable, Parameter-Frugal Test Against DESI DR2 and Planck PR4}
\author{Mike Edison Almeida Vallejo \\
...
as a single measured input, and the algebraic origin of the CMB matter density
($\omega_b$, $\omega_c$) remains an open problem. The growth amplitude $S_8$,
previously reported here as a standing tension with weak lensing, is not one:
measured against raw KiDS-1000 $\xi_\pm$ data with a single matter sector it
gives $0.7555\pm0.0192$, statistically indistinguishable from the $\Lambda$CDM control on the same raw data ($0.7571\pm0.0194$) despite four fewer free parameters. On the raw data itself --- the only quantity neither model produced --- $\Delta\chi^2=2.65$ for four \emph{fewer} free parameters, where four uninformative parameters would already return $4$ by chance ($\Delta\mathrm{BIC}=-19.0$ favouring SSEE). Against the later KiDS-Legacy release (357 points) the sector closes further: the
amplitude the shear asks for sits $0.49\sigma$ from the one the CMB fixes under
the same background, so fixing it leaves \emph{no} free cosmological parameter
and still predicts $S_8=0.8273$ against $0.8265\pm0.0176$ measured. The $\varphi$-dark-matter
extension that was formerly kept outside the core register is retracted. Both core forward predictions bear on
...
(Paper~7) is a distinct symbol with parent AURA, unrelated to $\beta$.
...
\begin{equation}
  ...
  w_a \;=\; -\,\frac{P_{\mathrm{sc}}}{I_g}
        \;=\; -\,\frac{\Omega+\varphi}{\pi+\Omega+\varphi}
        \;=\; -0.66997 .
  \label{eq:w0wa}
\end{equation}
...
set \citep{AlmeidaSSEE2026genesis}; the look-elsewhere count below is performed
over \emph{every} named invariant, with none enlarged, pruned, or selected to
improve the match. Counting over the complete set rather than a subset is the
conservative choice: it can only inflate the denominator of accidental hits, never
shrink it. We make no claim of temporal priority
...
reserved for unreleased data (\S\ref{sec:falsifiability}). Many invariants
share a value by construction---the $55$ names collapse to $25$ distinct numbers
(e.g.\ $\mathrm{IGNIS}=K_v$, $\mathrm{SOLAR}=\mathrm{MAR}$, and the Sovereign
convergence at $3\Omega$); the count below deduplicates by value, so each
...
$|w_a|=P_{\rm sc}/\mathrm{IGNIS}=\mathrm{PYROS}/(\pi+\mathrm{PYROS})$, both to machine precision
...
\end{equation}
Table~\ref{tab:lookelsewhere-tol} reports the full sensitivity to the tolerance,
and we state it honestly: the uniqueness is a property of the strict, sub-percent
regime that the \emph{exact} match actually occupies; relaxing the band far above
the DESI precision predictably densifies accidental hits. Within the model's own
vocabulary, at the precision of the measurement, each equation-of-state parameter
is \emph{essentially unique}---the opposite of a dense forest of coincidences, and
a stronger statement than a generic $(a\varphi+b\pi)/(c\varphi+d\pi)$ scan, which
...
rival to shadow. \emph{First, construction:} the canonical denominator
$\mathrm{IGNIS}=2\Omega$ is an integer multiple of the same scaffold that carries
$|w_0|$ ($M_v=3\Omega$), so $\mathrm{PYROS}/\mathrm{IGNIS}$ belongs to the shared
two-projection structure above, whereas $\mathrm{NYX}=(\varphi+7\pi)/2$ is the
retention-branch floor (charge $q=-3$) with no link to $|w_0|$; the selection is
therefore made by lineage, not by tolerance. \emph{Second, transcendence:} the
...
no $\varphi$-DM mass multiplier at all: that construction is retracted. Its
warrant had been \emph{falsifiability} --- a pre-registered free-streaming
scale that DESI~Y3 and Euclid would confirm or exclude. That warrant was
insufficient, and the episode is instructive: falsifiability is the minimum
requirement for a hypothesis to be scientific, not evidence that the entity it
concerns exists.
...
is not a second matter density at all: it is $1+w_0$, a quantity from the
equation of state. There is one matter density, $\Om^{\mathrm{cosm}}$; the
``two-$\Om$'' accounting of earlier versions is withdrawn.
...
(Paper~9): $H_0^{\rm glob,IR}=H_0^{\rm SH0ES}(1-f_{\rm scr}^{\rm IR})=68.13~\kms$, within $0.17\sigma$ of
...
steps). $\Delta\equiv\mathrm{SSEE}-\mathrm{model}$; negative $\Delta$ favours SSEE.}
\label{tab:modelcomp}
...
\begin{equation}
  ...
  \qquad
  \alpha_{\rm K}=15.591335 ,
\end{equation}
so that gravitational waves propagate at the speed of light
($\alpha_{\rm T}=0$, consistent with GW170817~\citep{GW170817}) and the only
non-trivial function is the kineticity.

\paragraph{Correction (2026-09-26): $0.403302$ is $s_K$, never $\alpha_K$.}
Earlier versions of this section wrote
$\alpha_{\rm K}(0)=3\,\Omega_{\rm DE}\,\Omega_m^{\mathrm{dyn}}=0.403302$ with
$\Omega_{\rm DE}=0.839950$ and $\Omega_m^{\mathrm{dyn}}=0.160050$, and reported a
\texttt{hi\_class} cross-check recovering $0.4033$ to $0.005\%$. Three things were
wrong with that sentence, and the cross-check inherited all three because it
validated the same mislabelled quantity. First, $0.839950$ and $0.160050$ are
$s_{\rm DE}=|w_0|$ and $s_m=1+w_0$: saturations, numbers of the equation of
state, not the densities $\Omega_{\rm DE}=0.691119$ and $\Omega_m=0.308881$.
Second, their product is not a kineticity. Following Paper~7, the object is
%
\begin{equation}
  s_K \equiv -\frac{1}{\rho_\phi}\frac{dp_\phi}{dN} = -3w_0(1+w_0)
      = 3\,s_{\rm DE}\,s_m = 0.403302458888 ,
  \label{eq:sK_sealed}
\end{equation}
%
the fractional dilution rate of the dark-energy \emph{pressure} per e-fold:
pure equation of state, with no $H$, no density and no $K(X)$ in it. That is
precisely why the screening amplitude of Paper~9, $f_{\rm scr}=s_K/(3\,\mathrm{MIRA})$,
is invariant under changes of the expansion anchor, whereas a genuine
$\alpha_K$ would not be. Third, the true kineticity is
$\alpha_K=15.591335$, unrelated to $s_K$. No result of this journal depended on
the mislabel: every use of $0.403302$ downstream is a use of $s_K$. The associated conformal coupling
...
This is decisively tested by the CMB. Running the Boltzmann
...
situation honestly, including a correction to what earlier versions of this
document reported. With the Poisson source evaluated at the CMB-calibrated
density $\Om^{\mathrm{cosm}}=0.308881$ ($\omega_m$-direct, shown physically
required by the Boltzmann test of \S\ref{sec:eft}), the growth factor is
essentially that of $\Lambda$CDM,
...
\begin{figure}[ht]
  ...
  \caption{$S_8$ with the primordial amplitude fixed versus fitted. Red
  ($S_8=0.827\pm0.006$): $A_s$ held at the Planck value, which imports the
  Planck--KiDS discrepancy. Green ($S_8=0.7555\pm0.0192$): $A_s$ fitted to the
  raw KiDS-1000 $\xi_\pm$ data with a single matter sector and the background
  fixed by the algebra --- $0.11\sigma$ from KiDS-1000 (green band); the $\Lambda$CDM control on the same raw data lands at $0.7571\pm0.0194$. Grey
  points: Planck 2018, KiDS-1000 and DES-Y3 with $1\sigma$ errors.
  \emph{(An earlier version of this figure showed a two-sector $\varphi$-DM
  curve; that construction is retracted.)}}
  \label{fig:s8_resolution}
\end{figure}

\paragraph{The honest statement.} What resolves $S_8$ here is not additional
freedom --- the background is \emph{more} constrained in this model than in
$\Lambda$CDM, with $A_s$ the only quantity able to move --- but the removal of
two things that should not have been there: a fixed $A_s$, and a target
statistic carrying $\Lambda$CDM in its reduction. The cost of the correction
is real and worth stating plainly: a pre-registered, genuinely falsifiable
$P(k)$ prediction is gone, withdrawn not because data refuted it but because
the construction generating it was not physical. What is gained is one
postulate fewer and four open problems closed by dissolution. A full
non-linear treatment still requires $N$-body simulations beyond the scope of
this work.
...
\item \textbf{The $\varphi$-dark-matter mass (OP-9): closed by dissolution.}
This entry previously recorded an algebraic mass $m_\varphi$ whose UV origin
was open. The particle itself is now retracted, so there is no multiplier left
to derive and the open problem closes without having been solved. The
free-streaming step in $P(k)$ that descended from it is withdrawn as a
prediction. OP-10, OP-11 and OP-12 close the same way.
...
The growth-of-structure status of \S\ref{sec:growth} no longer mirrors this
structure. The $S_8$ ``tension'' at $0.827$ was an artefact of holding $A_s$ at
the Planck value while comparing against a $\Lambda$CDM-conditioned summary
statistic; fitted to raw shear data with a single sector it is
$0.7555\pm0.0192$, matching the $\Lambda$CDM control on the same data ($0.7571\pm0.0194$) with four fewer free parameters. Against the later KiDS-Legacy release the growth sector needs no free amplitude either: the $A_s$ the shear asks for sits $0.49\sigma$ from the one the CMB fixes under the same background, and fixing it predicts $S_8=0.8273$ against $0.8265\pm0.0176$ measured. No extension sector is required, and the one
proposed is retracted. The non-linear treatment required to settle the matter
definitively still demands $N$-body simulations beyond this work's scope.
...
\begin{equation}
  ...
  \mathrm{AURA},\quad
  \mathrm{DUAL}=2\,\mathrm{AURA},\quad
  \mathrm{TRIAL}=3\,\mathrm{AURA}=T_r,\quad
  \mathrm{CUARTAL}=4\,\mathrm{AURA}.
  \label{eq:copylaw}
\end{equation}
The same law acts on the scaffold itself, giving the denominators
$K_v=2\Omega$ and $M_v=3\Omega$. The economy noted in
\S\ref{sec:look-elsewhere} is a direct consequence: since
$T_r=3\,\mathrm{AURA}$ and $M_v=3\Omega$, the common factor of three cancels in
\begin{equation}
  |w_0|=\frac{T_r}{M_v}=\frac{3\,\mathrm{AURA}}{3\,\Omega}
        =\frac{\mathrm{AURA}}{\Omega}=0.839950 ,
\end{equation}
so the equation-of-state numerator and denominator are not two independent
draws but a single ratio of the two branch roots, $\mathrm{AURA}/\Omega$.
...
without being the same entity: by construction
$\mathrm{IGNIS}=K_v=9.519253$ (disruptive vs.\ structural lineage) and
...
equation-of-state ratio lives entirely on this branch,
\begin{equation}
  |w_a|=\frac{P_{\rm sc}}{\mathrm{IGNIS}}=\frac{\mathrm{PYROS}}{\pi+\mathrm{PYROS}}=0.669975 ,
\end{equation}
where the denominator is read as the $\pi$-branch node IGNIS rather than the
scaffold copy $K_v$---the two coincide in value by Law~II.

\paragraph{Closure.} The set is finite because both laws terminate at the
four-dimensional integration invariant $M_v=\varphi+\pi+K_v=14.278880$ (coinciding in value with $3\Omega$), the
maximal dimensional invariant; no node exceeds it. This termination---not a
