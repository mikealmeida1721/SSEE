...
\emph{parents} (the entities it is generated from), the algebraic formula, and
the value. Symbols are in one-to-one correspondence with entities: no symbol
denotes two entities and no entity carries two symbols. This matters because two
entities may share a \emph{value} while being structurally distinct---$I_g$ and
$K_v$ both equal $9.51925$---so the symbol, not the number, identifies which one
a given equation uses.

\begin{center}
\begin{tabular}{lllll}
\toprule
Symbol & Name & Parents & Formula & Value \\
\midrule
$\varphi$   & ---               & seed (UV)              & $(1+\sqrt5)/2$        & $1.61803$  \\
$\pi$       & ---               & seed (IR)              & ---                   & $3.14159$  \\
$\Omega$    & OMEGA             & $\varphi,\pi$          & $\varphi+\pi$         & $4.75963$  \\
$\beta$     & BIAL              & $\varphi,\pi$          & $(\varphi+\pi)/2$     & $2.37981$  \\
$\mathrm{KAL}$  & KAL           & $\beta,\pi$            & $\beta+\pi$           & $5.52141$  \\
$\mathrm{AURA}$ & AURA          & $\beta,\varphi$        & $\beta+\varphi$       & $3.99785$  \\
$\mathrm{MIRA}$ & MIRA          & AURA                   & $\mathrm{AURA}/2$     & $1.99892$  \\
$P_{\mathrm{sc}}$ & PYROS       & $\Omega,\varphi$       & $\Omega+\varphi$      & $6.37766$  \\
$I_g$       & IGNIS             & $\pi,\mathrm{PYROS}$   & $\pi+P_{\mathrm{sc}}$ & $9.51925$  \\
$K_v$       & KRYSTOS$_V$       & $\varphi,\pi,\Omega$   & $\varphi+\pi+\Omega$  & $9.51925$  \\
$T_r$       & TRIAL             & AURA                   & $3\,\mathrm{AURA}=3(\varphi+\beta)$ & $11.99354$ \\
$M_v$       & ATLAS             & $\varphi,\pi,K_v$      & $\varphi+\pi+K_v$     & $14.27888$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{equation}
  H_0^{\mathrm{post}} = 67.7872 \pm 0.352~\kms ,
  \qquad
...
the algebraically-derived (not fit) fraction $f_{\rm scr}=\alpha_K/(3\,\mathrm{MIRA})=(\pi-\varphi)/\Om^2=0.06725$
...
$H_0^{\rm SH0ES}(1-f_{\rm scr}^{\rm UV})=73.04\times(1-0.069522)=67.962~\kms$
\emph{exactly}. We are deliberately precise about what is independent of what.
...
posterior ($67.7872$) and the CMB anchor ($67.962$) agree to $0.50\sigma$: DESI
...
\begin{figure}[t]
  ...
  $H_0=67.7872\pm0.352~\kms$ and $\Omega_b h^2 = 0.02207\pm0.00045$
...
The tight DESI DR2 neutrino-mass bound ($\Sigma m_\nu\lesssim0.06$--$0.07$~eV)
is derived \emph{within} $\Lambda$CDM and does not apply here: with $w_0,w_a$
free the same data relax it to $\sim0.13$--$0.16$~eV, so the SSEE value
$0.06849$~eV is comfortably allowed in its own background. Quoting the
$\Lambda$CDM bound against a dynamical-dark-energy model would be a category
error.} This is a physical constant common to all
