...
$w_a=-P_{\rm sc}/I_g=-0.670$, with no fitted amplitude. The point was fixed in
...
$P_{\mathrm{sc}}$ & $\Omega+\varphi$       & $6.37766$  & Dynamical evolution scalar \\
$I_g$ (IGNIS)  & $\pi+P_{\mathrm{sc}}$    & $9.51925$  & Disruptive convergence ($w_a$ denominator) \\
...
w_a=-\frac{P_{\mathrm{sc}}}{I_g}=-0.66997,\qquad
...
Eq.~\eqref{eq:h0anchor} is and is not, and about the direction in which it is
tested. $H_0^{\rm alg}=67.962~\kms$ is not computed from any single
measurement: it is the pure number $3(\varphi+\pi)^2$, and its physical status
as a genuine dimensionful rate comes from being the value that minimises the
Planck likelihood for the SSEE background (\S\ref{sec:mcmc}) and that the
DESI~DR2 BAO posterior independently returns to $0.04\sigma$ (Eq.~\ref{eq:h0post}).
Taking this global anchor as given, the screening cascade of Paper~9 runs
\emph{forward}, global to local:
$H_0^{\rm local}=H_0^{\rm alg}/(1-f_{\rm scr})=72.86~\kms$, where
$f_{\rm scr}=\alpha_K/(3\,\mathrm{MIRA})=(\pi-\varphi)/\Om^2=0.06725$ is a
dimensionless factor shown not to be a fit. This \emph{predicted} local rate is
then compared against the independent SH0ES measurement,
$H_0^{\rm SH0ES}=73.04\pm1.04~\kms$~\citep{Riess2022}, agreeing to $0.17\sigma$---not
an exact match (running the cascade in reverse on the measured SH0ES central
value gives $68.13~\kms$, not $67.962$; the $0.17\sigma$ gap is the honest
residual, not zero). Separately, the pre-registered dimensionless combination
$3(\varphi+\pi)^2=67.962$ \emph{coincides numerically} with the value of the
global rate in these units. We state plainly that this is a numerical
coincidence, not a dimensional identity: $3(\varphi+\pi)^2$ is a pure number,
and we make no claim that a dimensionless quantity \emph{equals} a
dimensionful rate---in Planck units $H_0\sim10^{-61}$ and no such coincidence
appears. The physical anchor is the CMB/BAO-calibrated global rate; the
algebraic match is a striking, pre-registered correspondence whose dimensional
origin---the bridge from the megaparsec scale to fundamental units---remains an
open problem. Because the anchor is the physical rate confirmed independently
at both ends of the cascade, the prediction is falsifiable through either: a
global $H_0$ displaced from $67.96~\kms$, or a local rate that moves the
$0.17\sigma$ agreement outside the SH0ES uncertainty, refutes the cascade.
...
\begin{equation}
  ...
  \qquad
  \Omega_{\rm DE}=\mathrm{AURA}/\Omega=1-\Om^{\mathrm{dyn}}=0.83995 ,
\end{equation}
...
$w_a=-P_{\rm sc}/I_g$ & Background (DE)   & $-0.670$                     & P1,2,4 & Postdiction \\
$(w_0,w_a)$ vs DESI DR3       & Background (DE)   & same fixed point                & P1,2   & \textbf{Prediction (pre-reg.)} \\
$H_0/\kms=3(\varphi+\pi)^2$   & Background (DE)   & $67.96\,\kms$                   & P1,2,9 & Global anchor (Post.~D) \\
...
$m_\phi=\Sigma m_\nu^{\rm act}(\mathrm{SOLAR}^2 K_v)$ & $\varphi$-DM & $40.70\,$eV      & P6     & \textbf{Prediction} \\
...
was performed. The bare-field $\alpha_K^{\rm field}=0.073$ (Paper~7) is the
pre-integration kinetic contribution, distinct from the background-integrated
$\alpha_K(0)=0.4033$ used and cross-checked against \texttt{hi\_class} in
...
spectral index $n_s=1-\varphi^{-7}$, and the expansion anchor $H_0=67.96~\kms$
...
\begin{equation}
  |w_0|=\frac{T_r}{M_v}=\frac{3\,\mathrm{AURA}}{3\,\Omega}
        =\frac{\mathrm{AURA}}{\Omega}=0.83995 ,
\end{equation}
...
\begin{equation}
  |w_a|=\frac{P_{\rm sc}}{\mathrm{IGNIS}}=\frac{\mathrm{PYROS}}{\pi+\mathrm{PYROS}}=0.66997 ,
\end{equation}
...
four-dimensional integration invariant $M_v=\varphi+\pi+K_v=14.27888$ (coinciding in value with $3\Omega$), the
