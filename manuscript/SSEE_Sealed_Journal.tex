...
Planck-anchored prior yields $H_0=66.53\pm0.44~\kms$. The
...
kept outside the core register), and the dynamical origin of MIRA and the
...
\begin{figure}[t]
  ...
  shaded contours are the DESI DR2 $w_0w_a$CDM posterior
  (BAO\,$+$\,CMB\,$+$\,DESY5). The SSEE point falls inside the $1\sigma$ region;
  the joint Mahalanobis distance is $\chi^2_{\rm 2D}=0.080$, an equivalent
  $0.05\sigma$ agreement (Eq.~\ref{eq:chi2}).}
  \label{fig:w0wa}
\end{figure}
...
$\Om^{\mathrm{dyn}}=0.160$) is mapped by the MIRA relation to $147.6$~Mpc
at $\Om^{\mathrm{cosm}}=0.31993$, consistent with the Planck-inferred
$147.09\pm0.26$~Mpc. The large pre-mapping scale is an internal geometric
...
\begin{figure}[ht]
  ...
  (red, $175.6$~Mpc) is carried by the MIRA mapping to $147.6$~Mpc
  (green), in agreement with Planck 2018 ($147.09\pm0.26$~Mpc, grey band)
  and the $\Lambda$CDM Eisenstein--Hu value (blue, $147.3$~Mpc). This is
  the visual counterpart of the Boltzmann test above: the same single
  algebraic constant that restores the acoustic peaks also restores the
  BAO standard ruler.}
  \label{fig:rd_dual}
\end{figure}
...
\begin{equation}
  H_0^{\mathrm{post}} = 66.533 \pm 0.442~\kms ,
  \qquad
  \Omega_b h^2 = 0.02261 \pm 0.00048 ,
  \label{eq:h0post}
\end{equation}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.62\textwidth]{fig_corner_ssee_professional.pdf}
  ...
  DESI DR2 BAO with a CMB-derived (MIRA) prior. The posterior is
  $H_0=66.533\pm0.442~\kms$ and $\Omega_b h^2 = 0.02261\pm0.00048$
  (Eq.~\ref{eq:h0post}). Dashed lines mark the anchor inputs
  $(H_0=67.037~\kms,\ \Omega_b h^2 = 0.02218)$; the $16/50/84\%$ quantiles are
  indicated.}
  \label{fig:corner}
\end{figure}
...
\begin{table}[t]
  ...
  (\text{SSEE}-\Lambda\text{CDM})=-28.0$ over $N=5914$ data points reflects the
...
\begin{figure}[t]
  ...
  structure is essential: replacing $\Om^{\mathrm{cosm}}=0.320$ with the bare
...
CMB-calibrated density $\Om^{\mathrm{cosm}}=0.320$ (the same MIRA sector that
the Boltzmann test of \S\ref{sec:eft} shows is physically required), the growth
factor is essentially that of $\Lambda$CDM,
$G\equiv D_1^{\rm SSEE}/D_1^{\Lambda\rm CDM}=1.011\pm0.005$, and the present-day
amplitude is
\begin{equation}
  \label{eq:s8}
  \sigma_8=0.820\pm0.006 , \qquad S_8=0.847
\end{equation}
in the single-sector treatment---a $3.9\sigma$ tension with KiDS-1000
...
$k_{\rm fs}\simeq0.659~\hMpc$, \emph{lowers} the amplitude to
$\sigma_8=0.742$, $S_8=0.766$, within $0.01\sigma$ of the KiDS-1000 value. The
mass that fixes the free-streaming scale,
$m_\varphi=\Sigma m_\nu^{\rm active}\,(\Omega_{\rm DNAV}^4+\mathrm{AURA}\cdot\mathrm{KAL})\simeq36.95$~eV,
...
\begin{figure}[ht]
  ...
  resolution. The single-sector prediction (red, $S_8=0.847\pm0.006$)
  carries the $3.9\sigma$ tension with KiDS-1000 reported in
  Eq.~\eqref{eq:s8}; the two-sector $\varphi$-DM extension (green,
  $S_8=0.766$) lands $0.01\sigma$ from KiDS-1000 (green band), with the
  free-streaming scale fixed by the forward-predicted mass
  $m_\varphi=36.95$~eV --- no parameter fitted to lensing data. Grey
...
$m_\varphi\simeq36.95$~eV used in the extension sector follows from the
...
in the matter power spectrum near $k_{\rm fs}\simeq 0.659~\hMpc$, testable by
...
the single-sector core inherits an $S_8$ tension at the $\Lambda$CDM level
($3.9\sigma$), and the two-sector extension that resolves it ($S_8=0.766$,
