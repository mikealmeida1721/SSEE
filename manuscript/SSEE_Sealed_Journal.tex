...
\emph{exactly}. Both directions use only the measured SH0ES datum and an
independently-derived screening fraction; no algebraic value is assumed in
advance. The CMB/BAO route of the preceding paragraph, which never touches
SH0ES, already confirms the same value to $0.04\sigma$. Three independent
checks---the unconditional SH0ES de-screening (IR floor, $0.17\sigma$), its
UV-completed sharpening (exact, conditional on Postulate~C.1), and the
CMB/BAO posterior ($0.04\sigma$)---converge on essentially the same number
without any of them being used to derive another; that convergence, not a
claim that $3(\varphi+\pi)^2$ literally
