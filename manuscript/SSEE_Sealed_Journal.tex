...
the independently-derived (not fit) fraction
$f_{\rm scr}=\alpha_K/(3\,\mathrm{MIRA})=(\pi-\varphi)/\Om^2=0.06725$ (Paper~9):
$H_0^{\rm SH0ES}(1-f_{\rm scr})=68.13~\kms$. This SH0ES-derived global rate
uses no input from the algebra at all, and it lands within $0.17\sigma$ of the
pure number $67.962$ (propagating the SH0ES uncertainty through $f_{\rm scr}$)---a
striking resemblance, not an exact identity. Independently of SH0ES, taking
$67.962~\kms$ as the CMB-optimal anchor for the SSEE background, the
DESI~DR2 BAO posterior returns $H_0^{\rm post}=67.9475\pm0.396~\kms$
(Eq.~\ref{eq:h0post}), agreeing to $0.04\sigma$. Two independent
routes---one from the local distance ladder, de-screened, and one from the
early-time CMB/BAO geometry---converge on essentially the same number
without either being used to derive the other; that convergence, not a claim
that $3(\varphi+\pi)^2$ literally equals a dimensionful rate, is the anchor's
physical content. In Planck units $H_0\sim10^{-61}$ and no such coincidence
appears; the dimensional origin---the bridge from the megaparsec scale to
fundamental units---remains open (\S\ref{sec:open}). The anchor is
falsifiable on both fronts: a shift in the CMB/BAO agreement beyond
$0.04\sigma$, or a SH0ES-derived local rate that moves outside $0.17\sigma$,
undermines it.
