...
(conditional on the derivation of $M^4=5\varphi^8\rho_{\rm crit}$, Postulate~C.1),
the identical inversion gives
$H_0^{\rm SH0ES}(1-f_{\rm scr}^{\rm UV})=73.040\times(1-0.069522)=67.962~\kms$
\emph{exactly}, to the precision quoted. Both directions use only the measured
SH0ES datum and an independently-derived screening fraction; no algebraic
value is assumed in advance. Independently of SH0ES, taking $67.962~\kms$ as
the CMB-optimal anchor for the SSEE background, the DESI~DR2 BAO posterior
returns $H_0^{\rm post}=67.9475\pm0.396~\kms$ (Eq.~\ref{eq:h0post}), agreeing
to $0.04\sigma$. Three independent checks---the unconditional SH0ES
de-screening (IR floor, $0.17\sigma$), its UV-completed sharpening (exact,
conditional on Postulate~C.1), and the CMB/BAO posterior ($0.04\sigma$)---converge
on essentially the same number without any of them being used to derive
another; that convergence, not a claim that $3(\varphi+\pi)^2$ literally
equals a dimensionful rate, is the anchor's physical content. In Planck units
...
open (\S\ref{sec:open}). The anchor is falsifiable on multiple fronts: a shift
in the CMB/BAO agreement beyond $0.04\sigma$, or a SH0ES-derived local rate
that moves the unconditional IR floor outside $0.17\sigma$, undermines it.
