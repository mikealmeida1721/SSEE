...
Eq.~\eqref{eq:h0anchor} is and is not. The anchor $67.962~\kms$ is a genuine
dimensionful rate, and its units are physical rather than decreed by a formula:
it is the global background rate carried down from the SH0ES \emph{local}
measurement through the screening cascade
$H_0^{\rm alg}=H_0^{\rm local}(1-f_{\rm scr})$ (Paper~9), where
$f_{\rm scr}=\alpha_K/(3\,\mathrm{MIRA})=(\pi-\varphi)/\Om^2$ is a dimensionless
factor shown not to be a fit. The units are inherited from the measured local
rate through a dimensionless screening factor. Independently, the pre-registered
dimensionless combination $3(\varphi+\pi)^2=67.962$ \emph{coincides numerically}
with the value of this rate in these units. We state plainly that this is a
numerical coincidence, not a dimensional identity: $3(\varphi+\pi)^2$ is a pure
number, and we make no claim that a dimensionless quantity \emph{equals} a
dimensionful rate---in Planck units $H_0\sim10^{-61}$ and no such coincidence
appears. The physical anchor is the SH0ES-calibrated global rate; the algebraic
match is a striking, pre-registered correspondence whose dimensional
origin---the bridge from the megaparsec scale to fundamental units---remains an
open problem. Because the anchor is the physical rate, the prediction is
falsifiable through it, independently of the coincidence: a global $H_0$
displaced from $67.96~\kms$ refutes the cascade.
