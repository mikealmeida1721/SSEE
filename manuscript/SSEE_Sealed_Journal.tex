...
DR2 $w_0w_a$CDM posterior the agreement is at the $0.16\sigma$ level
(Pantheon+; $0.2$--$1.5\sigma$ across supernova compilations)
($\chi^2_{\rm 2D}=0.27$). A Markov-chain analysis combining DESI BAO with the
$\omega_m$-direct CMB anchor as prior yields $H_0=66.41\pm0.39~\kms$. The
...
coefficient are open problems. The result is a falsifiable,
...
(correlation $\rho\simeq-0.85$, estimated from the public contours; the paper
does not quote it in text). Because the two parameters are strongly
...
covariance $\mathsf{C}$,
\begin{equation}
  \chi^2_{\rm 2D}=\mathbf{d}^{\!\top}\mathsf{C}^{-1}\mathbf{d}=0.27,
  \qquad
  p=0.87,
  \qquad
  \text{equivalent } 0.16\sigma .
  \label{eq:chi2}
\end{equation}
The component offsets are $\Delta w_0=-0.04\sigma$ (essentially exact) and
$\Delta w_a=-0.24\sigma$. Against the other official combinations the joint
distance is $1.23\sigma$ (DESY5), $1.50\sigma$ (Union3) and $1.52\sigma$
(no-SN DESI$+$CMB): the algebraic point is consistent with all four, with the
tension range $0.2$--$1.5\sigma$ set by the supernova compilation. The result
is robust to the assumed correlation ($\rho\in[-0.92,-0.70]$ shifts the
Pantheon+ figure by $<0.2\sigma$). We stress that this agreement is obtained
...
The $0.16\sigma$ figure is specific to the DESI DR2 dynamical-dark-energy
...
$0.2$--$1.5\sigma$ reflects the known spread among supernova compilations.
...
\begin{figure}[t]
  ...
  $(w_0,w_a)=(-T_r/M_v,\,-P_{\rm sc}/K_v)=(-0.840,-0.670)$ is fixed entirely by
  ...
  $1\sigma$ region; the joint Mahalanobis distance is $\chi^2_{\rm 2D}=0.27$,
  an equivalent $0.16\sigma$ agreement (Eq.~\ref{eq:chi2}), and within
  $1.2$--$1.5\sigma$ of the other compilations.}
  \label{fig:w0wa}
\end{figure}
...
$31$ named structural invariants---the \emph{complete} Genesis~5.12 set,
not a hand-picked subset---specified as a fixed set rather than expanded to
accommodate the data.\footnote{The dictionary is the full Genesis~5.12 constant
...
Enumerating every ordered pair over the complete dictionary yields $317$ distinct
...
$|w_a|=P_{\rm sc}/K_v=\mathrm{PYROS}/\mathrm{IGNIS}$, both to machine precision
($|d|=0$)---so the physically relevant tolerance is the observational precision of
DESI ($\sim\!\pm0.002$), not an arbitrary band. At that precision each target is
unique:
\begin{equation}
  |w_0|=0.840:\ \text{1 of 317},\qquad
  |w_a|=0.670:\ \text{1 of 317}\quad(\pm0.001).
...
\emph{complete} $31$-constant Genesis~5.12 dictionary ($317$ distinct ratios in
$(0,5]$; constants sharing a value by the no-self-sum law collapse to $20$ distinct
numbers). The DESI assignments are exact identities ($|d|=0$); the observationally
relevant band is $\lesssim\!\pm0.002$, where each parameter is unique. The count is
robust to the planned future dimensional copies (QUINTAL--DECAL): they add no hit
within $\pm0.002$ of either target.}
...
Paper~1; $0.41\sigma$ from Planck). The earlier presentation tied
...
horizon itself: the pure geometric SSEE scale ($175.6$~Mpc at
$\Om^{\mathrm{dyn}}=0.160$) and the physical CMB drag scale set by the full
algebraic $\omega_m$ ($147.17$~Mpc at $\Om^{\mathrm{cosm}}=0.30889$,
$\omega_m$-direct) which is consistent with the Planck-inferred
$147.09\pm0.26$~Mpc ($0.32\sigma$). The large geometric scale is an internal
quantity, not the model's prediction for the observed BAO standard ruler.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.82\textwidth]{fig_rd_dual.pdf}
  \caption{The dual sound horizon. The pure geometric SSEE drag scale
  (red, $175.6$~Mpc at $\Om^{\mathrm{dyn}}=0.160$) and the physical CMB drag
  scale set by the full algebraic $\omega_m$ (green, $147.17$~Mpc at
  $\Om^{\mathrm{cosm}}=0.30889$, $\omega_m$-direct), in agreement with Planck
  2018 ($147.09\pm0.26$~Mpc, grey band) and the $\Lambda$CDM Eisenstein--Hu
  value (blue, $147.3$~Mpc). This is the visual counterpart of the Boltzmann
  test above: the full matter density that restores the acoustic peaks also
  restores the BAO standard ruler. (The earlier MIRA-mapped $147.6$~Mpc at
  $0.31993$ is superseded.)}
  \label{fig:rd_dual}
\end{figure}
...
\begin{equation}
  H_0^{\mathrm{post}} = 66.412 \pm 0.385~\kms ,
  \qquad
  \Omega_b h^2 = 0.02343 \pm 0.00045 ,
  \label{eq:h0post}
\end{equation}
with $r_d = 147.17~\mathrm{Mpc}$ ($0.32\sigma$, $\omega_m$-set, $H_0$-invariant)
at the posterior value. These
are two stages of the same quantity---a CMB anchor and a BAO-corrected
posterior---not two independent measurements. (The earlier MIRA-anchored
$67.037 / 66.533$ stages, and the $67.159$ posterior obtained with the
mislabelled DR1 vector, are superseded.)

\paragraph{The BAO--CMB split is a known feature, reported openly.} The
$1.55~\kms$ gap between Eqs.~\eqref{eq:h0anchor} and~\eqref{eq:h0post}
($2.9\sigma$ of the prior width) is
the same intrinsic BAO--CMB tension discussed in \S\ref{sec:desi-agreement},
amplified by the $\sim40\%$ tighter DESI~DR2 uncertainties: the late-time BAO
prefer a lower $H_0$ than the early-time acoustic geometry, and with DR2
precision this preference is no longer mild. It is a property of
dynamical-dark-energy fits to the current data,
not an artefact of SSEE, and we make it explicit rather than hide it inside a
single quoted number: the raw dynamic-sector background is quantifiably
strained by the DR2 precision, and we track its interpretation as an open
item rather than absorb it silently. At the CMB anchor all early-universe
observables
($r_d$, $\theta_\ast$) sit at $\sim1\sigma$ simultaneously; the split
...
$100\,\theta_\ast=1.03693$ ($13.9\sigma$); we quote this not as a tension but
to make the
...
\begin{figure}[t]
  ...
  $H_0=66.412\pm0.385~\kms$ and $\Omega_b h^2 = 0.02343\pm0.00045$
  (Eq.~\ref{eq:h0post}). Dashed lines mark the anchor inputs
  $(H_0=67.962~\kms,\ \Omega_b h^2 = 0.02218)$; the $16/50/84\%$ quantiles are
  indicated.}
  \label{fig:corner}
\end{figure}
...
$k_{\rm fs}\simeq0.762~\hMpc$, \emph{lowers} the amplitude to
$\sigma_8=0.748$, $S_8=0.759$, within $0.01\sigma$ of the KiDS-1000 value. The
mass that fixes the free-streaming scale,
$m_\varphi=\Sigma m_\nu^{\rm active}\,(\mathrm{SOLAR}^2\cdot\mathrm{KRYSTOS})\simeq41.02$~eV,
is a forward prediction with no parameter fitted to lensing data
(\S\ref{sec:open}); the suppression scale is a CLASS output, not a calibrated
...
\begin{figure}[ht]
  ...
  $S_8=0.759$) lands $0.01\sigma$ from KiDS-1000 (green band), with the
  free-streaming scale fixed by the forward-predicted mass
  $m_\varphi=41.02$~eV --- no parameter fitted to lensing data. Grey
...
two-sector extension does resolve it ($0.01\sigma$ of KiDS-1000), but the UV
...
$m_\varphi\simeq41.02$~eV used in the extension sector follows from the
...
in the matter power spectrum near $k_{\rm fs}\simeq 0.762~\hMpc$, testable by
...
($3.5\sigma$ KiDS-1000, $S_8=0.846$), and the two-sector extension that resolves it ($S_8=0.759$,
$0.01\sigma$ of KiDS-1000) rests on the $m_\varphi$ multiplier whose UV origin
...
$0.16\sigma$ (Pantheon+) without a single fitted dark-sector parameter. The
...
them as a descriptive grammar of the Genesis~5.12 set
...
\S\ref{sec:open}. Stating them explicitly is what turns ``$31$ named invariants''
...
$\Omega=\varphi+\pi=4.75962$, and their mean is the bifurcation point
...
$\mathrm{AURA}=\beta+\varphi=3.99784$, which propagates \emph{only} by integer
...
\begin{equation}
  |w_0|=\frac{T_r}{M_v}=\frac{3\,\mathrm{AURA}}{3\,\Omega}
        =\frac{\mathrm{AURA}}{\Omega}=0.83996 ,
\end{equation}
...
$\mathrm{IGNIS}=K_v=9.51925$ (disruptive vs.\ structural lineage) and
...
$31$ named nodes collapse to $20$ distinct numbers. The second
equation-of-state ratio lives entirely on this branch,
\begin{equation}
  |w_a|=\frac{P_{\rm sc}}{K_v}=\frac{\mathrm{PYROS}}{\mathrm{IGNIS}}=0.67000 ,
\end{equation}
...
five-dimensional integration ceiling $M_v=\varphi+\pi+K_v=3\Omega=14.27887$, the
