...
input $\alphaK^{\rm fluid} = 0.403302$ ($\Delta = 0.005\%$), while the theoretical
...
approximation $\alphaK^{\rm fluid} = 3\Omde(1+w_0) = 0.403302$ as a conservative
...
\begin{equation}
  ...
  = \begin{cases} 0.037 & (\alphaK = 0.073,\ \text{bare-field}) \\ 0.2016 & (\alphaK = 0.403302,\ \text{fluid approx.}) \end{cases} > 0.
  \label{eq:Qs}
\end{equation}
...
where the EFTCAMB run uses the fluid-approximation input $\alphaK^{\rm fluid} = 0.403302$; the theoretical bare-field value derived in Paper~7 is $\alphaK = 0.073$.
...
\item \textbf{EFT stability.} EFTCAMB confirms ghost-freedom and gradient
  stability using the fluid-approximation input $\alphaK^{\rm fluid} = 0.403302$
