...
values within $1.01\sigma$ of the posteriors (mean $0.64\sigma$ across the five parameters).
...
complementary emcee MCMC with 100 walkers, 25\,000 steps and a 2\,000-step
burn-in, yielding $2.5\times10^{6}$ post-burn samples, an acceptance fraction
of 0.537 and $N_{\rm eff}=4.0\times10^{4}$ (integrated autocorrelation
$\tau_{\max}=62.3$).
...
four IGIMF-corrected galaxy clusters. Crucially, we impose \emph{flat}
(uninformative) priors on $w_0$ and $w_a$ over $[-1.30,-0.40]\times[-2.0,0.5]$:
this is a \emph{blind} test in which the data alone --- not a prior centred on the
\SSEE\ values --- constrain the equation of state, so the recovered tensions are
free of circularity.
...
\midrule
$H_0$ [km\,s$^{-1}$\,Mpc$^{-1}$] & $67.08 \pm 0.93$ & $67.96$ & $0.94\sigma$ \\
$\Omega_b h^2$                     & $0.02236 \pm 0.00015$ & $0.02237$ & $0.04\sigma$ \\
$\Omega_m$                         & $0.3214  \pm 0.0124$  & $0.30889$  & $1.01\sigma$ \\
$w_0$                              & $-0.733  \pm 0.147$   & $-0.840$  & $0.73\sigma$ \\
$w_a$                              & $-0.974  \pm 0.608$   & $-0.670$  & $0.50\sigma$ \\
\midrule
\textbf{Mean tension} & & & $\mathbf{0.64\sigma}$ \\
\textbf{Maximum tension} & & & $\mathbf{1.01\sigma}$ \\
\bottomrule
\end{tabular}
\end{table}

Under flat $w_0,w_a$ priors the equation-of-state parameters are only loosely
constrained (note the wide $w_a$ interval, reflecting the well-known
$w_0$--$w_a$ degeneracy of the combined data), yet all five algebraic \SSEE\
values still lie within $\simeq1\sigma$ of the data-driven posteriors: the
maximum tension is $1.01\sigma$ ($\Omega_m$) and the mean is $0.64\sigma$.
We stress that this joint result is \emph{conditional on the specific data
combination} (DESI~DR2 + Planck compressed + six $f\sigma_8$ surveys + four
clusters); a different combination --- which weights the early- and late-time
probes differently --- would shift the numbers. Corner plots and trace
diagnostics are archived at Zenodo \citep{AlmeidaSSEE2026zenodo}.
...
\item \textbf{Background sector.} The algebraic prediction
  ...
  predictions within $1.01\sigma$ (mean $0.64\sigma$).
