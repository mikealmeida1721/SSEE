...
factor) and distinct from the saturation $\sm = 1+w_0 = 0.160050$, which fixes
...
physically necessary: using only $\sm=0.160050$ as the CMB matter density
...
matter saturation $\sm = 1 - \Omde = 1 + w_0 = 0.160050$ --- a number of the
...
\emph{no} matter factor: $\sm=0.160050$ (DESI) and $\omega_m$ (CMB) are two
...
$\Delta\mathrm{BIC}$ & ref & $-6.43$ & $-6.35$ \\
$\Delta\mathrm{DIC}$ & ref & $-5.66$ & $-4.02$ \\
...
\begin{description}[itemsep=1pt]
  ...
  \item[SSEE dynamical-only:] same EoS and $h$, but $\Omega_b = 0.048432$,
    $\Omega_{\rm cdm} = 0.111568$ (total $\sm = 0.160050$), using only the
...
SSEE ($s_m=0.160050$)             & 240 & 597 & 922 & 31.5\% \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{CMB TT power spectrum from CLASS\@. \textit{Blue dashed}: \LCDM\ Planck~2018.
         ...
         (not the dynamical $\sm = 0.160050$) is needed to reproduce the acoustic scale.}
\label{fig:cmb_tt}
\end{figure}
...
$\Omega_{\phi\mathrm{DM}}=\OmmCMB-\sm$. The first term is a measured density
...
$H_0^{\rm glob,IR} = H_0^{\rm SH0ES}(1-\fscr^{\rm IR})$ & $68.13$ km/s/Mpc & $3(\phiG+\pi)^2=67.96214$ ($0.17\sigma$; IR only, partial) & Retrodiction \\
...
\item \textbf{CMB Boltzmann.} CLASS confirms that the full $\omega_m$-direct
  ...
  dynamical $\sm=0.160050$, the CMB RMS residual jumps by a factor of 22 and
...
$\sm$ & $1 - \Omde = 1+w_0$ (saturation, not a density) & $0.160$ & --- \\
