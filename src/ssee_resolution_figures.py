...
Figure A: fig_rd_dual.pdf       — dual sound horizon r_d: pure 175.6 Mpc -> MIRA-mapped
                                   147.6 Mpc vs Planck 147.09±0.26 Mpc.
Figure B: fig_s8_resolution.pdf — S8: single-sector challenge 0.847 -> two-sector
                                   resolution 0.766 (0.01 sigma KiDS) vs data bands.
...
# Figure A — dual r_d
# ════════════════════════════════════════════════════════════════════════════
RD_PURE  = 175.6    # Mpc — SSEE pure geometric drag scale (Omega_m,dyn = 0.160)
RD_MIRA  = 147.6    # Mpc — MIRA-mapped (Omega_m,CMB = 0.31993), Eisenstein-Hu
...
bars = [
    (r'SSEE pure geometric' + '\n' + r'($\Omega_{m,\rm dyn}=0.160$)', RD_PURE,  '#d6604d'),
    (r'SSEE MIRA-mapped' + '\n' + r'($\Omega_{m,\rm CMB}=0.31993$)',  RD_MIRA,  '#1a9641'),
...
# MIRA arrow: pure -> mapped
ax.annotate('', xy=(RD_MIRA + 2, 1.42), xytext=(RD_PURE - 2, 2.0),
            arrowprops=dict(arrowstyle='-|>', lw=1.8, color='#222222',
                            connectionstyle='arc3,rad=0.25'))
ax.text(163, 1.86, r'$\times\,|w_0| = T_r/M_v$' + '\n' + r'(MIRA mapping)',
...
ax.set_title(r'Dual sound horizon: the pure SSEE scale and its MIRA-mapped value vs Planck',
             fontsize=10.5)
...
S8_SINGLE = (0.847, 0.006)   # single-sector baseline — "the challenge" (3.9 sigma KiDS)
S8_TWOSEC = 0.766            # two-sector canonical — 0.01 sigma KiDS (resolution)
...
ax.annotate(r'$3.9\sigma$ vs KiDS — the challenge',
...
ax.text(0.806, 0.52, 'free-streaming\n' + r'$k_{\rm fs}=0.659\,h/{\rm Mpc}$' + '\n'
        + r'($m_\varphi = 36.95$ eV, forward)',
