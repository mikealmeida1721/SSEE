...
none pruned---conservative, since it can only inflate the accidental-hit
count. We claim no temporal priority: \S\ref{sec:desi-agreement} is a
parameter-free \emph{postdiction}; falsifiable predictions are reserved for
\S\ref{sec:falsifiability}. The $55$ names collapse to $25$ distinct numbers
(e.g.\ $\mathrm{IGNIS}=K_v$, $\mathrm{SOLAR}=\mathrm{MAR}$, convergence at
$3\Omega$), so the count deduplicates by value.} The honest question is therefore: among
