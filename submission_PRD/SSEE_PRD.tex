...
set \citep{AlmeidaSSEE2026genesis}; the count runs over \emph{every} named
invariant, none pruned to improve the match---a conservative choice that can
only inflate, never shrink, the accidental-hit denominator. We claim no temporal
priority: \S\ref{sec:desi-agreement} is a parameter-free \emph{postdiction}, with
timestamped predictions reserved for unreleased data (\S\ref{sec:falsifiability}).
Many invariants share a value---the $55$ names collapse to $25$ distinct numbers
(e.g.\ $\mathrm{IGNIS}=K_v$, $\mathrm{SOLAR}=\mathrm{MAR}$, Sovereign convergence
at $3\Omega$)---so the count deduplicates by value.} The honest question is therefore: among
